...
export function useReissueAccessCertificate(databaseId: number) {
  const { t } = useTranslation();
  const { addToast } = useToast();

  return useMutation({
    mutationFn: async (accessId: number) => {
      const result = await apiClient.databases.reissueAccessCertificate({
        params: { id: databaseId, accessId },
        body: {},
      });
    ...
    onSuccess: () => {
      addToast({ type: 'success', title: t('toast.databaseCertificateReissued') });
    },
    onError: (error) => {
      addToast({ type: 'error', title: t('toast.databaseCertificateReissueError'), description: error.message });
    },
  });
}
...
// The CA belongs to the MariaDB instance, not to one access: the same file verifies the server for every remote access, on every database of this machine.
export function useDownloadCaCertificate() {
  const accessToken = useAuthStore((state) => state.accessToken);

  return () => {
    const params = new URLSearchParams({ token: accessToken ?? '' });
    window.open(`/api/databases/engine/ca?${params.toString()}`, '_blank');
  };
}
