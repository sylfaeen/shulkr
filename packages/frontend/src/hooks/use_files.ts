...
export function useDownloadFolder(serverId: string) {
  const accessToken = useAuthStore((state) => state.accessToken);
  return {
    download(folderPath: string) {
      const params = new URLSearchParams({ path: folderPath, token: accessToken ?? '' });
      window.open(`/api/servers/${serverId}/files/download-folder?${params.toString()}`, '_blank');
    },
  };
}
