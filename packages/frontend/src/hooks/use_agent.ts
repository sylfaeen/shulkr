...
export function useAgentConfig(serverId: string, enabled: boolean) {
  return useQuery({
    queryKey: [...agentKey(serverId), 'config'],
    queryFn: async () => {
      const result = await apiClient.agents.getAgentConfig({ params: { id: serverId } });
      if (result.status !== 200) raise(result.body, result.status);
      return result.body;
    },
