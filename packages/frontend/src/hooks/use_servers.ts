...
export function useDeleteServer() {
  ...
  const mutation = useMutation({
    ...
    onMutate: async ({ id }) => {
      ...
      const previous = queryClient.getQueryData<Array<{ id: string; status: string; deleting: boolean }>>(queryKey);

      if (previous) {
        queryClient.setQueryData<typeof previous>(
          queryKey,
          previous.map((s) => (s.id === id ? { ...s, status: 'deleting', deleting: true } : s))
        );
      }
