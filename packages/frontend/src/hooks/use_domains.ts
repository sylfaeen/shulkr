...
export function useSetPanelDomain() {
  ...
  const mutation = useMutation({
    ...
    onError: (error) => {
      // The settings page renders the DNS mismatch inline with both IPs and the fix, a toast would only duplicate it.
      if (error instanceof PanelDomainDnsMismatchError) return;
