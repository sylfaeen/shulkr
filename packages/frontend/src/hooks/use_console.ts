          ...
          case 'console:history':
            if (message.lines) {
              const historyMessages = message.lines.map(
                (line: { type: 'stdout' | 'stderr'; data: string; timestamp: number; level?: string; logTime?: string }) => ({
            ...
            if (message.lines) {
              setMessages((prev) => {
                const batchMessages = message.lines!.map(
                  (line: { outputType?: string; data: string; timestamp: number; level?: string; logTime?: string }) => ({
