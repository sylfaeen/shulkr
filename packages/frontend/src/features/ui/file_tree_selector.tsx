      ...
      >
        {isDirectory ? (
          <Button
            ...
            size={'icon-xs'}
            icon={ChevronRight}
            iconClass={cn('size-3.5 transition-transform duration-150', node.expanded && 'rotate-90')}
            onClick={() => onExpand(node.path)}
            className={
              'size-4 shrink-0 rounded text-zinc-600 transition-colors hover:text-zinc-600 dark:text-zinc-400 dark:hover:text-zinc-300'
            }
          />
        ) : (
