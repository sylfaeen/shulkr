...
function SelectTrigger({
  ...
  return (
    ...
    >
      {children}
      <SelectPrimitive.Icon render={<ChevronDownIcon className={cn('opacity-50', size === 'sm' ? 'size-3' : 'size-4')} />} />
    </SelectPrimitive.Trigger>
  );
}
    ...
    <SelectPrimitive.Portal>
      <SelectPrimitive.Positioner className={'z-50'}>
        <SelectPrimitive.Popup
          ...
          className={cn(
            'bg-popover text-popover-foreground data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-[closed]:animate-out data-[closed]:fade-out-0 data-[closed]:zoom-out-95 data-[open]:animate-in data-[open]:fade-in-0 data-[open]:zoom-in-95 border-border relative min-w-32 origin-[var(--transform-origin)] overflow-x-hidden overflow-y-auto rounded-md border shadow-md',
            className
          )}
          {...props}
        >
