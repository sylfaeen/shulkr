...
import {
  ...
  DropdownMenuItem,
  DropdownMenuTrigger,
} from '@shulkr/frontend/features/ui/base/dropdown-menu';
      ...
      {results.map((match) => (
        ...
        >
          <Button
            ...
            className={
              'flex h-auto w-full items-center justify-between rounded-none border-b border-black/10 px-4 py-2.5 text-left font-normal transition-colors hover:bg-zinc-50 dark:border-white/10 dark:hover:bg-zinc-800'
            }
          >
        ...
        {tables.map((table) => {
          ...
          return (
            <Button
              ...
              className={cn(
                'relative flex h-auto w-full items-center justify-between gap-2 rounded-none border-b border-black/5 py-2.5 pr-4 pl-4 text-left font-normal transition-colors last:border-b-0 dark:border-white/5',
  ...
  if (!isOpen) {
    return (
      <Button
        ...
        className={
          'group flex h-auto shrink-0 items-center gap-2.5 rounded-xl border border-dashed border-black/10 bg-transparent px-4 py-2.5 text-sm font-normal text-zinc-400 transition-all hover:border-solid hover:border-black/15 hover:bg-white hover:text-zinc-600 dark:border-white/10 dark:hover:border-white/15 dark:hover:bg-zinc-900 dark:hover:text-zinc-300'
        }
        icon={Terminal}
      >
        ...
        <div className={'max-h-40 overflow-y-auto border-b border-black/10 dark:border-white/10'}>
          {history.map((entry, i) => (
            <Button
              ...
              className={
                'flex h-auto w-full items-center justify-between gap-4 rounded-none px-4 py-2 text-left font-normal transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/50'
              }
            >
          ...
          {bookmarks.length === 0 ? (
            ...
            bookmarks.map((bm, i) => (
              ...
              >
                <Button
                  variant={'ghost'}
                  onClick={() => handleInsertSql(bm.sql)}
                  className={'flex h-auto min-w-0 flex-1 flex-col items-start justify-start rounded-none p-0 text-left font-normal hover:bg-transparent dark:hover:bg-transparent'}
                >
