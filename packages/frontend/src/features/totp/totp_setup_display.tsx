...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription } from '@shulkr/frontend/features/ui/base/dialog';
...
export function TotpQrDisplay({ qrCodeUri, secret }: { qrCodeUri: string; secret: string }) {
  ...
  const qrSrc = useMemo(() => {
    const svg = renderSVG(qrCodeUri, { border: 4, whiteColor: '#ffffff', blackColor: '#000000' });
    ...
    <div className={'grid gap-5 sm:grid-cols-2'}>
      ...
      >
        <div className={'rounded-lg bg-white p-3 shadow-sm dark:bg-zinc-800'}>
          <div className={'flex size-40 items-center justify-center'}>
            <img src={qrSrc} alt={'QR Code'} className={'size-40'} />
          </div>
        </div>
      ...
      >
        ...
        <Button
          ...
          iconClass={copied ? 'size-3.5 text-green-600' : undefined}
        >
          {copied ? t('settings.twoFactor.copied') : t('settings.twoFactor.copy')}
        </Button>
      </div>
    </div>
  );
    ...
    <div className={'space-y-4'}>
      ...
      <div className={'flex gap-2'}>
        <Button
          ...
          iconClass={copied ? 'size-3.5 text-green-600' : undefined}
        >
          {copied ? t('settings.twoFactor.copied') : t('settings.twoFactor.recoveryCodes.copyAll')}
        </Button>
        <Button onClick={handleDownload} variant={'secondary'} size={'sm'} className={'flex-1'} icon={Download}>
      ...
      <div className={'p-5'}>
        {enabled ? (
          <div className={'space-y-4'}>
            ...
            <Button onClick={onDisable} variant={'outline'} size={'sm'} icon={ShieldOff} {...{ loading }}>
          ...
          <div className={'space-y-4'}>
            ...
            <Button onClick={onSetupStart} size={'sm'} icon={Shield} {...{ loading }}>
