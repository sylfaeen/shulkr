...
export function TaskStatsCards({ serverId, taskId }: TaskStatsCardsProps) {
...
function StatCard({
  ...
  return (
    <div
      className={'rounded-lg border border-black/6 bg-white/60 px-3 py-2 dark:border-white/6 dark:bg-zinc-900/40'}
    >
      <div className={'flex items-center gap-1.5 text-[11px] text-zinc-500 dark:text-zinc-500'}>
        {icon}
        <span>{label}</span>
      </div>
      <div className={cn('mt-0.5 font-jetbrains text-lg font-medium tabular-nums', accentClass ?? 'text-zinc-800 dark:text-zinc-200')}>
        {value}
      </div>
    </div>
  );
