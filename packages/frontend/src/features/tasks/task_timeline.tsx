...
import { type TaskExecution, type TaskExecutionStatus } from '@shulkr/frontend/hooks/use_tasks';
...
export function TaskTimeline({ executions, limit = 30 }: TaskTimelineProps) {
  const { t } = useTranslation();

  const slice = [...executions]
...
function TimelineCell({ exec }: { exec: TaskExecution }) {
  ...
  const title = t(`tasks.timeline.tooltip`, {
    status: t(`tasks.executionStatus.${normalizeStatus(exec.status)}`),
    date,
  ...
  return (
    <span
      title={title}
      className={cn(
        'h-5 flex-1 min-w-1 max-w-2 rounded-sm transition',
        colorForStatus(exec.status)
      )}
    />
  );
}
