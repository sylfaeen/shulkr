...
export function ExecutionDetail({ exec }: { exec: TaskExecution }) {
  ...
  return (
    <div className={'mt-2 rounded-md border border-black/6 bg-zinc-50/50 p-3 dark:border-white/6 dark:bg-zinc-900/30'}>
      ...
      {exec.output && (
        <div className={'mt-3'}>
          <div className={'mb-1 text-xs font-medium text-zinc-600 dark:text-zinc-400'}>{t('tasks.executionDetail.output')}</div>
          <pre
            className={
              'font-jetbrains max-h-40 overflow-auto rounded-md bg-zinc-900/80 p-2.5 text-xs leading-relaxed text-zinc-100'
            }
          >
            {exec.output}
          </pre>
      ...
      {steps.map((step) => {
        ...
        return (
          ...
          >
            <span
              className={
                'font-jetbrains flex size-5 shrink-0 items-center justify-center rounded-md bg-zinc-100 text-xs font-medium text-zinc-600 tabular-nums dark:bg-zinc-800 dark:text-zinc-400'
              }
            >
              {step.step + 1}
            </span>
            ...
            <div className={'min-w-0 flex-1'}>
              <div className={'flex items-center gap-2'}>
                ...
                <span className={'font-jetbrains text-xs text-zinc-500 tabular-nums dark:text-zinc-500'}>
                  {t('tasks.duration', { ms: step.durationMs })}
                </span>
              </div>
              {step.error && <div className={'mt-0.5 truncate text-xs text-red-400'}>{step.error}</div>}
            </div>
          </li>
        );
      })}
...
function DetailField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <div className={'text-xs font-medium text-zinc-500 dark:text-zinc-500'}>{label}</div>
