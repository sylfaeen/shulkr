...
import { type TaskExecution, type TaskExecutionStatus, type TaskStepResult } from '@shulkr/frontend/hooks/use_tasks';

type ExecutionDetailProps = {
  exec: TaskExecution;
};

export function ExecutionDetail({ exec }: ExecutionDetailProps) {
      ...
      <div className={'grid gap-3 md:grid-cols-3'}>
        <DetailField label={t('tasks.executionDetail.status')}>
          <div className={'flex items-center gap-1.5'}>
            ...
            <span className={'text-xs'}>{t(`tasks.executionStatus.${normalizeStatus(exec.status)}`)}</span>
          </div>
        </DetailField>
    ...
    <ol className={'space-y-1.5'}>
      {steps.map((step) => {
        ...
        return (
          <li
            key={step.step}
            className={'flex items-start gap-2 rounded-md border border-black/6 bg-white/60 p-2 dark:border-white/6 dark:bg-zinc-800/30'}
          >
            <span
              className={
...
function normalizeStatus(status: TaskExecutionStatus): 'success' | 'error' | 'skipped' | 'running' | 'pending' {
