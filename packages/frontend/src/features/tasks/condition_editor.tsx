                ...
                {(['and', 'or'] as const).map((mode) => {
                  ...
                  return (
                    <Button
                      ...
                      className={cn(
                        'h-auto rounded-md border px-2.5 py-1 text-xs transition',
                        isActive
    ...
    <div className={'flex flex-wrap items-center gap-2'}>
      <Select value={rule.type} onValueChange={(v) => setType(v as ConditionRuleType)}>
        <SelectTrigger className={'h-8 w-40 text-xs'}>
          <SelectValue>{(value: string) => t(`tasks.conditions.types.${value}`)}</SelectValue>
        </SelectTrigger>
        <SelectContent>
      ...
      {rule.type === 'server_status' && (
        <Select value={(rule.config.status as string | undefined) ?? 'online'} onValueChange={(v) => updateConfig({ status: v })}>
          <SelectTrigger className={'h-8 w-32 text-xs'}>
            <SelectValue>{(value: string) => t(`tasks.conditions.serverStatus.${value}`)}</SelectValue>
          </SelectTrigger>
          <SelectContent>
