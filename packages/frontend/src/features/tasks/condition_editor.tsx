...
type PlayerOperator = (typeof PLAYER_OPERATORS)[number];
    ...
    <div className={'space-y-3 rounded-lg border border-black/8 bg-zinc-50/50 p-3 dark:border-white/8 dark:bg-zinc-900/30'}>
      <div className={'flex items-start gap-3'}>
        <Switch
          ...
          aria-label={t('tasks.conditions.title')}
          className={'mt-0.5'}
        />
        <div>
  ...
  return (
    <div className={'flex flex-wrap items-center gap-2'}>
      ...
      {rule.type === 'server_status' && (
        <Select
          value={(rule.config.status as string | undefined) ?? 'online'}
      ...
      {rule.type === 'player_count' && (
        <>
          <Select
            value={(rule.config.operator as PlayerOperator | undefined) ?? '>'}
