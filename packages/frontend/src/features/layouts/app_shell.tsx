    ...
    <SidebarProvider>
      <div className={cn('shulkr', hasNewVersion && 'new-version')}>
        <a
          href={'#main-content'}
          className={
            'sr-only focus:not-sr-only focus:fixed focus:top-2 focus:left-2 focus:z-50 focus:rounded-lg focus:bg-zinc-900 focus:px-4 focus:py-2 focus:text-sm focus:font-medium focus:text-white focus:shadow-lg'
          }
        >
          Skip to content
        </a>
...
function UpdateBanner({ data }: { data: { currentVersion: string; latestVersion: string | null; ipAddress: string | null } }) {
    ...
    <div className={'h-10 shrink-0 border-b border-purple-200 bg-purple-100'}>
      <div className={'flex'}>
        ...
        <div className={'flex max-w-325 flex-1 items-center justify-between gap-4 px-4 py-2.5 sm:px-6 lg:pl-5 xl:pr-0'}>
          ...
          <a
            href={`${DOCS_URL}/Configuration#updating-shulkr`}
            target={'_blank'}
...
function SidebarToggle() {
  ...
  return (
    <div className={'flex items-center justify-between border-b border-black/10 px-4 py-3 min-[960px]:hidden'}>
      ...
      <Button onClick={() => setMobileOpen(true)} variant={'secondary'} size={'icon-sm'} aria-label={'Open menu'} icon={Menu} />
    </div>
  );
}
