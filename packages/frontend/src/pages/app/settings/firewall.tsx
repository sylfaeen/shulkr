...
function FirewallListSection() {
  ...
  const filteredRules = useMemo(() => {
    ...
    return rules.filter(
      (r) => r.label.toLowerCase().includes(q) || (r.from_ip ?? '').toLowerCase().includes(q) || String(r.port ?? '').includes(q)
    );
  }, [rules, searchQuery]);

  const { items: sortedRules, control: sortControl } = useSortableList<FirewallRule>(filteredRules, {
    storageKey: 'firewall',
    fields: [
      { key: 'label', label: t('common.sort.byLabel'), accessor: (r) => r.label },
      { key: 'action', label: t('common.sort.byAction'), accessor: (r) => r.action },
      { key: 'createdAt', label: t('common.sort.byCreatedAt'), accessor: (r) => r.created_at, type: 'date' },
    ],
  });
  ...
  return (
    <>
          ...
          <div className={'min-w-0'}>
            <div className={'flex items-center gap-2'}>
              <Badge variant={rule.action === 'allow' ? 'success' : 'destructive'}>
                {t(rule.action === 'allow' ? 'settings.firewall.actionAllow' : 'settings.firewall.actionDeny')}
              </Badge>
              <span className={'font-jetbrains text-sm font-medium text-zinc-800 tabular-nums dark:text-zinc-200'}>
                {rule.port ?? t('settings.firewall.anyPort')}
              </span>
              <Badge>{PROTOCOL_LABELS[rule.protocol]}</Badge>
              {rule.from_ip && (
                <span className={'font-jetbrains text-xs text-zinc-500 dark:text-zinc-400'}>← {rule.from_ip}</span>
              )}
                  ...
                  <TooltipTrigger
                    render={
                      <div className={'flex items-center'}>
                        <Switch checked={rule.enabled} onCheckedChange={handleToggleChange} disabled={!canToggle} />
                      </div>
                    }
                  />
                  <TooltipContent className={'rounded-lg px-2.5 py-1.5 text-sm'}>
                    {rule.enabled ? t('settings.firewall.tooltipDisable') : t('settings.firewall.tooltipEnable')}
                  </TooltipContent>
                </Tooltip>
                {canRemove && (
                  <Tooltip>
                    ...
                    />
                    <TooltipContent className={'rounded-lg px-2.5 py-1.5 text-sm'}>{t('settings.firewall.tooltipDelete')}</TooltipContent>
                  </Tooltip>
                )}
              </TooltipProvider>
            </>
      ...
      <PasswordGate
        ...
        title={t('settings.firewall.tooltipDelete')}
        description={t('settings.firewall.deleteRuleDescription')}
        confirmLabel={t('settings.firewall.tooltipDelete')}
        destructive
        onConfirm={async () => {
