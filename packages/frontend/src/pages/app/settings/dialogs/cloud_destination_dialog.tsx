...
import { z } from 'zod';
import { Boxes, Check, Cloud, Database, ExternalLink, Server, X, Zap } from 'lucide-react';
import {
  Dialog,
  DialogBody,
  DialogContent,
...
const PROVIDER_PRESETS: Record<CloudProvider, ProviderPreset> = {
  ...
  ovh: {
    ...
    defaultRegion: 'gra',
    defaultEndpoint: 'https://s3.gra.io.cloud.ovh.net',
    endpointPlaceholder: 'https://s3.{region}.io.cloud.ovh.net',
    credentialsUrl: 'https://www.ovh.com/manager/#/public-cloud/pci/projects',
    regionHint: 'e.g. gra, sbg, bhs, de, uk, waw',
  },
  scaleway: {
    provider: 'scaleway',
    icon: Boxes,
    ...
    credentialsUrl: 'https://console.scaleway.com/iam/api-keys',
    regionHint: 'e.g. fr-par, nl-ams, pl-waw',
  },
  'minio-custom': {
...
const PROVIDERS_ORDER: Array<CloudProvider> = ['aws-s3', 'cloudflare-r2', 'ovh', 'scaleway', 'minio-custom'];
...
export function CloudDestinationDialog({
  ...
  const destinationSchema = z.object({
    ...
    provider: z.enum(['aws-s3', 'cloudflare-r2', 'ovh', 'scaleway', 'minio-custom']),
                    ...
                    {PROVIDERS_ORDER.map((p) => {
                      ...
                      return (
                        <Button
                          ...
                          className={cn(
                            'h-auto flex-col items-start rounded-lg border p-3 text-left transition-all',
                            active
                              ? preset.accentClass
        ...
        <DialogFooter className={'justify-between!'}>
          <div className={'flex items-center justify-start gap-2'}>
            <Button
              ...
              icon={!testResult ? Check : undefined}
            >
              {testResult ? <TestResultDisplay result={testResult} /> : t('cloudDestinations.testConnection')}
            </Button>
          </div>
...
function TestResultDisplay({ result }: { result: TestConnectionResult }) {
  ...
  return (
    <div>
      <div className={cn('flex items-center gap-2', allOk ? 'text-green-500' : 'text-red-500')}>
