...
import { Form, FormField, FormItem, FormLabel, FormControl, FormMessage, FormDescription } from '@shulkr/frontend/features/ui/base/form';
import {
  FIREWALL_PRESETS,
  PRESET_STYLES,
  PROTOCOL_STYLES,
...
function RulePreview({
  ...
  return (
    <div className={'rounded-md border border-dashed border-black/10 bg-zinc-50/50 px-3 py-2 dark:border-white/10 dark:bg-zinc-900/40'}>
      ...
      <div className={'font-jetbrains text-xs text-zinc-600 dark:text-zinc-400'}>
        <span className={cn('font-semibold', tone)}>{verb.toUpperCase()}</span>{' '}
        <span>{protocol.toUpperCase()}</span>{' '}
        <span className={'text-zinc-400 dark:text-zinc-500'}>{t('settings.firewall.previewFrom')}</span>{' '}
        <span>{fromText}</span>{' '}
        <span className={'text-zinc-400 dark:text-zinc-500'}>{t('settings.firewall.previewToPort')}</span>{' '}
        <span>{portText}</span>
      </div>
    </div>
  );
