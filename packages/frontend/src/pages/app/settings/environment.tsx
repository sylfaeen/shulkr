          ...
          {revealed && (
            <FeatureCard.Footer>
              <div className={'flex items-center justify-end gap-2'}>
                <Button onClick={handleCancel} variant={'ghost'} size={'sm'} disabled={!hasChanges}>
                  {t('common.cancel')}
                </Button>
                <Button onClick={handleSave} size={'sm'} disabled={!canWrite || !hasChanges || saveStatus === 'saving'} icon={Save}>
                  {t('common.save')}
                </Button>
              </div>
            </FeatureCard.Footer>
...
function RevealOverlay({ onReveal }: { onReveal: () => void }) {
  ...
  return (
    ...
    >
      ...
      <Button onClick={onReveal} icon={Eye}>
