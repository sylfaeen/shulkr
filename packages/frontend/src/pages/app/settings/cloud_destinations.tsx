...
export function CloudDestinationsPage() {
  ...
  const { items: sortedDestinations, control: sortControl } = useSortableList<CloudDestinationResponse>(destinations, {
              ...
              sortedDestinations.map((dest) => (
                <FeatureCard.Row key={dest.id} interactive={canUpdate} onClick={() => canUpdate && setEditingDest(dest)}>
                  ...
                  <div className={'flex shrink-0 items-center gap-2'} onClick={(e) => e.stopPropagation()}>
                    ...
                    {canDelete && (
                      <Button variant={'ghost-destructive'} size={'icon'} onClick={() => setGateDestId(dest.id)} icon={Trash2} />
                    )}
                  </div>
                </FeatureCard.Row>
              ))
