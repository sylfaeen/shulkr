...
export function CloudDestinationsPage() {
  ...
  const filteredDestinations = useMemo(() => {
    ...
    return destinations.filter((d) => d.name.toLowerCase().includes(q) || d.bucket.toLowerCase().includes(q));
  }, [destinations, searchQuery]);

  const { items: sortedDestinations, control: sortControl } = useSortableList<CloudDestinationResponse>(filteredDestinations, {
    storageKey: 'cloud-destinations',
    fields: [
      { key: 'name', label: t('common.sort.byName'), accessor: (d) => d.name },
      { key: 'createdAt', label: t('common.sort.byCreatedAt'), accessor: (d) => d.createdAt, type: 'date' },
    ],
  });
      ...
      <PageContent>
        <FeatureCard>
          <FeatureCard.Header>
            ...
            <FeatureCard.Actions>
              {destinations && destinations.length > 0 && sortControl}
              {canCreate && (
