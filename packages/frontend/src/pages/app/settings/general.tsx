...
function DiskUsageSection() {
  ...
  const shulkrPercent = data ? Math.round((data.shulkr.total / data.disk.total) * 100) : 0;
          ...
          <>
            ...
            <FeatureCard.Row layout={'column'} className={'gap-3'}>
              <div className={'flex w-full items-center justify-between'}>
                ...
                <span className={'font-jetbrains text-sm text-zinc-600 tabular-nums dark:text-zinc-400'}>
                  {formatFileSize(data.shulkr.total)} ({shulkrPercent}%)
                </span>
              </div>
              {data.shulkr.servers.length > 0 && (
                <div className={'w-full space-y-1.5'}>
                  {data.shulkr.servers.map((server) => (
