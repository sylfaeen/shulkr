...
import { AlertTriangle, CheckCircle2, Copy, Globe, Info, Lock, Coffee, Trash2, Loader2, HardDrive, Server } from 'lucide-react';
...
function PanelDomainSection() {
  ...
  const dnsMismatch = setPanelDomain.error instanceof PanelDomainDnsMismatchError ? setPanelDomain.error : null;
  const sslFailure = setPanelDomain.error instanceof PanelDomainSslError ? setPanelDomain.error : null;
  const hstsTld = panelDomain ? getHstsPreloadedTld(panelDomain.domain) : null;
      ...
      {panelDomain ? (
        <>
          ...
          <FeatureCard.Footer alert>
            {!panelDomain.sslEnabled && hstsTld && <HstsWarning tld={hstsTld} />}
        ...
        <FeatureCard.Body>
          <div className={'space-y-0 divide-y divide-black/4 dark:divide-white/6'}>
            ...
            <div className={'px-5 py-4'}>
              ...
              {dnsMismatch && <DnsMismatchAlert error={dnsMismatch} fallbackServerIp={serverIp} />}
              {sslFailure && <SslFailureAlert error={sslFailure} />}
            </div>
          </div>
        </FeatureCard.Body>
      )}
...
function HstsWarning({ tld }: { tld: string }) {
  const { t } = useTranslation();

  return (
    <Alert variant={'warning'} className={'mt-3'}>
      <Lock className={'size-4'} />
      <AlertDescription>{t('appSettings.panelDomain.hstsWarning', { tld })}</AlertDescription>
    </Alert>
  );
}

function DnsMismatchAlert({ error, fallbackServerIp }: { error: PanelDomainDnsMismatchError; fallbackServerIp: string }) {
  const messageKey = {
    mismatch: 'dnsMismatch',
    propagating: 'dnsPropagating',
    unresolved: 'dnsUnresolved',
  } as const;

  const { t } = useTranslation();
  const serverIp = error.serverIp ?? fallbackServerIp;
  const resolvedIp = error.resolvedIp ?? '';

  return (
    <Alert variant={error.reason === 'propagating' ? 'warning' : 'destructive'} className={'mt-3'}>
      <AlertTriangle className={'size-4'} />
      <AlertDescription>
        <p>{t(`appSettings.panelDomain.${messageKey[error.reason]}`, { domain: error.domain, resolvedIp, serverIp })}</p>
        {error.reason === 'mismatch' && resolvedIp && isCloudflareIp(resolvedIp) && (
          <p className={'mt-1.5'}>{t('appSettings.panelDomain.cloudflareProxyHint')}</p>
        )}
      </AlertDescription>
    </Alert>
  );
...
function SslFailureAlert({ error }: { error: PanelDomainSslError }) {
  const { t } = useTranslation();
  ...
  return (
    <Alert variant={'destructive'} className={'mt-3'}>
      <AlertTriangle className={'size-4'} />
      <AlertDescription>
        <p>{t('appSettings.panelDomain.sslFailed')}</p>
        <pre className={'font-jetbrains mt-2 max-h-40 overflow-auto text-xs break-all whitespace-pre-wrap'}>{error.detail}</pre>
      </AlertDescription>
    </Alert>
  );
}
...
function JavaSection() {
  ...
  const filteredJava = useMemo(() => {
    const list = javaVersions ?? [];
    const q = searchQuery.trim().toLowerCase();
    if (!q) return list;

    return list.filter((j) => j.name.toLowerCase().includes(q) || j.path.toLowerCase().includes(q));
  }, [javaVersions, searchQuery]);
