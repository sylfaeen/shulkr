...
import { BookOpen, Globe, Server, Settings, Users } from 'lucide-react';
import {
  useSidebarItems,
...
export function MainLayout() {
  ...
  const sections = useMemo((): Array<SidebarNavSection> => {
    ...
    if (can('settings:cloud-destinations'))
      settingsChildren.push({ key: 'settingsCloudDestinations', path: '/app/settings/cloud-destinations' });
    ...
    if (can('shares:files')) {
      items.push({ key: 'shares', path: '/app/public-files', icon: Globe, children: undefined });
    }
    ...
    return [
      {
        items: [...items, { key: 'docs', path: DOCS_URL, icon: BookOpen, bottom: true, external: true, children: undefined }],
      },
    ];
  }, [servers, can]);
