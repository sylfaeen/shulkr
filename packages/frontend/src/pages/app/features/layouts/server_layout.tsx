...
function getServerNavSections(
  serverId: string,
  serverName: string,
  can: (...perms: Array<PermissionId>) => boolean
  ...
  if (can('server:databases')) items.push({ key: 'databases', path: `${basePath}/databases`, icon: Database });
  ...
  items.push({ key: 'docs', path: DOCS_URL, icon: BookOpen, bottom: true, external: true });
