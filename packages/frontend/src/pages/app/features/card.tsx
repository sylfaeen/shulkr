...
FeatureCard.Footer = function FeatureCardFooter({
  ...
  return <div className={cn(alert ? 'pt-1' : 'p-4', className)} {...rest} />;
};
...
FeatureCard.Search = function FeatureCardSearch({
  value,
  onChange,
  placeholder,
}: {
  ...
  return (
    <FeatureCard.Row className={'bg-white py-0 dark:bg-zinc-800/50'}>
      <div className={'relative w-full'}>
        <Search className={'absolute top-1/2 left-0 size-3.5 -translate-y-1/2 text-zinc-500'} strokeWidth={2} />
        <Input
          type={'text'}
          value={value}
          ...
          className={
            'h-9 rounded-none border-0 bg-transparent pl-6 text-sm shadow-none focus-visible:bg-transparent focus-visible:ring-0'
          }
        />
      </div>
    </FeatureCard.Row>
