...
function StatusBadge({ phase }: { phase: AgentPhase }) {
  ...
  switch (phase) {
    ...
    case 'notInstalled':
      return <Badge variant={'outline'}>{t('agent.status.notInstalled')}</Badge>;
    case 'outdated':
      return <Badge className={'bg-amber-500/20 text-amber-700 dark:text-amber-300'}>{t('agent.status.outdated')}</Badge>;
    case 'connected':
      return <Badge className={'bg-emerald-500/20 text-emerald-700 dark:text-emerald-300'}>{t('agent.status.connected')}</Badge>;
    case 'pending':
      return <Badge className={'bg-amber-500/20 text-amber-700 dark:text-amber-300'}>{t('agent.status.pending')}</Badge>;
    default:
