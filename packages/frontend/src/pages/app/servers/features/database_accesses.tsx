...
export function DatabaseAccesses({
  ...
  return (
    <div className={'bg-muted/40 border-border ml-5 border-l-2 py-3 pr-5 pl-4'}>
      <div className={'flex items-center justify-between gap-4'}>
        <p className={'text-xs font-medium tracking-wide text-zinc-500 uppercase dark:text-zinc-400'}>
          {t('databases.access.title')}
        </p>
      ...
      {isLoading ? (
        ...
        <p className={'text-xs text-zinc-500 dark:text-zinc-400'}>{t('databases.access.empty')}</p>
      ) : (
...
function AccessRow({ access, databaseId, serverId }: { access: DatabaseAccessResponse; databaseId: number; serverId: string }) {
  ...
  const [certificate, setCertificate] = useState<{ clientCertificate: string; clientKey: string } | null>(null);
  ...
  const handleReissue = async () => {
    setCertificate(await reissueCertificate.mutateAsync(access.id));
  };
  ...
  return (
    <div className={'border-border rounded-lg border bg-white p-3 dark:bg-zinc-900'}>
