...
import { useCommandSuggestions } from '@shulkr/frontend/hooks/use_command_suggestions';
...
export function ServerConsole({
  ...
  const suggestions = useCommandSuggestions(serverId, inputValue);
  ...
  const ghostText =
    currentSuggestion && inputValue && currentSuggestion.startsWith(inputValue)
      ? currentSuggestion.slice(inputValue.length)
      : null;
            ...
            >
              <div className={'relative flex-1'}>
                ...
                {ghostText && (
                  <span
                    className={
                      'font-jetbrains pointer-events-none absolute top-1/2 left-8 -translate-y-1/2 text-sm text-zinc-600'
                    }
                    aria-hidden={'true'}
                  >
              ...
              <Button type={'submit'} variant={'ghost'} size={'sm'} disabled={!isConnected || !inputValue.trim()} icon={Send} />
            </form>
          </>
        )}
      </div>
