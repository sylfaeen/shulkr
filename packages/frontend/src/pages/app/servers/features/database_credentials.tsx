...
function ConnectionUrl({ url, password }: { url: string; password: string }) {
  const tunnelled = url.startsWith('mysql+ssh://');
  const { t } = useTranslation();
  const [copied, setCopied] = useState(false);
  const [revealed, setRevealed] = useState(false);

  // Copies, then hands the URL to the operating system so the registered client (TablePlus and friends) opens on it. The browser asks before launching an external application, and nothing happens if no client is registered, hence the copy first.
  ...
  return (
    <div className={'border-border space-y-1.5 rounded-lg border p-3'}>
      ...
      <p className={'text-xs text-zinc-400 dark:text-zinc-500'}>
        {tunnelled ? t('databases.credentials.connectionUrlHint') : t('databases.credentials.connectionUrlDirectHint')}
      </p>
    </div>
  );
}
...
// Shared by the creation dialog and the reissue action on an existing access. The private key exists only in this response, it is never stored anywhere.
export function ClientCertificateBlock({ certificate, privateKey }: { certificate: string; privateKey: string }) {
  const { t } = useTranslation();
  ...
  return (
    <div className={'space-y-2 rounded-lg border border-amber-200 bg-amber-50 p-3 dark:border-amber-900 dark:bg-amber-950/30'}>
      <p className={'text-sm text-amber-800 dark:text-amber-400'}>{t('databases.access.certificateNotice')}</p>
      <div className={'flex gap-2'}>
        <Button size={'sm'} variant={'outline'} onClick={() => download(certificate, 'client-cert.pem')}>
          {t('databases.access.downloadClientCert')}
        </Button>
        <Button size={'sm'} variant={'outline'} onClick={() => download(privateKey, 'client-key.pem')}>
          {t('databases.access.downloadClientKey')}
        </Button>
      </div>
    </div>
  );
