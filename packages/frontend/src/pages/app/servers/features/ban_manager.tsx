...
export function BanManager({ serverId }: { serverId: string }) {
  ...
  const { items: sortedPlayerEntries, control: sortControl } = useSortableList(playerEntries, {
    storageKey: 'bans-players',
    fields: [
      { key: 'name', label: t('common.sort.byName'), accessor: (e) => e.name },
      { key: 'created', label: t('common.sort.byCreatedAt'), accessor: (e) => e.created, type: 'date' },
    ],
  });

  const { items: sortedIpEntries } = useSortableList(ipEntries, {
    storageKey: 'bans-ips',
    fields: [
      { key: 'ip', label: 'IP', accessor: (e) => e.ip },
      { key: 'created', label: t('common.sort.byCreatedAt'), accessor: (e) => e.created, type: 'date' },
    ],
  });
  ...
  return (
    <FeatureCard>
      <FeatureCard.Header>
        ...
        {totalCount > 0 && <FeatureCard.Actions>{sortControl}</FeatureCard.Actions>}
      </FeatureCard.Header>
      <FeatureCard.Body>
        {isLoading ? (
          <div className={'p-3'}>
