            ...
            data.sessions.map((session) => (
              ...
              >
                <div className={'flex items-center gap-2'}>
                  {session.leftAt ? <LogOut className={'size-3 text-red-400'} /> : <LogIn className={'size-3 text-green-500'} />}
                ...
                <div className={'flex items-center gap-3 text-zinc-500 dark:text-zinc-400'}>
                  ...
                  {session.leftAt === null && <span className={'text-green-500'}>{t('players.online', { count: '' })}</span>}
                </div>
              </div>
            ))
          )}
