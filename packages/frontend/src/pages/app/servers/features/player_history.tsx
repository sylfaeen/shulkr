...
export function PlayerHistoryPanel({ serverId }: { serverId: string }) {
  ...
  const { data } = usePlayerHistory(serverId, limit);

  const hasMore = data ? data.sessions.length < data.total && limit < MAX_RECORDS : false;
          ...
          {data.sessions.length === 0 ? (
            ...
            <>
              {data.sessions.map((session) => (
                ...
                >
                  <div className={'flex items-center gap-2'}>
                    ...
                    <PlayerLink name={session.playerName} {...{ serverId }} />
                  </div>
              ...
              {hasMore && (
                <button
                  type={'button'}
                  onClick={() => setLimit((prev) => Math.min(prev + PAGE_SIZE, MAX_RECORDS))}
                  className={
