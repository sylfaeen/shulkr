...
const backupStrategySchema = z
  .object({
    ...
    localRetentionCount: z.coerce.number().min(1).optional(),
              ...
              >
                ...
                {mode === 'hybrid' && (
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel className={'text-xs font-medium'}>{t('backupStrategy.localRetentionCount')}</FormLabel>
                        <FormControl>
                          <Input
                            type={'number'}
                            min={1}
                        ...
                        <FormDescription className={'text-[11px]'}>{t('backupStrategy.localRetentionHint')}</FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
...
function ModeCard({ icon: Icon, active, accentClass, title, description, disabled, onClick }: ModeCardProps) {
  return (
    <button
      type={'button'}
      onClick={onClick}
