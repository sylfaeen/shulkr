...
export function BackupStrategySection({ serverId }: { serverId: string }) {
  ...
  return (
    <FeatureCard>
      ...
      <FeatureCard.Body className={'space-y-4'}>
        ...
        {noCloudConfigured && local.mode === 'local-only' && (
          <div className={'rounded-md border border-amber-500/20 bg-amber-500/5 p-3 text-xs text-amber-700 dark:text-amber-400'}>
            {t('backupStrategy.hints.noDestinations')}
          </div>
        )}
        ...
        {(local.mode === 'cloud-only' || local.mode === 'hybrid') && (
          <div className={'space-y-3 rounded-lg border border-black/6 bg-zinc-50/50 p-3 dark:border-white/6 dark:bg-zinc-900/30'}>
            <div>
              ...
              <Select value={local.cloudDestinationId ?? ''} onValueChange={(v) => update({ cloudDestinationId: v })} disabled={!canEdit}>
            ...
            <div>
              ...
              />
              <p className={'mt-1 text-[11px] text-zinc-500 dark:text-zinc-500'}>
                {t('backupStrategy.cloudRetentionHint')}
              </p>
            </div>
          </div>
        )}
...
function ModeCard({
  ...
  return (
    ...
    >
      <Icon className={'mb-2 size-4 text-zinc-600 dark:text-zinc-400'} strokeWidth={2} />
