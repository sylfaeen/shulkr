...
export function WhitelistManager({ serverId }: { serverId: string }) {
  ...
  const { items: sortedEntries, control: sortControl } = useSortableList(data?.entries, {
    storageKey: 'whitelist',
    fields: [{ key: 'name', label: t('common.sort.byName'), accessor: (e) => e.name }],
  });
  ...
  return (
    <FeatureCard>
      <FeatureCard.Header>
        ...
        {data && data.entries.length > 0 && <FeatureCard.Actions>{sortControl}</FeatureCard.Actions>}
      </FeatureCard.Header>
      <FeatureCard.Body>
        {canWhitelist && (
