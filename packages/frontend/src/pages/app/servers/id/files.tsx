...
function FilesSection({
  ...
  const { download: downloadFolder } = useDownloadFolder(serverId);
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={
            'flex flex-wrap items-center justify-between gap-2 border-b border-black/10 px-3 py-2 sm:px-4 sm:py-2.5 dark:border-white/10'
          }
        >
...
function FileRow({
  ...
  return (
    ...
    >
      <td className={'w-full px-3 py-1.5 sm:px-4'} onClick={action === 'idle' ? handleClick : undefined}>
        <div className={'flex min-w-0 items-center gap-2.5'}>
          <div className={'flex size-7 shrink-0 items-center justify-center rounded-md bg-zinc-100 dark:bg-zinc-800'}>
            {isDirectory ? (
      ...
      <td
        className={cn('hidden px-2 py-1.5 text-right whitespace-nowrap sm:table-cell', action !== 'idle' && 'invisible')}
      ...
      >
        <span className={'font-jetbrains text-sm text-zinc-600 tabular-nums dark:text-zinc-400'}>
          {file.type === 'directory' && file.size === 0 ? (
            <LoaderCircle className={'float-end size-3 animate-spin'} strokeWidth={2} />
          ) : (
      ...
      <td className={'px-3 py-1.5 text-right whitespace-nowrap sm:px-4'}>
        {action === 'rename' ? (
