...
import { z } from 'zod';
import { AlertTriangle, Check, Download, Info, Package, RotateCcw, Trash2, Upload } from 'lucide-react';
        ...
        <ServerPageHeader.Left>
          ...
          <ServerPageHeader.Info>
            <ServerPageHeader.Heading>
              ...
              <ServerPageHeader.Docs page={'Configuration'} />
            </ServerPageHeader.Heading>
...
function JarListSection({ serverId }: { serverId: string }) {
  ...
  const filteredJars = useMemo(() => {
    ...
    return jars.filter((j) => j.name.toLowerCase().includes(q));
  }, [jars, searchQuery]);
    ...
    <FeatureCard>
      ...
      <FeatureCard.Body>
        {jarsLoading ? (
          ...
          <FeatureCard.Empty
            icon={Package}
            title={t('settings.noJarsFound')}
            description={t('settings.noJarsFoundDescription')}
          />
        ) : (
          <>
...
function PaperMCSection({ serverId }: { serverId: string }) {
  ...
  const paperMcSchema = z.object({
    project: z.enum(['paper', 'folia', 'velocity', 'waterfall']),
    version: z.string().min(1),
    build: z.number().int().nullable(),
  });

  type PaperMcFormValues = z.infer<typeof paperMcSchema>;

  const form = useForm<PaperMcFormValues>({
    resolver: zodResolver(paperMcSchema),
    defaultValues: { project: 'paper', version: '', build: null },
  });
  ...
  const { data: buildsData, isLoading: buildsLoading } = usePaperBuilds(selectedProject, selectedVersion || null);
  const versions = versionsData?.versions;
  const builds = buildsData?.builds;
  const isDownloading = downloadJar.isPending;

  if (!canDownload) return null;

  const handleDownload = async (data: PaperMcFormValues) => {
    ...
    try {
      ...
      form.reset({ project: data.project, version: '', build: null });
    } catch (err) {
            ...
            <div className={'rounded-xl border border-black/6 bg-zinc-50/50 p-4 dark:border-white/6 dark:bg-zinc-800/50'}>
              <div className={'grid grid-cols-1 items-end gap-3 sm:grid-cols-2 lg:grid-cols-4'}>
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t('settings.papermc.software')}</FormLabel>
                      <FormControl>
                        <Select
                          value={field.value}
                          onValueChange={(value) => {
                            ...
                            form.setValue('build', null);
                          }}
                          disabled={isDownloading}
                        >
                          <SelectTrigger>
                            <SelectValue>
                              {(value: string) => PAPERMC_PROJECTS.find((p) => p.value === value)?.label ?? value}
                            </SelectValue>
                          </SelectTrigger>
                          <SelectContent>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t('settings.papermc.version')}</FormLabel>
                      <FormControl>
                        <Select
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t('settings.buildOptional')}</FormLabel>
                      <FormControl>
                        <Select
...
function JvmConfigSection({ serverId }: { serverId: string }) {
  ...
  const handleSave = useCallback(async () => {
    ...
    await updateServer.mutateAsync({
      ...
      jvm_flags: data.jvm_flags.trim(),
      java_port: data.java_port,
      auto_start: data.auto_start,
      auto_restart_on_crash: data.auto_restart_on_crash,
      java_path: data.java_mode === 'custom' ? data.custom_java_path || null : data.java_path || null,
    });
                        ...
                        >
                          <SelectTrigger>
                            <SelectValue>
                              {(value: string) =>
                                value === 'custom'
                                  ? t('settings.customJavaPath')
                                  : (javaVersions?.find((java) => java.path === value)?.name ?? value)
                              }
                            </SelectValue>
                          </SelectTrigger>
                          <SelectContent>
      ...
      <FeatureCard.Row className={'items-center py-3'}>
        ...
        <div className={'flex shrink-0 items-center gap-1.5'}>
          {activateConfirm ? (
            ...
            <>
              {canActive && (
                <Button onClick={() => setActivateConfirm(true)} variant={'secondary'} size={'xs'} icon={Check}>
                  {t('settings.activate')}
                </Button>
              )}
              {canDelete && (
                <Button onClick={() => setGateJarName(jar.name)} variant={'ghost-destructive'} size={'icon-sm'} icon={Trash2} />
              )}
            </>
          ) : null}
        </div>
