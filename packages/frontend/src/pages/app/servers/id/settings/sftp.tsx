...
function ConnectionInfoCell({ label, value }: { label: string; value: string }) {
  ...
  return (
    <Button
      ...
      onClick={handleCopy}
      className={
        'group flex h-auto flex-col items-start gap-1 rounded-none bg-white px-5 py-3.5 text-left font-normal transition-colors hover:bg-zinc-50 dark:bg-zinc-900 dark:hover:bg-zinc-800/80'
      }
    >
...
function AccountsSection({ serverId }: { serverId: string }) {
  ...
  const { data: accountsData, isLoading: accountsLoading } = useSftpAccounts(serverId);
  ...
  const { items: sortedAccounts, control: sortControl } = useSortableList<SftpAccountResponse>(filteredAccounts, {
    storageKey: 'sftp-accounts',
    fields: [
      { key: 'username', label: t('common.sort.byUsername'), accessor: (a) => a.username },
      { key: 'createdAt', label: t('common.sort.byCreatedAt'), accessor: (a) => a.createdAt, type: 'date' },
    ],
  });
    ...
    <>
      <FeatureCard>
        <FeatureCard.Header>
          ...
          <FeatureCard.Actions>
            {accounts.length > 0 && sortControl}
            {canCreate && (
              <Button onClick={() => setCreateGateOpen(true)} icon={Plus}>
    ...
    <>
      <FeatureCard.Row interactive className={'items-center py-3'}>
        ...
        <FeatureCard.RowControl>
          {canUpdate && <Button onClick={onEdit} variant={'ghost'} size={'icon'} icon={Pencil} />}
          {canDelete && (
            <Button onClick={() => setDeleteGateOpen(true)} variant={'ghost-destructive'} size={'icon'} icon={Trash2} />
          )}
        </FeatureCard.RowControl>
      </FeatureCard.Row>
      <PasswordGate
