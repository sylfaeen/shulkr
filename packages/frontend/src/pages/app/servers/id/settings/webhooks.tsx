...
function WebhooksContent({ serverId }: { serverId: string }) {
  ...
  const { items: sortedWebhooks, control: sortControl } = useSortableList(webhooks, {
    storageKey: 'webhooks',
    fields: [
      { key: 'name', label: t('common.sort.byName'), accessor: (w) => w.name },
      { key: 'createdAt', label: t('common.sort.byCreatedAt'), accessor: (w) => w.createdAt, type: 'date' },
    ],
  });
      ...
      <FeatureCard.Stack>
        <FeatureCard>
          <FeatureCard.Header>
            ...
            <FeatureCard.Actions>
              {webhooks && webhooks.length > 0 && sortControl}
              {canCreate && (
              ...
              sortedWebhooks.map((wh) => (
                <FeatureCard.Row key={wh.id} interactive onClick={() => canUpdate && setEditingWebhookId(wh.id)}>
                  ...
                  <div className={'flex shrink-0 items-center gap-2'} onClick={(e) => e.stopPropagation()}>
                    ...
                    />
                    {canDelete && (
                      <Button variant={'ghost-destructive'} size={'icon'} onClick={() => setGateWebhookId(wh.id)} icon={Trash2} />
                    )}
                  </div>
                </FeatureCard.Row>
              ))
