              ...
              webhooks.map((wh) => (
                <FeatureCard.Row key={wh.id} interactive onClick={() => canUpdate && setEditingWebhookId(wh.id)}>
                  ...
                  <div className={'flex shrink-0 items-center gap-2'} onClick={(e) => e.stopPropagation()}>
                    ...
                    {canUpdate && (
                      <Button size={'icon-sm'} variant={'ghost'} onClick={() => setEditingWebhookId(wh.id)} icon={Pencil} iconClass={'size-3.5'} />
                    )}
                    {canTest && (
                      <Button
                        size={'icon-sm'}
                    ...
                    <Button size={'icon-sm'} variant={'ghost'} onClick={() => setDeliveriesFor(wh.id)} icon={Clock} iconClass={'size-3.5'} />
                    {canDelete && (
                      <Button variant={'ghost-destructive'} size={'icon-sm'} onClick={() => setGateWebhookId(wh.id)} icon={Trash2} />
                    )}
                  </div>
                </FeatureCard.Row>
              ))
