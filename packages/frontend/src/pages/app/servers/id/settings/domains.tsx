...
function HttpDomainListSection({ serverId }: { serverId: string }) {
  ...
  const { items: sortedHttpDomains, control: sortControl } = useSortableList(httpDomains, {
    storageKey: 'http-domains',
    fields: [{ key: 'domain', label: t('common.sort.byDomain'), accessor: (d) => d.domain }],
  });
    ...
    <>
      <FeatureCard>
        <FeatureCard.Header>
          ...
          <FeatureCard.Actions>
            {httpDomains.length > 0 && sortControl}
            {hasSsl && canSsl && (
              <Button
