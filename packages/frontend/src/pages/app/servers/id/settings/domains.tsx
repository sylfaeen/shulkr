...
function PlayerDomainSection({ serverId }: { serverId: string }) {
  ...
  const filteredConnectionDomains = useMemo(() => {
    const q = searchQuery.trim().toLowerCase();
    if (!q) return connectionDomains;

    return connectionDomains.filter((d) => d.domain.toLowerCase().includes(q));
  }, [connectionDomains, searchQuery]);
      ...
      <FeatureCard.Body>
        {connectionDomains.length === 0 ? (
          ...
          <>
            ...
            {filteredConnectionDomains.map((domain) => (
        ...
        <div className={'flex items-center justify-between px-3 py-1.5'}>
          ...
          <TooltipProvider delay={200}>
            <span className={'font-jetbrains text-sm font-medium'}>
              <Tooltip>
                <TooltipTrigger render={<span className={'cursor-help text-zinc-800 dark:text-zinc-200'}>_minecraft.</span>} />
...
function HttpDomainListSection({ serverId }: { serverId: string }) {
  ...
  const filteredHttpDomains = useMemo(() => {
    const q = searchQuery.trim().toLowerCase();
    if (!q) return httpDomains;

    return httpDomains.filter((d) => d.domain.toLowerCase().includes(q));
  }, [httpDomains, searchQuery]);
      ...
      <FeatureCard>
        ...
        <FeatureCard.Body>
          {httpDomains.length === 0 ? (
            ...
            <>
              <FeatureCard.Search value={searchQuery} onChange={setSearchQuery} />
              {filteredHttpDomains.map((domain) => (
            ...
            {!domain.sslEnabled && canSsl && (
              <Tooltip>
                <TooltipTrigger
                  render={
                    <Button
                      onClick={() => onEnableSsl.mutateAsync(domain.id)}
                      variant={'success'}
                      size={'xs'}
                      icon={Lock}
                      iconClass={'size-3.5'}
