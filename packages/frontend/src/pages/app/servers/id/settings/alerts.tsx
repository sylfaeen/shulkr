              ...
              rules.map((rule) => (
                <FeatureCard.Row key={rule.id} interactive onClick={() => canUpdate && setEditRule(rule as AlertRuleProps)}>
                  ...
                  <div className={'flex shrink-0 items-center gap-2'} onClick={(e) => e.stopPropagation()}>
                    ...
                    {canUpdate && (
                      <Button size={'icon-sm'} variant={'ghost'} onClick={() => setEditRule(rule as AlertRuleProps)} icon={Pencil} iconClass={'size-3.5'} />
                    )}
                    {canDelete && (
                      <Button variant={'ghost-destructive'} size={'icon-sm'} onClick={() => setGateAlertId(rule.id)} icon={Trash2} />
                    )}
                  </div>
                </FeatureCard.Row>
              ))
