...
function AlertsContent({ serverId }: { serverId: string }) {
  ...
  const { items: sortedRules, control: sortControl } = useSortableList(rules, {
    storageKey: 'alerts',
    fields: [
      { key: 'name', label: t('common.sort.byName'), accessor: (r) => r.name },
      { key: 'createdAt', label: t('common.sort.byCreatedAt'), accessor: (r) => r.createdAt, type: 'date' },
    ],
  });
      ...
      <FeatureCard.Stack>
        <FeatureCard>
          <FeatureCard.Header>
            ...
            <FeatureCard.Actions>
              {rules && rules.length > 0 && sortControl}
              {canCreate && (
              ...
              sortedRules.map((rule) => (
                <FeatureCard.Row key={rule.id} interactive onClick={() => canUpdate && setEditRule(rule as AlertRuleProps)}>
                  ...
                  <div className={'flex shrink-0 items-center gap-2'} onClick={(e) => e.stopPropagation()}>
                    ...
                    {canDelete && (
                      <Button variant={'ghost-destructive'} size={'icon'} onClick={() => setGateAlertId(rule.id)} icon={Trash2} />
                    )}
                  </div>
                </FeatureCard.Row>
              ))
