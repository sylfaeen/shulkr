...
function GameStatsSection({
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      // Fonts are bundled as same-origin @fontsource assets, so the CSSOM is readable and html-to-image can collect and base64-inline every @font-face.
      ...
      addToast({ type: 'success', title: t('playerProfile.stats.exportSuccess') });
    } catch {
      addToast({ type: 'error', title: t('playerProfile.stats.exportError') });
    } finally {
  ...
  return (
    <div className={'shrink-0'}>
      <Button
        ...
        className={
          'flex h-auto items-center gap-1.5 p-0 text-sm font-normal text-zinc-500 transition-colors hover:bg-transparent hover:text-zinc-700 dark:text-zinc-400 dark:hover:bg-transparent dark:hover:text-zinc-300'
        }
        icon={History}
        iconClass={'size-3.5'}
      >
            ...
            <>
              ...
              {hasMore && (
                <Button
                  ...
                  className={
                    'h-auto w-full rounded-none border-t border-black/4 px-4 py-2 text-xs font-normal text-zinc-500 transition-colors hover:bg-zinc-100 hover:text-zinc-700 dark:border-white/4 dark:hover:bg-zinc-800 dark:hover:text-zinc-300'
                  }
                >
                  {t('players.loadMore')}
                </Button>
...
function ItemsTable({ rows }: { rows: Array<ItemRow> }) {
  ...
  return (
    <div className={'max-h-140 overflow-auto rounded-lg border border-black/6 bg-white dark:border-white/6 dark:bg-zinc-900/40'}>
  ...
  return (
    <th className={cn('px-3 py-2 font-medium', align === 'right' && 'text-right')}>
      <Button
        ...
        onClick={onClick}
        className={cn(
          'inline-flex h-auto items-center gap-1 p-0 font-medium tracking-wider uppercase transition-colors hover:bg-transparent dark:hover:bg-transparent',
