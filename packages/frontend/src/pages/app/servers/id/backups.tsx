...
import { Archive, CloudUpload, Download, HardDrive, Loader2, Pencil, Plus, Settings, Trash2, Check, X } from 'lucide-react';
...
function BackupsSection({ serverId, onOpenSettings }: { serverId: string; onOpenSettings: () => void }) {
  ...
  const handleUploadToCloud = async (cloudDestinationId: string) => {
    if (!uploadDialogFilename) return;
    try {
      await uploadBackup.mutateAsync({ filename: uploadDialogFilename, cloudDestinationId });
  ...
  return (
    <>
            ...
            <>
              ...
              {backups.map((backup, index) => (
                <BackupRow
                  ...
                  uploading={uploadBackup.isPending && uploadDialogFilename === backup.filename}
                  {...{ backup, index }}
                />
              ))}
              <PasswordGate
      ...
      <UploadBackupToCloudDialog
        ...
        isPending={uploadBackup.isPending}
        onClose={() => {
          if (!uploadBackup.isPending) setUploadDialogFilename(null);
        }}
      ...
      <div className={'flex shrink-0 items-center gap-1.5'}>
        {isCreating ? null : (
          <TooltipProvider delay={300}>
            ...
            {canUploadToCloud && backup.location === 'local' && (
              <Tooltip>
                <TooltipTrigger
                  render={
                    <Button
                ...
                <TooltipContent className={'rounded-lg px-2.5 py-1.5 text-sm'}>
                  {t('backups.tooltipUploadToCloud')}
                </TooltipContent>
              </Tooltip>
            )}
            {canRename && (
