...
function BackupRow({
  ...
  const startRename = () => {
    setEditValue(backup.filename.replace(/\.zip$/, ''));
    setEditing(true);
  };

  const confirmRename = () => {
    const newFilename = editValue.trim() + '.zip';
    if (newFilename && newFilename !== backup.filename) {
  ...
  return (
    <FeatureCard.Row interactive className={cn('min-h-18.25 items-center py-3', isCreating && 'opacity-70')}>
          ...
          {editing ? (
            <div className={'flex items-center gap-1.5'}>
              <Input
                ...
                className={'font-jetbrains h-7 w-auto min-w-16 text-sm'}
              />
              <span className={'text-sm text-zinc-400'}>.zip</span>
              <Button onClick={confirmRename} variant={'ghost'} size={'icon-xs'} disabled={renamePending}>
      ...
      <div className={'flex shrink-0 items-center gap-1.5'}>
        {isCreating ? null : deleteConfirm === backup.filename ? (
          <div className={'flex items-center gap-1.5'}>
            <span className={'text-sm text-zinc-600 dark:text-zinc-400'}>{t('common.confirm')}?</span>
            <Button
