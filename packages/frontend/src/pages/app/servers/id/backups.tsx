...
import { Archive, Download, HardDrive, Loader2, LoaderCircle, Plus, Settings, Trash2 } from 'lucide-react';
...
function BackupRow({
  ...
  backup: { filename: string; size: number; created: string; status?: 'creating' | 'ready' };
  index: number;
  selected: boolean;
  ...
  return (
    <FeatureCard.Row interactive className={cn('min-h-18.25 items-center gap-4 py-3', isCreating && 'opacity-70')}>
      <Checkbox checked={selected} onCheckedChange={onToggleSelect} onClick={(e) => e.stopPropagation()} disabled={isCreating} />
      <div className={'flex min-w-0 flex-1 items-center gap-3'}>
        <div
          className={cn(
            'flex size-8 shrink-0 items-center justify-center rounded-lg text-white transition-opacity',
            isCreating ? 'bg-amber-500' : 'bg-blue-600'
          )}
        >
          {isCreating ? (
        ...
        <div className={'min-w-0'}>
          ...
          <div className={'mt-0.5 flex items-center gap-2'}>
            {isCreating && (
              <Badge variant={'warning'} className={'animate-pulse font-medium'}>
                {t('backups.creating')}
              </Badge>
            )}
      ...
      <div className={'flex shrink-0 items-center gap-1.5'}>
        {isCreating ? (
          <span className={'text-xs text-amber-500'}>{t('backups.inProgress')}</span>
