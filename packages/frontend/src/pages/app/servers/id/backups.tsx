import { useMemo, useState } from 'react';
import { useParams } from '@tanstack/react-router';
import { useTranslation } from 'react-i18next';
import {
  AlertCircle,
  Archive,
  CloudUpload,
  Download,
  HardDrive,
  Loader2,
  Pencil,
  Plus,
  Settings,
  Share2,
  Trash2,
  Check,
  X,
} from 'lucide-react';
import { ServerPageSkeleton } from '@shulkr/frontend/pages/app/servers/features/server_page_skeleton';
import { SkeletonList } from '@shulkr/frontend/features/ui/skeleton_presets';
import { BackupStrategySection } from '@shulkr/frontend/pages/app/servers/features/backup_strategy_section';
import { PageError } from '@shulkr/frontend/features/ui/page_error';
import { useServer, useBackupServer, useUpdateServer } from '@shulkr/frontend/hooks/use_servers';
import { useBackups, useRenameBackup, useDeleteBackup, useUploadBackupToCloud } from '@shulkr/frontend/hooks/use_backups';
import { useCloudDestinations } from '@shulkr/frontend/hooks/use_cloud_destinations';
import { PasswordGate } from '@shulkr/frontend/features/password_gate';
import { Input } from '@shulkr/frontend/features/ui/base/input';
import { useQueryClient } from '@tanstack/react-query';
import { useAuthStore } from '@shulkr/frontend/stores/auth_store';
import { cn } from '@shulkr/frontend/lib/cn';
import { Button } from '@shulkr/frontend/features/ui/base/button';
import { Checkbox } from '@shulkr/frontend/features/ui/base/checkbox';
import { CreateBackupDialog } from '@shulkr/frontend/pages/app/servers/dialogs/create_backup_dialog';
import { BackupSettingsDialog } from '@shulkr/frontend/pages/app/servers/dialogs/backup_settings_dialog';
import { UploadBackupToCloudDialog } from '@shulkr/frontend/pages/app/servers/dialogs/upload_backup_to_cloud_dialog';
import { ShareBackupDialog } from '@shulkr/frontend/pages/app/servers/dialogs/share_backup_dialog';
import { ServerPageHeader } from '@shulkr/frontend/pages/app/servers/features/server_page_header';
import { Badge } from '@shulkr/frontend/features/ui/base/badge';
import { FeatureCard } from '@shulkr/frontend/pages/app/features/card';
import { PageContent } from '@shulkr/frontend/pages/app/features/page_content';
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '@shulkr/frontend/features/ui/base/tooltip';
import { Label } from '@shulkr/frontend/features/ui/base/label';
import { usePageTitle } from '@shulkr/frontend/hooks/use_page_title';
import { useHasPermission } from '@shulkr/frontend/hooks/use_permissions';
import { formatRelativeDate } from '@shulkr/frontend/lib/date';
import { formatFileSize } from '@shulkr/frontend/hooks/use_files';
import { useSortableList } from '@shulkr/frontend/pages/app/features/sort/use_sortable_list';

export function ServerBackupsPage() {
  const { t } = useTranslation();
  const { id } = useParams({ strict: false });
  const { data: server, isLoading: serverLoading } = useServer(id || '');
  const updateServer = useUpdateServer();
  const queryClient = useQueryClient();
  const [settingsOpen, setSettingsOpen] = useState(false);

  usePageTitle(server?.name ? `${server.name} • ${t('nav.backups')}` : t('nav.backups'));

  if (serverLoading) return <ServerPageSkeleton />;
  if (!server) return <PageError message={t('errors.generic')} />;

  const openSettings = () => setSettingsOpen(true);

  return (
    <>
      <ServerPageHeader>
        <ServerPageHeader.Left>
          <ServerPageHeader.Icon icon={Archive} />
          <ServerPageHeader.Info>
            <ServerPageHeader.Heading>
              <ServerPageHeader.ServerName />
              <ServerPageHeader.PageName>{t('backups.title')}</ServerPageHeader.PageName>
              <ServerPageHeader.Docs page={'Tasks'} />
            </ServerPageHeader.Heading>
            <ServerPageHeader.Description>{t('backups.subtitle')}</ServerPageHeader.Description>
          </ServerPageHeader.Info>
        </ServerPageHeader.Left>
      </ServerPageHeader>
      <PageContent>
        <FeatureCard.Stack>
          <BackupStrategySection serverId={server.id} onOpenSettings={openSettings} />
          <BackupsSection serverId={server.id} onOpenSettings={openSettings} />
        </FeatureCard.Stack>
      </PageContent>
      <BackupSettingsDialog
        open={settingsOpen}
        onOpenChange={setSettingsOpen}
        currentMaxBackups={server.max_backups ?? 0}
        isPending={updateServer.isPending}
        onClose={() => setSettingsOpen(false)}
        onSave={async (maxBackups) => {
          await updateServer.mutateAsync({ id: server.id, max_backups: maxBackups });
          queryClient.invalidateQueries({ queryKey: ['servers', 'byId', server.id] }).then();
          setSettingsOpen(false);
        }}
      />
    </>
  );
}

function BackupsSection({ serverId, onOpenSettings }: { serverId: string; onOpenSettings: () => void }) {
  const { t } = useTranslation();
  const can = useHasPermission();
  const canList = can('server:backups:list');
  const canCreate = can('server:backups:create');
  const canDownload = can('server:backups:download');
  const canDelete = can('server:backups:delete');
  const canShare = can('server:backups:share');
  const [deleteConfirm, setDeleteConfirm] = useState<string | null>(null);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [selected, setSelected] = useState<Set<string>>(() => new Set());
  const [bulkDeleteConfirm, setBulkDeleteConfirm] = useState(false);
  const [uploadDialogFilename, setUploadDialogFilename] = useState<string | null>(null);
  const [shareDialogFilename, setShareDialogFilename] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState('');
  const { data: backups, isLoading: backupsLoading } = useBackups(serverId);
  const { data: cloudDestinations } = useCloudDestinations();
  const backupServer = useBackupServer();
  const renameBackup = useRenameBackup(serverId);
  const deleteBackup = useDeleteBackup(serverId);
  const uploadBackup = useUploadBackupToCloud(serverId);
  const queryClient = useQueryClient();
  const enabledCloudStorages = cloudDestinations?.filter((d) => d.enabled) ?? [];
  const totalSize = backups?.reduce((acc, b) => acc + b.size, 0) ?? 0;
  const hasSelection = selected.size > 0;
  const allSelected = backups !== undefined && backups.length > 0 && selected.size === backups.length;

  const filteredBackups = useMemo(() => {
    const list = backups ?? [];
    const q = searchQuery.trim().toLowerCase();
    if (!q) return list;

    return list.filter((b) => b.filename.toLowerCase().includes(q));
  }, [backups, searchQuery]);

  const { items: sortedBackups, control: sortControl } = useSortableList(filteredBackups, {
    storageKey: 'backups',
    defaultKey: 'created',
    defaultDirection: 'desc',
    fields: [
      { key: 'created', label: t('common.sort.byCreatedAt'), accessor: (b) => b.created, type: 'date' },
      { key: 'filename', label: t('common.sort.byFilename'), accessor: (b) => b.filename },
    ],
  });

  const latestFilename =
    backups && backups.length > 0
      ? backups.reduce((latest, b) => (new Date(b.created).getTime() > new Date(latest.created).getTime() ? b : latest)).filename
      : null;

  const toggleSelect = (filename: string) => {
    setSelected((prev) => {
      const next = new Set(prev);

      if (next.has(filename)) {
        next.delete(filename);
      } else {
        next.add(filename);
      }

      return next;
    });
  };

  const toggleSelectAll = () => {
    if (!backups) return;

    if (allSelected) {
      setSelected(new Set());
    } else {
      setSelected(new Set(backups.map((b) => b.filename)));
    }
  };

  const isLatest = (filename: string) => filename === latestFilename;

  const handleBackupConfirm = async (paths: Array<string>) => {
    setDialogOpen(false);

    try {
      await backupServer.mutateAsync(serverId, paths);
      queryClient.invalidateQueries({ queryKey: ['servers', 'listBackups', serverId] }).then();
    } catch {}
  };

  const handleDownload = (filename: string) => {
    if (!canDownload) return;
    const token = useAuthStore.getState().accessToken;
    const url = `/api/servers/backups/${encodeURIComponent(filename)}?token=${encodeURIComponent(token ?? '')}`;
    const a = document.createElement('a');
    a.href = url;
    a.download = filename;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
  };

  const handleDelete = async (filename: string) => {
    if (!canDelete) return;

    try {
      await deleteBackup.mutateAsync({ filename });
      setDeleteConfirm(null);

      setSelected((prev) => {
        const next = new Set(prev);
        next.delete(filename);

        return next;
      });
    } catch {}
  };

  const handleUploadToCloud = (cloudDestinationId: string) => {
    if (!uploadDialogFilename) return;
    const filename = uploadDialogFilename;
    setUploadDialogFilename(null);
    uploadBackup.mutate({ filename, cloudDestinationId });
  };

  const handleBulkDelete = async () => {
    if (!canDelete) return;

    for (const filename of selected) {
      try {
        await deleteBackup.mutateAsync({ filename });
      } catch {}
    }

    setSelected(new Set());
    setBulkDeleteConfirm(false);
  };

  return (
    <>
      <FeatureCard>
        <FeatureCard.Header>
          <FeatureCard.Content>
            <FeatureCard.Title count={backups && backups.length > 0 && backups.length}>{t('backups.title')}</FeatureCard.Title>
            <FeatureCard.Description>{t('backups.subtitle')}</FeatureCard.Description>
          </FeatureCard.Content>
          <FeatureCard.Actions className={'gap-2 sm:gap-6'}>
            {backups && backups.length > 0 && (
              <div className={'flex items-center gap-1.5 text-sm text-zinc-600 dark:text-zinc-400'}>
                <HardDrive className={'size-3'} strokeWidth={2} />
                <span>{formatFileSize(totalSize)}</span>
              </div>
            )}
            <div className={'flex items-center justify-end gap-2'}>
              {canList && (
                <Button onClick={onOpenSettings} variant={'secondary'} icon={Settings}>
                  {t('backups.settings.button')}
                </Button>
              )}
              {canCreate && (
                <Button onClick={() => setDialogOpen(true)} icon={Plus}>
                  {t('backups.backupNow')}
                </Button>
              )}
            </div>
          </FeatureCard.Actions>
        </FeatureCard.Header>
        <FeatureCard.Body>
          {backupsLoading ? (
            <FeatureCard.Row className={'py-2'}>
              <SkeletonList rows={5} className={'w-full [&>*]:min-h-18.25'} />
            </FeatureCard.Row>
          ) : !backups || backups.length === 0 ? (
            <FeatureCard.Empty icon={Archive} title={t('backups.noBackups')} description={t('backups.createFirst')} />
          ) : (
            <>
              {backups.length > 0 && (
                <>
                  <FeatureCard.Search value={searchQuery} onChange={setSearchQuery} />
                  <FeatureCard.Row className={'min-h-10.5 bg-white py-2 dark:bg-zinc-800/50'}>
                    <Label className={'flex items-center gap-3'}>
                      <Checkbox checked={allSelected} onCheckedChange={toggleSelectAll} />
                      <span className={'text-sm font-medium text-zinc-500 dark:text-zinc-400'}>{t('backups.selectAll')}</span>
                    </Label>
                    <div className={'flex flex-wrap items-center justify-end gap-2 sm:gap-4'}>
                      {hasSelection && (
                        <>
                          <span className={'text-sm text-zinc-600 dark:text-zinc-400'}>
                            {t('backups.selected', { count: selected.size })}
                          </span>
                          {canDelete && (
                            <div className={'flex items-center gap-2'}>
                              {bulkDeleteConfirm ? (
                                <div className={'flex items-center gap-1.5'}>
                                  <span className={'text-sm text-zinc-600 dark:text-zinc-400'}>{t('common.confirm')}?</span>
                                  <Button
                                    onClick={handleBulkDelete}
                                    variant={'destructive'}
                                    size={'xs'}
                                    disabled={deleteBackup.isPending}
                                    loading={deleteBackup.isPending}
                                  >
                                    {t('common.yes')}
                                  </Button>
                                  <Button onClick={() => setBulkDeleteConfirm(false)} variant={'ghost'} size={'xs'}>
                                    {t('common.no')}
                                  </Button>
                                </div>
                              ) : (
                                <Button
                                  onClick={() => setBulkDeleteConfirm(true)}
                                  variant={'ghost-destructive'}
                                  size={'xs'}
                                  icon={Trash2}
                                  iconClass={'size-3.5'}
                                >
                                  {t('common.delete')}
                                </Button>
                              )}
                            </div>
                          )}
                        </>
                      )}
                      {sortControl}
                    </div>
                  </FeatureCard.Row>
                </>
              )}
              {sortedBackups.map((backup) => (
                <BackupRow
                  key={backup.filename}
                  selected={selected.has(backup.filename)}
                  onToggleSelect={() => toggleSelect(backup.filename)}
                  renamePending={renameBackup.isPending}
                  onDownload={handleDownload}
                  onRename={(filename, newFilename) => renameBackup.mutateAsync({ filename, newFilename })}
                  onRequestDelete={setDeleteConfirm}
                  onRequestUploadToCloud={setUploadDialogFilename}
                  onRequestShare={setShareDialogFilename}
                  canShare={canShare}
                  canUploadToCloud={canCreate && enabledCloudStorages.length > 0}
                  isNewest={isLatest(backup.filename)}
                  {...{ backup }}
                />
              ))}
              <PasswordGate
                open={deleteConfirm !== null}
                onOpenChange={(open) => !open && setDeleteConfirm(null)}
                title={t('common.delete')}
                description={deleteConfirm ?? ''}
                confirmLabel={t('common.delete')}
                destructive
                onConfirm={async () => {
                  if (deleteConfirm) await handleDelete(deleteConfirm);
                }}
              />
            </>
          )}
        </FeatureCard.Body>
      </FeatureCard>
      <CreateBackupDialog
        open={dialogOpen}
        isPending={backupServer.isPending}
        onClose={() => setDialogOpen(false)}
        onConfirm={handleBackupConfirm}
        {...{ serverId }}
      />
      <UploadBackupToCloudDialog
        open={uploadDialogFilename !== null}
        filename={uploadDialogFilename}
        destinations={enabledCloudStorages}
        isPending={false}
        onClose={() => setUploadDialogFilename(null)}
        onConfirm={handleUploadToCloud}
      />
      <ShareBackupDialog
        open={shareDialogFilename !== null}
        filename={shareDialogFilename}
        onClose={() => setShareDialogFilename(null)}
      />
    </>
  );
}

function BackupRow({
  backup,
  isNewest,
  selected,
  renamePending,
  canUploadToCloud,
  canShare,
  onToggleSelect,
  onDownload,
  onRename,
  onRequestDelete,
  onRequestUploadToCloud,
  onRequestShare,
}: {
  backup: {
    filename: string;
    size: number;
    created: string;
    status?: 'creating' | 'ready';
    progress?: number;
    location?: 'local' | 'cloud' | 'hybrid';
    cloudUploadStatus?: 'uploading' | 'failed' | null;
    cloudUploadError?: string | null;
  };
  isNewest: boolean;
  selected: boolean;
  renamePending: boolean;
  canUploadToCloud: boolean;
  canShare: boolean;
  onToggleSelect: () => void;
  onDownload: (filename: string) => void;
  onRename: (filename: string, newFilename: string) => void;
  onRequestDelete: (filename: string) => void;
  onRequestUploadToCloud: (filename: string) => void;
  onRequestShare: (filename: string) => void;
}) {
  const { t } = useTranslation();
  const can = useHasPermission();
  const canDownload = can('server:backups:download');
  const canRename = can('server:backups:rename');
  const canDelete = can('server:backups:delete');
  const [editing, setEditing] = useState(false);
  const [editValue, setEditValue] = useState('');
  const source = getBackupSource(backup.filename);
  const backupType = getBackupType(backup.filename);
  const isCreating = backup.status === 'creating';
  const isUploading = backup.cloudUploadStatus === 'uploading';
  const uploadFailed = backup.cloudUploadStatus === 'failed';
  const showNewestBadge = isNewest && !isCreating;
  // Extract server slug prefix (e.g. "my-server-") so user can only edit the rest
  const slugMatch = backup.filename.match(/^([a-z0-9-]+?-(?:manual|auto)(?:-incremental)?-)/);
  const slugPrefix = slugMatch ? slugMatch[1] : '';
  const editablePart = backup.filename.slice(slugPrefix.length).replace(/\.zip$/, '');

  const startRename = () => {
    if (!canRename) return;
    setEditValue(editablePart);
    setEditing(true);
  };

  const confirmRename = () => {
    if (!canRename) return;
    const newFilename = slugPrefix + editValue.trim() + '.zip';

    if (newFilename !== backup.filename && editValue.trim()) {
      onRename(backup.filename, newFilename);
    }

    setEditing(false);
  };

  const cancelRename = () => setEditing(false);

  return (
    <FeatureCard.Row interactive className={cn('min-h-18.25 items-center py-3', isCreating && 'opacity-70')}>
      <div className={'flex min-w-0 flex-1 items-center gap-3'}>
        <Checkbox
          checked={selected}
          onCheckedChange={onToggleSelect}
          onClick={(e) => e.stopPropagation()}
          disabled={isCreating}
        />
        <div
          className={cn(
            'flex size-8 shrink-0 items-center justify-center rounded-lg text-white transition-opacity',
            isCreating ? 'bg-orange-500' : 'bg-blue-600'
          )}
        >
          {isCreating ? (
            <Loader2 className={'size-4 animate-spin'} strokeWidth={2} />
          ) : (
            <Archive className={'size-4'} strokeWidth={2} />
          )}
        </div>
        <div className={'min-w-0'}>
          {editing ? (
            <div className={'flex items-center gap-0'}>
              {slugPrefix && <span className={'font-jetbrains text-sm text-zinc-400 dark:text-zinc-500'}>{slugPrefix}</span>}
              <Input
                autoFocus
                value={editValue}
                onChange={(e) => setEditValue(e.target.value)}
                onKeyDown={(e) => {
                  if (e.key === 'Enter') confirmRename();
                  if (e.key === 'Escape') cancelRename();
                }}
                style={{ width: `${Math.max(editValue.length, 1)}ch` }}
                className={
                  'font-jetbrains h-7 w-auto min-w-16 rounded-none border-x-0 border-t-0 border-b px-0 text-sm shadow-none focus-visible:ring-0'
                }
              />
              <span className={'font-jetbrains text-sm text-zinc-400 dark:text-zinc-500'}>.zip</span>
              <Button
                onClick={confirmRename}
                variant={'ghost'}
                size={'icon-xs'}
                className={'ml-1.5'}
                disabled={renamePending}
                icon={Check}
                iconClass={'size-3.5'}
              />
              <Button onClick={cancelRename} variant={'ghost'} size={'icon-xs'} icon={X} iconClass={'size-3.5'} />
            </div>
          ) : (
            <span className={'font-jetbrains truncate text-sm font-medium text-zinc-800 dark:text-zinc-200'}>
              {backup.filename}
            </span>
          )}
          <div className={'mt-0.5 flex w-full items-center gap-2'}>
            {isCreating && (
              <div className={'flex w-full items-center gap-2'}>
                <div className={'h-1.5 w-full overflow-hidden rounded-full bg-orange-200 dark:bg-orange-900/30'}>
                  <div
                    className={'h-full rounded-full bg-orange-500 transition-all duration-700 ease-out'}
                    style={{ width: `${backup.progress ?? 0}%` }}
                  />
                </div>
                <span className={'font-jetbrains text-xs text-orange-600 tabular-nums dark:text-orange-400'}>
                  {backup.progress ?? 0}%
                </span>
              </div>
            )}
            {showNewestBadge && <Badge className={'font-medium'}>{t('backups.latest')}</Badge>}
            {!isCreating && isUploading && (
              <Badge variant={'secondary'} className={'gap-1 font-medium text-blue-700 dark:text-blue-300'}>
                <Loader2 className={'size-3 animate-spin'} strokeWidth={2.5} />
                {t('backups.uploadingToCloud')}
              </Badge>
            )}
            {!isCreating && !isUploading && uploadFailed && (
              <TooltipProvider delay={300}>
                <Tooltip>
                  <TooltipTrigger
                    render={
                      <Badge
                        variant={'outline'}
                        className={'gap-1 border-red-300 font-medium text-red-700 dark:border-red-900 dark:text-red-400'}
                      >
                        <AlertCircle className={'size-3'} strokeWidth={2.5} />
                        {t('backups.uploadFailed')}
                      </Badge>
                    }
                  />
                  <TooltipContent className={'max-w-xs rounded-lg px-2.5 py-1.5 text-sm'}>
                    {backup.cloudUploadError ?? t('backups.uploadFailedTooltip')}
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            )}
            {!isCreating && source && (
              <Badge variant={'secondary'} className={'font-medium'}>
                {t(`backups.source${source === 'auto' ? 'Auto' : 'Manual'}`)}
              </Badge>
            )}
            {!isCreating && (
              <Badge variant={backupType === 'incremental' ? 'outline' : 'secondary'} className={'font-medium'}>
                {t(`backups.type${backupType === 'incremental' ? 'Incremental' : 'Full'}`)}
              </Badge>
            )}
            {!isCreating && (
              <>
                {(showNewestBadge || source) && <span className={'text-sm text-zinc-300 dark:text-zinc-700'}>·</span>}
                <span className={'font-jetbrains text-sm text-zinc-600 tabular-nums dark:text-zinc-400'}>
                  {formatFileSize(backup.size)}
                </span>
                <span className={'text-sm text-zinc-300 dark:text-zinc-700'}>·</span>
                <span className={'text-sm text-zinc-600 dark:text-zinc-400'}>{formatRelativeDate(backup.created)}</span>
              </>
            )}
          </div>
        </div>
      </div>
      <div className={'flex shrink-0 items-center gap-1.5'}>
        {isCreating ? null : (
          <TooltipProvider delay={300}>
            {canDownload && (
              <Tooltip>
                <TooltipTrigger
                  render={
                    <Button onClick={() => onDownload(backup.filename)} variant={'ghost'} size={'icon-sm'} icon={Download} />
                  }
                />
                <TooltipContent className={'rounded-lg px-2.5 py-1.5 text-sm'}>{t('backups.tooltipDownload')}</TooltipContent>
              </Tooltip>
            )}
            {canShare && backup.location !== 'cloud' && (
              <Tooltip>
                <TooltipTrigger
                  render={
                    <Button onClick={() => onRequestShare(backup.filename)} variant={'ghost'} size={'icon-sm'} icon={Share2} />
                  }
                />
                <TooltipContent className={'rounded-lg px-2.5 py-1.5 text-sm'}>{t('backups.tooltipShare')}</TooltipContent>
              </Tooltip>
            )}
            {canUploadToCloud && backup.location === 'local' && !isUploading && (
              <Tooltip>
                <TooltipTrigger
                  render={
                    <Button
                      onClick={() => onRequestUploadToCloud(backup.filename)}
                      variant={'ghost'}
                      size={'icon-sm'}
                      icon={CloudUpload}
                    />
                  }
                />
                <TooltipContent className={'rounded-lg px-2.5 py-1.5 text-sm'}>
                  {uploadFailed ? t('backups.tooltipUploadRetry') : t('backups.tooltipUploadToCloud')}
                </TooltipContent>
              </Tooltip>
            )}
            {canRename && (
              <Tooltip>
                <TooltipTrigger render={<Button onClick={startRename} variant={'ghost'} size={'icon-sm'} icon={Pencil} />} />
                <TooltipContent className={'rounded-lg px-2.5 py-1.5 text-sm'}>{t('backups.tooltipRename')}</TooltipContent>
              </Tooltip>
            )}
            {canDelete && (
              <Tooltip>
                <TooltipTrigger
                  render={
                    <Button
                      onClick={() => onRequestDelete(backup.filename)}
                      variant={'ghost-destructive'}
                      size={'icon-sm'}
                      icon={Trash2}
                    />
                  }
                />
                <TooltipContent className={'rounded-lg px-2.5 py-1.5 text-sm'}>{t('backups.tooltipDelete')}</TooltipContent>
              </Tooltip>
            )}
          </TooltipProvider>
        )}
      </div>
    </FeatureCard.Row>
  );
}

function getBackupSource(filename: string): 'manual' | 'auto' | null {
  if (filename.includes('-manual-')) return 'manual';
  if (filename.includes('-auto-')) return 'auto';

  return null;
}

function getBackupType(filename: string): 'full' | 'incremental' {
  return filename.includes('-incremental-') ? 'incremental' : 'full';
}
