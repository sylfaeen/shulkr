...
import { AlertTriangle, HardDrive, Loader2, LoaderCircle, Package, Power, PowerOff, Puzzle, Trash2, Upload } from 'lucide-react';
  ...
  return (
    <>
      ...
      <PageContent>
        ...
        <FeatureCard.Stack>
          <PluginsSection serverId={server.id} />
        </FeatureCard.Stack>
      </PageContent>
    </>
  );
...
function PluginsSection({ serverId }: { serverId: string }) {
  const { t } = useTranslation();
