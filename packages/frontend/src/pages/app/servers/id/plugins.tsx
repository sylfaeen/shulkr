...
import { AlertTriangle, HardDrive, Loader2, Package, Power, PowerOff, Puzzle, Store, Trash2, Upload } from 'lucide-react';
...
function PluginsSection({ serverId, onOpenMarketplace }: { serverId: string; onOpenMarketplace: (slug?: string) => void }) {
  ...
  const canMarketplace = can('server:plugins:marketplace');
  const canUpload = can('server:plugins:upload');
  const canToggle = can('server:plugins:toggle');
  const canDeletePlugin = can('server:plugins:delete');
  ...
  const filteredPlugins = useMemo(() => {
    ...
    return plugins.filter((p) => p.name.toLowerCase().includes(q) || p.filename.toLowerCase().includes(q));
  }, [plugins, searchQuery]);

  const { items: sortedPlugins, control: sortControl } = useSortableList<PluginInfo>(filteredPlugins, {
    storageKey: 'plugins',
    fields: [
      { key: 'name', label: t('common.sort.byName'), accessor: (p) => p.name },
      { key: 'modified', label: t('common.sort.byModified'), accessor: (p) => p.modified, type: 'date' },
    ],
  });
        ...
        <FeatureCard.Header>
          ...
          <FeatureCard.Actions className={'gap-2 sm:gap-6'}>
            ...
            <div className={'flex items-center justify-end gap-2'}>
              ...
              {canMarketplace && (
                <Button variant={'secondary'} onClick={() => onOpenMarketplace()} icon={Store}>
                  {t('plugins.marketplace')}
                </Button>
              )}
              {canUpload && (
                <Button onClick={() => fileInputRef.current?.click()} loading={uploadPlugin.isPending} icon={Upload}>
                  {t('plugins.upload')}
                </Button>
              )}
            </div>
...
function PluginRow({
  ...
  const can = useHasPermission();
  const canToggle = can('server:plugins:toggle');
  const canDelete = can('server:plugins:delete');
  ...
  return (
    <FeatureCard.Row interactive className={'min-h-18.25 w-full items-center py-3'}>
          ...
          <div className={'flex items-center gap-2'}>
            ...
            {availableVersion && (
              <Button
                ...
                className={cn('h-auto p-0 hover:bg-transparent', 'cursor-pointer', !onUpdateClick && 'cursor-default')}
              >
                <Badge
                  className={
                ...
                <TooltipTrigger
                  render={
                    <div className={'flex items-center'}>
                      <Switch checked={plugin.enabled} onCheckedChange={() => setToggleConfirm(true)} disabled={!canToggle} />
                    </div>
                  }
                />
