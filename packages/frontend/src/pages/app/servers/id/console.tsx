    ...
    <div className={'flex gap-2'}>
      {isStopped && (
        ...
        >
          ...
          <span className={'hidden sm:inline'}>{t('servers.actions.start')}</span>
        </Button>
      )}
      {isRunning && (
        <>
          ...
          >
            ...
            <span className={'hidden sm:inline'}>{t('servers.actions.stop')}</span>
          </Button>
          <Button
            className={'min-h-[44px] min-w-[44px] bg-amber-500 text-white hover:bg-amber-600'}
            onClick={() => handleAction(restartServer)}
            disabled={isActionPending}
            loading={restartServer.isPending}
          >
            <RotateCcw className={'size-4'} />
            <span className={'hidden sm:inline'}>{t('servers.actions.restart')}</span>
          </Button>
        </>
      )}
    </div>
...
function MetricsBar({
  ...
  return (
    <div className={'grid shrink-0 grid-cols-2 gap-x-4 gap-y-2 sm:flex sm:flex-wrap sm:items-center sm:gap-x-6'}>
      <MetricItem
        icon={Cpu}
      ...
      <TooltipProvider delayDuration={200}>
        <Tooltip>
          ...
          <TooltipContent>
            {isRunning && players.length > 0 ? players.join(', ') : t('players.noPlayers')}
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
      <div className={'hidden h-4 w-px bg-black/6 sm:block'} />
