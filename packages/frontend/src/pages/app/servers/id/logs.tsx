...
import { Archive, ArrowDown, FileText, Loader2, Merge, Radio, Search, ScrollText, Filter, Trash2 } from 'lucide-react';
...
import { useLogFiles, useLogContent, useDeleteLog, useMergeLogs, formatLogSize } from '@shulkr/frontend/hooks/use_logs';
...
function LiveLogs({ serverId }: { serverId: string }) {
  ...
  return (
    <div className={'relative flex h-full flex-col'}>
      <div className={'flex max-w-full flex-1 flex-col overflow-hidden rounded-xl border border-white/10 bg-zinc-950'}>
        <div
          className={
            'flex shrink-0 flex-wrap items-center justify-between gap-2 bg-zinc-900 px-4 py-2.5'
          }
        >
                        ...
                        {bulkMergeConfirm ? (
                          <div className={'flex items-center gap-1.5'}>
                            ...
                            <Button
                              ...
                              variant={'default'}
                              size={'xs'}
                              disabled={mergeLogs.isPending}
                              loading={mergeLogs.isPending}
                            >
                              {t('common.yes')}
                            </Button>
...
function ArchiveViewer({ serverId, filename, onBack }: { serverId: string; filename: string; onBack: () => void }) {
  const { t } = useTranslation();
  ...
  return (
    <div className={'flex h-full flex-col'}>
      <div className={'flex max-w-full flex-1 flex-col overflow-hidden rounded-xl bg-zinc-100/80 p-1 shadow-inner dark:bg-zinc-800/80'}>
        <div className={'flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg shadow-inner-xs'}>
          <div
            className={
              'flex shrink-0 flex-wrap items-center justify-between gap-2 bg-zinc-50 px-4 py-2.5 dark:bg-zinc-900'
            }
          >
            <div className={'flex items-center gap-2'}>
              <Button
          ...
          <div className={'min-h-0 flex-1 overflow-y-auto bg-white px-3 py-2 dark:bg-zinc-950'}>
            {isLoading ? (
