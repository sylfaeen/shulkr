        ...
        <div ref={containerRef} onScroll={handleScroll} className={'min-h-0 flex-1 overflow-y-auto px-3 py-2'}>
          {filteredMessages.length === 0 ? (
            ...
            filteredMessages.map((msg) => {
              ...
              return <ConsoleLogLine key={msg.id} date={datePart} level={msg.level} message={msg.data} />;
            })
          )}
        </div>
      </div>
          ...
          <div className={'min-h-0 flex-1 overflow-y-auto bg-white p-2 dark:bg-zinc-950'}>
            {isLoading ? (
              ...
              filteredLines.map((line, i) => (
                <LogLine key={i} lineNumber={i + 1} date={archiveDate} level={line.level} message={line.message} />
              ))
            )}
          </div>
        </div>
