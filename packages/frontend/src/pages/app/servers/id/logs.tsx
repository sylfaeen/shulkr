        ...
        <div ref={containerRef} onScroll={handleScroll} className={'min-h-0 flex-1 overflow-y-auto px-3 py-2'}>
          {filteredMessages.length === 0 ? (
            ...
            filteredMessages.map((msg) => {
              const datePart = new Date(msg.timestamp).toLocaleDateString(i18n.language, {
                day: '2-digit',
                month: '2-digit',
                year: 'numeric',
              });
...
function extractDateFromFilename(filename: string, locale: string): string | undefined {
  ...
  if (mcMatch) {
    return new Date(`${mcMatch[1]}-${mcMatch[2]}-${mcMatch[3]}`).toLocaleDateString(locale, {
      day: '2-digit',
      month: '2-digit',
      year: 'numeric',
    });
  ...
  if (mergedMatch) {
    return new Date(Number(mergedMatch[1])).toLocaleDateString(locale, {
      day: '2-digit',
      month: '2-digit',
      year: 'numeric',
    });
