...
import { Archive, ArrowDown, FileText, Loader2, Merge, OctagonAlert, Radio, Search, ScrollText, Trash2 } from 'lucide-react';
...
import { useLogFiles, useLogContent, useDeleteLog, useMergeLogs, useMergePreview, formatLogSize } from '@shulkr/frontend/hooks/use_logs';
      ...
      <DialogContent className={'max-w-md'}>
        ...
        <DialogBody>
          <Alert variant={'warning'}>
            ...
            <AlertDescription>
              {t('logs.mergeWarningDescription', { totalLines: totalLines.toLocaleString() })}
            </AlertDescription>
          </Alert>
        </DialogBody>
        <DialogFooter>
          ...
          <div className={'min-h-0 flex-1 overflow-y-auto bg-white p-2 dark:bg-zinc-950'}>
            {isLoading ? (
              ...
              filteredLines.map((line, i) => (
                <LogLine key={i} lineNumber={i + 1} time={line.time} level={line.level} message={line.message} />
              ))
            )}
          </div>
        </div>
