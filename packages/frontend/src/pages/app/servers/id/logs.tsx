...
import { Tabs, TabsList, TabsTrigger } from '@shulkr/frontend/features/ui/base/tabs';
...
function LogsContent({
  ...
  const { messages, isConnected, isConnecting, playerDetails, hasMore, isLoadingMore, loadMore } = useConsoleWebSocket(serverId);
...
function extractDateFromFilename(filename: string, locale: string): string | undefined {
  ...
  if (mcMatch) {
    return formatDisplayDate(`${mcMatch[1]}-${mcMatch[2]}-${mcMatch[3]}`, locale);
  }
            ...
            {isLoading ? (
              ...
              filteredLines.map((line, i) => (
                <LogLine
                  key={i}
                  lineNumber={i + 1}
                  date={line.date ? formatDisplayDate(line.date, i18n.language) : archiveDate}
                  level={line.level}
    ...
    <div className={'flex items-center gap-1'}>
      {LEVELS.map((level) => (
        <Button
          ...
          className={cn(
            'h-auto rounded-full px-2.5 py-0.5 text-[11px] font-semibold transition-colors',
            active === level
              ? level === 'ERROR'
