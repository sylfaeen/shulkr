...
function TasksSection({ serverId }: { serverId: string }) {
  ...
  const { items: sortedTasks, control: sortControl } = useSortableList<ScheduledTask>(tasks, {
    storageKey: 'tasks',
    fields: [
    ...
    <FeatureCard.Stack className={'gap-y-0'}>
      <FeatureCard.Row>
        <Button
          type={'button'}
          variant={'ghost'}
          className={'flex h-auto items-start justify-start gap-3 whitespace-normal rounded-none p-0 text-left font-normal hover:bg-transparent dark:hover:bg-transparent'}
