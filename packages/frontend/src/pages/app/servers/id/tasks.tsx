...
import { Alert, AlertDescription } from '@shulkr/frontend/features/ui/base/alert';
...
function TasksSection({ serverId }: { serverId: string }) {
  ...
  const filteredTasks = useMemo(() => {
    ...
    return tasks.filter((task) => task.name.toLowerCase().includes(q) || (task.command ?? '').toLowerCase().includes(q));
  }, [tasks, searchQuery]);

  const { items: sortedTasks, control: sortControl } = useSortableList<ScheduledTask>(filteredTasks, {
    storageKey: 'tasks',
    fields: [
      { key: 'name', label: t('common.sort.byName'), accessor: (task) => task.name },
      { key: 'createdAt', label: t('common.sort.byCreatedAt'), accessor: (task) => task.createdAt, type: 'date' },
    ],
  });
    ...
    <>
      <FeatureCard>
        <FeatureCard.Header>
          ...
          <FeatureCard.Actions>
            {tasks && tasks.length > 0 && sortControl}
            {canCreate && (
              <>
    ...
    <FeatureCard.Stack className={'gap-y-0'}>
      <FeatureCard.Row>
        <Button
          ...
          variant={'ghost'}
          className={
            'flex h-auto items-start justify-start gap-3 rounded-none p-0 text-left font-normal whitespace-normal hover:bg-transparent dark:hover:bg-transparent'
          }
            ...
            <>
              <TooltipProvider delay={300}>
                ...
                {canRun && (
                  <Tooltip>
                    <TooltipTrigger
                      render={<Button onClick={() => onRun(task.id)} variant={'ghost'} size={'icon'} icon={Play} />}
                    />
                ...
                {canUpdate && (
                  <Tooltip>
                    <TooltipTrigger
                      render={<Button onClick={() => onEdit(task)} variant={'ghost'} size={'icon'} icon={Pencil} />}
                    />
    ...
    <div className={'mt-2 flex flex-wrap items-center gap-1'}>
      {visiblePaths.map((p) => {
        ...
        return (
          <span
            key={p}
            className={
              'font-jetbrains inline-flex items-center gap-1 rounded-md border border-black/6 bg-zinc-100/80 px-1.5 py-0.5 text-sm text-zinc-600 dark:border-white/8 dark:bg-zinc-800/80 dark:text-zinc-400'
            }
          >
      ...
      {hiddenCount > 0 && (
        <Button
          ...
          className={
            'font-jetbrains inline-flex h-auto items-center rounded-md border border-black/6 bg-zinc-200/60 px-1.5 py-0.5 text-sm font-medium text-zinc-500 transition-colors hover:bg-zinc-200 hover:text-zinc-700 dark:border-white/8 dark:bg-zinc-700/60 dark:text-zinc-400 dark:hover:bg-zinc-700 dark:hover:text-zinc-300'
          }
        >
      ...
      {steps.map((step, index) => {
        ...
        return (
          <li key={index} className={'flex items-center gap-1'}>
            <span
              className={cn(
                'inline-flex items-center gap-1 rounded-md border border-black/6 bg-zinc-100/80 px-1.5 py-0.5 text-sm text-zinc-600 dark:border-white/8 dark:bg-zinc-800/80 dark:text-zinc-400',
                step.onError === 'continue' && 'border-amber-500/30 bg-amber-500/5'
              )}
              title={summary}
            >
              <span className={'font-jetbrains text-xs font-medium text-zinc-500 tabular-nums dark:text-zinc-500'}>
                {index + 1}
              </span>
              ...
              return (
                <div key={exec.id}>
                  <Button
                    ...
                    className={
                      'flex h-auto w-full items-center justify-between gap-2 rounded-md px-2 py-1 text-sm font-normal hover:bg-zinc-100/60 dark:hover:bg-zinc-800/60'
                    }
                  >
                    <div className={'flex items-center justify-start gap-2'}>
                      <StatusIcon status={exec.status} />
                      <span className={'font-jetbrains text-xs text-zinc-600 tabular-nums dark:text-zinc-400'}>
                        {new Date(exec.executedAt).toLocaleString()}
                      </span>
                      {exec.status === 'skipped' && (
                        <span
                          className={
                            'rounded-sm bg-zinc-500/10 px-1.5 py-0.5 text-xs font-medium text-zinc-500 dark:text-zinc-400'
                          }
                        >
                    ...
                    <div className={'flex items-center gap-2'}>
                      {exec.output && (exec.status === 'error' || exec.status === 'failure') && (
                        <span className={'max-w-60 truncate text-xs text-red-400'}>{exec.output}</span>
                      )}
                      ...
                      {exec.status === 'pending' && (
                        <span className={'text-xs text-zinc-400 dark:text-zinc-500'}>{t('tasks.pending')}</span>
                      )}
                      {exec.status === 'running' && <span className={'text-xs text-blue-500'}>{t('tasks.running')}</span>}
                      <ChevronDown
                        className={cn(
    ...
    return (
      <Alert variant={'info'}>
        ...
        <AlertDescription>
          ...
          <p className={'text-xs opacity-70'}>{t('tasks.timezoneBannerSame')}</p>
        </AlertDescription>
      </Alert>
    );
  }
      ...
      <AlertDescription>
        <div className={'flex flex-wrap items-center gap-x-3 gap-y-1'}>
          ...
          <div className={'flex items-center gap-1.5'}>
            ...
            <span className={'font-jetbrains text-xs tabular-nums opacity-60'}>
              ({diffHours >= 0 ? '+' : ''}
              {diffHours}h)
            </span>
          </div>
        </div>
        <p className={'text-xs opacity-70'}>{t('tasks.timezoneBannerWarning')}</p>
      </AlertDescription>
    </Alert>
  );
}
