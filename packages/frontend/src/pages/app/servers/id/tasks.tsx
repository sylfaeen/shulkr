...
import { CheckCircle2, ChevronDown, Clock, File, Folder, Loader2, Pencil, Plus, Terminal, Trash2, XCircle } from 'lucide-react';
...
export function ServerTasksPage() {
  ...
  if (!serverId) return <PageError message={t('errors.generic')} />;
          ...
          {toggleConfirm === task.id ? (
            ...
            <>
              <TooltipProvider delayDuration={300}>
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button onClick={() => onDeleteConfirm(task.id)} variant={'ghost-destructive'} size={'icon-sm'}>
...
type BackupPathsDisplayProps = {
  command: string;
};

function BackupPathsDisplay({ command }: BackupPathsDisplayProps) {
  const [expanded, setExpanded] = useState(false);
