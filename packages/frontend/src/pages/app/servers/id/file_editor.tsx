          ...
          <div className={cn('shadow-inner-xs flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg', MONACO_CONTAINER_CLASS)}>
            ...
            >
              ...
              <div className={'flex items-center gap-2'}>
                {saveStatus === 'saved' && (
                  <span className={'text-xs font-medium text-green-600 dark:text-green-400'}>{t('common.save')} ✓</span>
                )}
                {hasChanges && <span className={'text-xs font-medium text-amber-500'}>{t('files.unsavedChanges')}</span>}
              </div>
            </div>
          </div>
        </div>
