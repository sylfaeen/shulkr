...
export function ServerFileEditorPage() {
  ...
  return (
    <>
      ...
      <PageContent>
        <div className={'flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl bg-zinc-100/80 p-1 shadow-inner dark:bg-zinc-800/80'}>
          <div className={cn('flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg shadow-inner-xs', MONACO_CONTAINER_CLASS)}>
            ...
            <div
              className={
                'flex shrink-0 items-center justify-between border-t border-zinc-200/80 bg-zinc-50 px-4 py-1.5 dark:border-zinc-700/50 dark:bg-zinc-900'
              }
            >
              <div className={'flex items-center gap-3'}>
                <span
                  className={
                    'font-jetbrains rounded-md bg-zinc-200/60 px-2 py-0.5 text-[11px] font-medium text-zinc-600 dark:bg-zinc-700/60 dark:text-zinc-400'
                  }
                >
                  {language}
                </span>
                <span className={'text-[11px] text-zinc-400 dark:text-zinc-500'}>{filename}</span>
              </div>
              <div className={'flex items-center gap-2'}>
                {saveStatus === 'saved' && (
                  <span className={'text-[11px] font-medium text-green-600 dark:text-green-400'}>{t('common.save')} ✓</span>
                )}
                {hasChanges && (
                  <span className={'text-[11px] font-medium text-amber-500'}>{t('files.unsavedChanges')}</span>
                )}
              </div>
            </div>
          </div>
