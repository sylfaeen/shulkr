...
import { useDownloadFile, formatFileSize } from '@shulkr/frontend/hooks/use_files';
import {
  useSqlite,
  type TableInfo,
      ...
      <PageContent fill>
        {server.status === 'running' && (
          <Alert variant={'warning'} className={'mb-4'}>
            ...
            <AlertDescription>
              {t('files.dbViewer.serverRunningWarning', 'The server is running — data may be incomplete or locked.')}
            </AlertDescription>
          </Alert>
        )}
        <DbViewerContent serverId={server.id} {...{ filePath }} />
      </PageContent>
    </>
  );
}

function DbViewerContent({ serverId, filePath }: { serverId: string; filePath: string }) {
  const { t } = useTranslation();
  ...
  if (sqlite.needsSizeWarning) {
    return (
      <SizeWarning
        fileSize={sqlite.fileSize}
        onAccept={sqlite.acceptSizeWarning}
        onDownload={() => {
          download(filePath);
        }}
      />
    ...
    return (
      <StatusCard>
        ...
        <p className={'font-medium text-zinc-700 dark:text-zinc-300'}>
          {isNotSqlite
            ? t('files.dbViewer.notSqlite', 'This file is not a valid SQLite database')
            : t('files.dbViewer.corrupt', 'This database file appears to be corrupted')}
        </p>
        <p className={'text-sm text-zinc-500 dark:text-zinc-400'}>{!isNotSqlite && sqlite.state.error}</p>
        <div className={'flex gap-2'}>
          <Button variant={'secondary'} onClick={() => download(filePath)}>
            <Download className={'size-4'} />
            {t('files.download')}
          </Button>
          {!isNotSqlite && (
            <Button variant={'secondary'} onClick={sqlite.reload}>
  ...
  if (tables.length === 0) {
    return (
      <StatusCard>
        ...
        <p className={'font-medium text-zinc-700 dark:text-zinc-300'}>
          {t('files.dbViewer.emptyDb', 'This database contains no tables')}
        </p>
      </StatusCard>
    );
  }
    ...
    <div className={'flex min-h-0 flex-1 flex-col gap-4'}>
      <div className={'relative shrink-0'}>
        ...
        <input
          ...
          placeholder={t('files.dbViewer.searchPlaceholder', 'Search across all tables...')}
          className={
...
function GlobalSearchResults({ results, term, onSelectTable }: GlobalSearchResultsProps) {
  const { t } = useTranslation();
    ...
    return (
      <StatusCard>
        ...
        <p className={'text-sm text-zinc-500 dark:text-zinc-400'}>
          {t('files.dbViewer.noResults', "No results found for '{{term}}'", { term })}
        </p>
      </StatusCard>
    );
  }
...
function HighlightedCell({ value, term }: HighlightedCellProps) {
  if (value === null) {
...
function SizeWarning({ fileSize, onAccept, onDownload }: SizeWarningProps) {
  const { t } = useTranslation();
  return (
    <StatusCard>
      <AlertTriangle className={'size-8 text-amber-500'} strokeWidth={1.5} />
      <p className={'font-medium text-zinc-700 dark:text-zinc-300'}>{t('files.dbViewer.largeFile', 'This database is large')}</p>
      <p className={'text-sm text-zinc-500 dark:text-zinc-400'}>
        {t('files.dbViewer.largeFileDesc', '{{size}} will be loaded into your browser memory. This may affect performance.', {
          size: formatFileSize(fileSize),
        })}
      </p>
      <div className={'flex gap-2'}>
        <Button onClick={onAccept}>{t('files.dbViewer.loadAnyway', 'Load anyway')}</Button>
...
function DatabaseOverview({ tables, fileSize, onSelectTable }: DatabaseOverviewProps) {
  const { t } = useTranslation();
  ...
  return (
    <div className={'flex flex-1 flex-col overflow-hidden'}>
      <div className={'flex items-center gap-4 border-b border-black/10 px-4 py-2.5 dark:border-white/10'}>
        <OverviewStat label={t('files.dbViewer.tables', 'Tables')} value={String(tableCount)} />
        <OverviewStat label={t('files.dbViewer.views', 'Views')} value={String(viewCount)} />
        <OverviewStat label={t('files.dbViewer.totalRows', 'Rows')} value={totalRows.toLocaleString()} />
        <OverviewStat label={t('files.dbViewer.fileSize', 'Size')} value={formatFileSize(fileSize)} />
      </div>
...
function TableList({ tables, selectedTable, onSelect }: TableListProps) {
  return (
        ...
        <TooltipProvider delayDuration={300}>
          <div className={'flex shrink-0 items-center gap-1'}>
            <Tooltip>
              ...
              <TooltipContent>{t('files.dbViewer.raw', 'Raw mode')}</TooltipContent>
            </Tooltip>
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
              ...
              <TooltipContent>{t('files.dbViewer.filter', 'Filter')}</TooltipContent>
            </Tooltip>
                  ...
                  return (
                    <Tooltip key={col}>
                      <TooltipTrigger asChild>
                        ...
                        >
                          {col}
                          {colSchema?.pk && <span className={'ml-1 text-amber-500'}>*</span>}
      ...
      {totalPages > 1 && (
        <div className={'flex items-center justify-between border-t border-black/10 px-4 py-2 dark:border-white/10'}>
          ...
          <div className={'flex items-center gap-0.5'}>
            <Button variant={'ghost'} size={'icon-sm'} disabled={data.page <= 1} onClick={() => onPageChange(data.page - 1)}>
  ...
  if (!isOpen) {
    return (
      ...
      >
        ...
        {t('files.dbViewer.openConsole', 'SQL Console')}
        <kbd
          className={
  ...
  return (
    <div className={'shrink-0 overflow-hidden rounded-xl border border-black/10 bg-white dark:border-white/10 dark:bg-zinc-900'}>
      <div className={'flex items-center justify-between border-b border-black/10 px-4 py-2 dark:border-white/10'}>
        <div className={'flex items-center gap-2.5'}>
          ...
          <span className={'text-sm font-medium text-zinc-700 dark:text-zinc-300'}>
            {t('files.dbViewer.sqlConsole', 'SQL Console')}
          </span>
          <span
            className={
          ...
          >
            {t('files.dbViewer.readOnly', 'READ-ONLY')}
          </span>
        </div>
        <div className={'flex items-center gap-2'}>
          ...
          {sql.trim() && (
            <Button
              ...
              title={t('files.dbViewer.saveBookmark', 'Save as bookmark')}
            >
              <BookmarkCheck className={'size-3'} />
            </Button>
          )}
          <Button size={'xs'} onClick={handleExecute} disabled={!sql.trim() || executing} loading={executing}>
            <Play className={'size-3'} />
            {t('files.dbViewer.execute', 'Execute')}
          </Button>
      ...
      {showBookmarkInput && (
        <div className={'flex items-center gap-2 border-b border-black/10 px-4 py-2 dark:border-white/10'}>
          <input
            ...
            placeholder={t('files.dbViewer.bookmarkName', 'Bookmark name...')}
            className={
      ...
      {showBookmarks && (
        <div className={'max-h-40 overflow-y-auto border-b border-black/10 dark:border-white/10'}>
          {bookmarks.length === 0 ? (
            <p className={'px-4 py-3 text-xs text-zinc-400'}>{t('files.dbViewer.noBookmarks', 'No bookmarks yet')}</p>
          ) : (
            bookmarks.map((bm, i) => (
              <div
                key={i}
              ...
              >
                ...
                <Button variant={'ghost-destructive'} size={'icon-sm'} onClick={() => handleDeleteBookmark(i)}>
                  <X className={'size-3'} />
                </Button>
              </div>
            ))
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant={'ghost'} size={'xs'} disabled={exporting} loading={exporting}>
          <FileDown className={'size-3'} />
          {t('files.dbViewer.export', 'Export')}
        </Button>
      </DropdownMenuTrigger>
