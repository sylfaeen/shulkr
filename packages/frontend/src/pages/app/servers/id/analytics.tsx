...
import { SkeletonCard, SkeletonChart } from '@shulkr/frontend/features/ui/skeleton_presets';
...
import { GcPanel } from '../../../../features/gc_panel';
import { AgentLiveSections } from '@shulkr/frontend/pages/app/servers/features/analytics/agent_live_sections';
import { useAgentStatus } from '@shulkr/frontend/hooks/use_agent';
import { useAgentLive } from '@shulkr/frontend/hooks/use_agent_live';
...
function AgentEnrichedSection({ serverId }: { serverId: string }) {
  const { data: status } = useAgentStatus(serverId);
  const isConnected = status?.connected === true;
  const { data: live } = useAgentLive(serverId, isConnected);

  if (!isConnected || !live) return null;

  return <AgentLiveSections data={live} />;
}
  ...
  return (
    <div className={'space-y-6'}>
      ...
      {isLoading ? (
        ...
        <EmptyState icon={BarChart3} title={t('analytics.empty.title')} description={t('analytics.empty.description')} />
      ) : (
        <>
