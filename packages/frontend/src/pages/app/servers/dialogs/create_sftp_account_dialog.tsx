...
type CreateSftpAccountDialogProps = {
  open: boolean;
  onOpenChange: () => void;
  serverId: string;
};

export function CreateSftpAccountDialog({ open, onOpenChange, serverId }: CreateSftpAccountDialogProps) {
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onOpenChange()}>
      <DialogContent className={'max-w-2xl'}>
        ...
        <DialogFooter>
          ...
          <Button type={'submit'} form={'create-sftp-account-form'}>
...
type CreateSftpAccountFormProps = {
  serverId: string;
  enabled: boolean;
  onClose: () => void;
};

function CreateSftpAccountForm({ serverId, enabled, onClose }: CreateSftpAccountFormProps) {
  const { t } = useTranslation();
