...
function CreateSftpAccountForm({ serverId, enabled, onClose }: { serverId: string; enabled: boolean; onClose: () => void }) {
  ...
  const form = useForm<CreateSftpAccountFormValues>({
    ...
    defaultValues: { username: '', password: '', permissions: 'read-write', allowedPaths: [] },
  });
...
export function SftpAccountFormFields({
  ...
  const allowedPaths = form.watch('allowedPaths');
  const selectedPaths = new Set(allowedPaths);

  const handleSelectedPathsChange = (paths: Set<string>) => {
    form.setValue('allowedPaths', Array.from(paths), { shouldDirty: true, shouldValidate: true });
  };
  ...
  return (
    <>
              ...
              <FormControl>
                <SelectTrigger>
                  <SelectValue>
                    {(value: string) =>
                      value === 'read-only'
                        ? t('settings.sftp.dialog.permissionReadOnly')
                        : t('settings.sftp.dialog.permissionReadWrite')
                    }
                  </SelectValue>
                </SelectTrigger>
              </FormControl>
      ...
      />
      <div>
        <Label>{t('settings.sftp.dialog.allowedPaths')}</Label>
