...
import { usePlugins } from '@shulkr/frontend/hooks/use_plugins';
...
function MarketplaceHeader({ showBack, source, onBack, onClose, onSourceChange }: MarketplaceHeaderProps) {
  return (
...
function PluginRow({ hit, onSelect }: PluginRowProps) {
  return (
    <button
      onClick={onSelect}
      className={cn(
...
function PluginDetail({
  ...
  const { data: pluginsData } = usePlugins(serverId);
  ...
  const installedVersions = useMemo(() => {
    const set = new Set(sessionInstalled);
            ...
            <div className={'flex items-center gap-2.5'}>
              ...
              <a
                ...
                className={cn(
                  ...
                  source === 'hangar' ? 'text-blue-500 hover:bg-blue-500/10' : 'text-[#1bd96a] hover:bg-[#1bd96a]/10'
                )}
              >
          ...
          {versionsLoading ? (
            ...
            <div className={'flex flex-col gap-1.5'}>
              {versions.slice(0, 15).map((version, idx) => {
                ...
                const isInstalled = installedVersions.has(version.version_number);
