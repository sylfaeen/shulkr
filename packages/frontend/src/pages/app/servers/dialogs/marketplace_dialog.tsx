...
import { ArrowLeft, ArrowUpRight, Check, Download, LoaderCircle, Package, Search, X } from 'lucide-react';
  ...
  return (
    ...
    >
      <DialogContent className={'max-w-3xl'} showCloseButton={false}>
        ...
        />
        {view.type === 'detail' ? (
          <PluginDetail idOrSlug={view.idOrSlug} onBack={() => setView({ type: 'search' })} {...{ serverId }} />
        ) : (
          <div className={'max-h-[70vh] overflow-y-auto p-5'}>
            <MarketplaceSearch onSelectPlugin={(slug) => setView({ type: 'detail', idOrSlug: slug })} />
          </div>
        )}
      </DialogContent>
    </Dialog>
...
function MarketplaceHeader({ showBack, onBack, onClose }: { showBack: boolean; onBack: () => void; onClose: () => void }) {
  return (
    <div className={'flex items-center justify-between border-b border-black/10 px-5 py-3.5 dark:border-white/10'}>
      <div className={'flex items-center gap-3'}>
        ...
        <div className={'flex items-center gap-2.5'}>
          ...
          <span
            className={
              'rounded-full bg-[#1bd96a]/10 px-2 py-0.5 text-[10px] font-semibold tracking-wide text-[#1bd96a] uppercase'
            }
          >
            Plugins
          </span>
...
function MarketplaceSearch({ onSelectPlugin }: { onSelectPlugin: (slug: string) => void }) {
  const { t } = useTranslation();
  const [query, setQuery] = useState('');
  const [searchQuery, setSearchQuery] = useState('');

  const { data, isLoading } = useMarketplaceSearch({ q: searchQuery, limit: 30 });
  ...
  return (
    <div className={'flex flex-col gap-4'}>
      <div className={'relative'}>
        ...
        {isLoading && (
          <LoaderCircle
            className={'absolute top-1/2 right-3.5 size-4 -translate-y-1/2 animate-spin text-[#1bd96a]'}
            strokeWidth={2}
          />
        )}
      </div>
      {!searchQuery.trim() && (
        <div className={'flex flex-col items-center gap-4 py-14 text-center'}>
          <div className={'flex size-16 items-center justify-center rounded-2xl bg-[#1bd96a]/5 dark:bg-[#1bd96a]/10'}>
            <Search className={'size-7 text-[#1bd96a]/40'} strokeWidth={1.5} />
          </div>
          <div>
            <p className={'text-sm font-medium text-zinc-600 dark:text-zinc-400'}>{t('marketplace.startSearching')}</p>
            <p className={'mt-1 text-xs text-zinc-400 dark:text-zinc-500'}>{t('marketplace.searchHint')}</p>
          </div>
        </div>
      )}
        ...
        <div className={'flex flex-col gap-3'}>
          ...
          <div className={'flex flex-col gap-1.5'}>
            {data.hits.map((hit) => (
              <button
                key={hit.project_id}
                onClick={() => onSelectPlugin(hit.slug)}
                className={cn(
                  'group flex items-center gap-3.5 rounded-xl px-3.5 py-3 text-left transition-all',
              ...
              >
                ...
                <div className={'min-w-0 flex-1'}>
                  <div className={'flex items-center gap-2'}>
                    <span
                      className={
                        'text-sm font-medium text-zinc-800 group-hover:text-zinc-900 dark:text-zinc-200 dark:group-hover:text-zinc-100'
                      }
                    >
                      {hit.title}
                    </span>
                ...
                <div className={'flex shrink-0 items-center gap-3'}>
                  ...
                  <ArrowUpRight
                    className={
                      'size-3.5 text-zinc-300 transition-colors group-hover:text-[#1bd96a] dark:text-zinc-600 dark:group-hover:text-[#1bd96a]'
                    }
                  />
                </div>
              </button>
...
function PluginDetail({ idOrSlug, serverId, onBack }: { idOrSlug: string; serverId: string; onBack: () => void }) {
  const { t } = useTranslation();
  const { data: project, isLoading: projectLoading } = useMarketplaceProject(idOrSlug);
  const { data: versions, isLoading: versionsLoading } = useMarketplaceVersions(idOrSlug);
  ...
  const handleInstall = useCallback(
    (version: { id: string; files: Array<{ url: string; filename: string; hashes: { sha512: string }; primary: boolean }> }) => {
      if (!project) return;
      ...
      installMutation.mutate(
        ...
        { onSuccess: () => setInstalledVersions((prev) => new Set(prev).add(version.id)) }
      );
    },
    [project, installMutation]
  );

  if (projectLoading) {
    return (
      <div className={'flex items-center justify-center py-20'}>
        <LoaderCircle className={'size-6 animate-spin text-[#1bd96a]'} strokeWidth={2} />
      </div>
    );
  }
          ...
          <div className={'min-w-0 flex-1'}>
            <div className={'flex items-center gap-2.5'}>
              ...
              <a
                ...
                className={
                  'flex items-center gap-1 rounded-md px-2 py-0.5 text-xs text-[#1bd96a] transition-colors hover:bg-[#1bd96a]/10'
                }
              >
                modrinth
          ...
          {versionsLoading ? (
            ...
            <div className={'flex flex-col gap-1.5'}>
              {versions.slice(0, 15).map((version, idx) => {
                ...
                const isInstalled = installedVersions.has(version.id);
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      idx === 0
                        ? 'bg-[#1bd96a]/5 ring-1 ring-[#1bd96a]/15 dark:bg-[#1bd96a]/5 dark:ring-[#1bd96a]/10'
                  ...
                  >
                    <div className={'min-w-0 flex-1'}>
                      <div className={'flex items-center gap-2'}>
                        ...
                        {idx === 0 && (
                          <span
                            className={
                              'rounded-full bg-[#1bd96a]/15 px-1.5 py-0.5 text-[9px] font-bold tracking-wide text-[#1bd96a] uppercase'
                            }
                          >
                            Latest
                          </span>
                      ...
                      <div className={'mt-1.5 flex flex-wrap items-center gap-1'}>
                        ...
                        {version.loaders.map((l) => (
                          <span
                            key={l}
                            className={
                              'font-jetbrains rounded bg-[#1bd96a]/8 px-1.5 py-0.5 text-[10px] text-[#1bd96a] dark:bg-[#1bd96a]/10'
                            }
                          >
                            {l}
                          </span>
                        ))}
                        <span className={'font-jetbrains ml-1 text-[10px] text-zinc-300 dark:text-zinc-600'}>
                    ...
                    <div className={'flex shrink-0 items-center gap-2.5'}>
                      {primaryFile && (
                        <span className={'font-jetbrains text-[10px] text-zinc-400 tabular-nums dark:text-zinc-500'}>
                          {primaryFile.size >= 1_048_576
                            ? `${(primaryFile.size / 1_048_576).toFixed(1)} MB`
                      ...
                      {isInstalled ? (
                        <span
                          className={
                            'inline-flex items-center gap-1 rounded-lg bg-[#1bd96a]/10 px-3 py-1.5 text-xs font-medium text-[#1bd96a]'
                          }
                        >
