...
import type { WebhookEvent, WebhookLanguage } from '@shulkr/shared';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
import { Popover, PopoverContent, PopoverTrigger } from '@shulkr/frontend/features/ui/base/popover';
...
export function CreateWebhookDialog({
  ...
  const [events, setEvents] = useState<Array<WebhookEvent>>([...ALL_EVENTS]);
          ...
          <div className={'space-y-2'}>
            ...
            <Select value={language} onValueChange={(v) => onLanguageChange(v as WebhookLanguage)}>
              <SelectTrigger className={'w-40'}>
                <SelectValue>{(value: string) => LANGUAGES.find((lang) => lang.code === value)?.name ?? value}</SelectValue>
              </SelectTrigger>
              <SelectContent>
                {LANGUAGES.map((lang) => (
...
function MessageEditor({
  ...
  const activeEvent = focusedEvent && events.includes(focusedEvent as WebhookEvent) ? focusedEvent : (selectedEvents[0] ?? null);
              ...
              <div key={group.group} className={'py-0.5'}>
                ...
                {groupEvents.map((event) => (
                  <Button
                    ...
                    className={cn(
                      'flex h-auto w-full items-center justify-start gap-2 rounded-none px-3 py-1.5 text-left text-sm font-normal transition-colors',
                      activeEvent === event
        ...
        <div className={'flex items-center gap-0.5'}>
          {isCustom && (
            <Button
              ...
              onClick={onReset}
              className={
                'p-1 text-zinc-400 hover:bg-zinc-100 hover:text-zinc-600 dark:hover:bg-zinc-800 dark:hover:text-zinc-300'
              }
        ...
        <div className={'flex flex-wrap items-center gap-1.5'}>
          {variables.map((v) => (
            <Button
              ...
              className={
                'font-jetbrains h-auto rounded bg-zinc-100 px-1.5 py-0.5 text-xs font-normal hover:bg-zinc-200 dark:bg-zinc-800 dark:hover:bg-zinc-700'
              }
            >
              {`{{${v}}}`}
            </Button>
      ...
      <PopoverTrigger
        render={
          <Button
            ...
            variant={'ghost'}
            size={'icon-xs'}
            className={'p-1 text-zinc-400 hover:bg-zinc-100 hover:text-zinc-600 dark:hover:bg-zinc-800 dark:hover:text-zinc-300'}
