...
function CreateTaskForm({
  ...
  const [chainSteps, setChainSteps] = useState<Array<ChainStep>>([]);
  const [conditions, setConditions] = useState<TaskConditions | null>(null);
  ...
  const handleFormSubmit = (data: TaskFormValues) => {
    ...
    if (taskType === 'chain' && chainSteps.length === 0) {
    ...
    return onSubmit({
      ...
      ...(taskType === 'chain' && { steps: chainSteps }),
    ...
    <Form {...form}>
      <form id={'create-task-form'} className={'space-y-6'} onSubmit={form.handleSubmit(handleFormSubmit)}>
        <TaskFormFields
          ...
          {...{ form, taskType, serverId, serverTime, backupPaths, chainSteps, conditions, treeRef }}
        />
      </form>
    </Form>
  );
...
type TaskFormFieldsProps = {
  ...
  chainSteps: Array<ChainStep>;
  onChainStepsChange: (steps: Array<ChainStep>) => void;
  conditions: TaskConditions | null;
  onConditionsChange: (conditions: TaskConditions | null) => void;
...
export function TaskFormFields({
  ...
  return (
    <>
      ...
      {taskType === 'chain' && <ChainEditor steps={chainSteps} onChange={onChainStepsChange} {...{ serverId }} />}
      <ConditionEditor value={conditions} onChange={onConditionsChange} />
    </>
  );
}
