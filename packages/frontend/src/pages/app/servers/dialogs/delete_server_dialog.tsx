        ...
        <DialogBody>
          ...
          {isRunning && (
            <Alert variant={'destructive'}>
              ...
              <AlertDescription dangerouslySetInnerHTML={{ __html: t('servers.deleteMustBeStopped') }} />
            </Alert>
          )}
          {!isRunning && (
            <Alert variant={'info'}>
              <OctagonAlert className={'size-4'} />
              <AlertDescription dangerouslySetInnerHTML={{ __html: t('servers.backupQuestion') }} />
            </Alert>
          )}
        </DialogBody>
        <DialogFooter>
          <div className={'flex w-full flex-col gap-2'}>
            ...
            <Button onClick={onCancel} variant={'secondary'} className={'w-full'} disabled={isDeleting}>
              {t('common.cancel')}
            </Button>
          </div>
        </DialogFooter>
