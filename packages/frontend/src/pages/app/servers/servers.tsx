...
import { isWizardSkipped } from '@shulkr/frontend/hooks/use_wizard';
...
export function ServersPage() {
  ...
  const isEmpty = !isLoading && servers && servers.length === 0 && !showCreateDialog;
  const shouldRedirectToWizard = Boolean(isEmpty && canCreate && user && !isWizardSkipped(user.id));

  const { items: sortedServers, control: sortControl } = useSortableList<ServerResponse>(servers, {
    storageKey: 'servers',
    fields: [
      { key: 'name', label: t('common.sort.byName'), accessor: (s) => s.name },
      { key: 'createdAt', label: t('common.sort.byCreatedAt'), accessor: (s) => s.created_at, type: 'date' },
    ],
  });

  useEffect(() => {
    if (shouldRedirectToWizard) navigate({ to: '/app/first-server' });
      ...
      <div className={'space-y-6'}>
        <div className={'flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'}>
          ...
          <div className={'flex flex-wrap items-center gap-2'}>
            {servers && servers.length > 0 && sortControl}
            {canCreate && (
        ...
        <div className={'space-y-4'}>
          ...
          {sortedServers.map((server: ServerResponse) => (
