...
export function FirstServerWizardPage() {
  ...
  const handleCreate = async () => {
    const result = await createServer
      .mutateAsync({
        name: state.name.trim() || 'Mon serveur',
        type: state.type,
        size: state.size,
          ...
          return (
            <li
              ...
              className={cn(
                ...
                checked ? 'border-sky-500/30 bg-sky-500/5' : 'border-black/8 bg-white dark:border-white/8 dark:bg-zinc-900/40'
              )}
            >
              <Checkbox checked={checked} onCheckedChange={() => onToggle(p)} disabled={p.required} className={'mt-0.5'} />
