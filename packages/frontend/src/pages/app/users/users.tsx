...
export function UsersPage() {
  ...
  const { items: sortedUsers, control: sortControl } = useSortableList<UserResponse>(users, {
    storageKey: 'users',
    fields: [
      { key: 'username', label: t('common.sort.byUsername'), accessor: (u) => u.username },
      { key: 'createdAt', label: t('common.sort.byCreatedAt'), accessor: (u) => u.created_at, type: 'date' },
    ],
  });
      ...
      <div className={'space-y-6'}>
        <div className={'flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'}>
          ...
          <div className={'flex flex-wrap items-center gap-2'}>
            {users && users.length > 0 && sortControl}
            {canCreate && (
...
function UserRow({ user, currentUserId, onEdit }: { user: UserResponse; currentUserId: number | undefined; onEdit: () => void }) {
  const { t } = useTranslation();
