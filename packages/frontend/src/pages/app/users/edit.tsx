...
function EditSection({ user, isSelf }: { user: { id: number; username: string; permissions: Array<string> }; isSelf: boolean }) {
  ...
  const handleSubmit = async (data: EditUserFormValues) => {
    ...
    try {
      ...
      form.reset({ username: data.username, password: '', permissions: data.permissions });
    } catch (err) {
          ...
          <FeatureCard.Body>
            ...
            <FeatureCard.Row layout={'column'}>
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className={'w-full'}>
                    <FormLabel>{t('users.permissions')}</FormLabel>
                    {isSelf && (
                    ...
                    <FormControl>
                      <div className={cn('w-full', isSelf && 'pointer-events-none opacity-60')}>
                        <PermissionPicker value={field.value} onChange={field.onChange} disabled={isSelf} />
                      </div>
                    </FormControl>
                    <FormMessage />
                  </FormItem>
