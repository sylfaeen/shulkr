...
export function EditUserDialog({ user, onSubmit, onCancel, isLoading, error }: EditUserDialogProps) {
  ...
  const togglePermission = (permission: string) => {
    if (permission === '*') {
      form.setValue('permissions', permissions.includes('*') ? [] : ['*'], { shouldDirty: true });
    } else {
      const newPermissions = permissions.filter((p) => p !== '*');
      const next = newPermissions.includes(permission)
  ...
  const handleSubmit = (data: EditUserFormValues) => {
    const formData: EditUserFormData = { username: data.username, permissions: data.permissions };
    if (data.password) {
