...
// Shapes consumed by the database browser. They mirror what the SQLite viewer already renders, so the same UI serves both engines.
...
export type DatabaseTableInfo = z.infer<typeof databaseTableInfoSchema>;

export const databaseTableSchemaSchema = z.object({
  columns: z.array(
    z.object({
      cid: z.number(),
      name: z.string(),
  ...
  foreignKeys: z.array(z.object({ from: z.string(), table: z.string(), to: z.string() })),
});

export type DatabaseTableSchema = z.infer<typeof databaseTableSchemaSchema>;
...
export const databaseTableDataResultSchema = databaseQueryResultSchema.extend({
  totalRows: z.number(),
  page: z.number(),
  pageSize: z.number(),
});

export type DatabaseTableDataResult = z.infer<typeof databaseTableDataResultSchema>;

export const databaseColumnFilterSchema = z.object({
  column: z.string(),
  value: z.string(),
});

export type DatabaseColumnFilter = z.infer<typeof databaseColumnFilterSchema>;

export const databaseSearchMatchSchema = z.object({
  table: z.string(),
  columns: z.array(z.string()),
  rows: z.array(z.array(z.unknown())),
  matchCount: z.number(),
...
export type DatabaseSearchMatch = z.infer<typeof databaseSearchMatchSchema>;
