...
const agentPlayerSchema = z.object({
  ...
  // Live player state (game servers only)
  health: z.number().min(0).max(20).nullable().optional(),
  food: z.number().int().min(0).max(20).nullable().optional(),
  x: z.number().nullable().optional(),
  y: z.number().nullable().optional(),
  z: z.number().nullable().optional(),
  op: z.boolean().nullable().optional(),
});
...
export const agentConfigSchema = z.object({
  ...
  push_interval_seconds: z.number().int().nullable(),
...
export const agentsContract = c.router({
  ...
  getAgentConfig: {
    method: 'GET',
    path: '/api/servers/:id/agent/config',
    responses: {
