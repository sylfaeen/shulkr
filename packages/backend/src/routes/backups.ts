...
export async function publicBackupRoutes(fastify: FastifyInstance) {
  // GET /api/public/backups/:token Unauthenticated. The token is the only input: it maps to a DB row that holds the internal filename, so no path traversal or enumeration is possible. Every invalid case returns the same generic 404 to avoid an oracle.
  fastify.get(
    '/backups/:token',
