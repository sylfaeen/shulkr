...
import { readCaCertificate } from '@shulkr/backend/services/database_engine_service';
...
// The CA certificate a remote client needs to verify the engine. Public by nature, it carries no secret, but it is served here rather than as a JSON endpoint because the browser downloads it as a file.
export async function databaseCaCertificateRoute(fastify: FastifyInstance) {
  fastify.get(
    '/databases/engine/ca',
    async (
      request: FastifyRequest<{
      ...
      if (!checkPermission(user.permissions, 'server:databases:access')) {
        return reply.status(403).send({
          success: false,
          error: { code: ErrorCodes.AUTH_FORBIDDEN, message: 'Insufficient permissions' },
        });
      ...
      const certificate = await readCaCertificate(getAppDeps());

      if (!certificate) {
        return reply.status(404).send({
          success: false,
          error: { code: ErrorCodes.DATABASE_ENGINE_NOT_INSTALLED, message: 'No certificate found on this machine' },
        });
      }

      return reply
        .header('Content-Disposition', 'attachment; filename="shulkr-ca.pem"')
