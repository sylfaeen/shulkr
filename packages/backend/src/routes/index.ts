...
import { backupRoutes, publicBackupRoutes } from '@shulkr/backend/routes/backups';
import { databaseCaCertificateRoute, databaseDownloadRoute, serverDatabaseDownloadRoute } from '@shulkr/backend/routes/database';
import { avatarRoute } from '@shulkr/backend/routes/avatars';
import { textureRoute } from '@shulkr/backend/routes/textures';
import { publicFileChunkRoute, publicFileRoute } from '@shulkr/backend/routes/public_files';
...
export async function registerRoutes(fastify: FastifyInstance) {
  ...
  await fastify.register(publicBackupRoutes, { prefix: '/share' });
  await fastify.register(databaseDownloadRoute, { prefix: '/api/settings' });
  await fastify.register(serverDatabaseDownloadRoute, { prefix: '/api' });
  await fastify.register(databaseCaCertificateRoute, { prefix: '/api' });
  await fastify.register(avatarRoute, { prefix: '/api/players' });
  await fastify.register(textureRoute, { prefix: '/api/minecraft/texture' });
  await fastify.register(publicFileChunkRoute, { prefix: '/api/public-files' });
  await fastify.register(publicFileRoute, { prefix: '/f' });
}
