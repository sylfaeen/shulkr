...
import { tpsService } from '@shulkr/backend/services/tps_service';
...
notificationService.on('notification:new', ({ userId, unreadCount }: { userId: number; unreadCount: number }) => {
  const sockets = userConnections.get(userId);
  if (!sockets) return;
  const message = JSON.stringify({ type: 'notification:new', unreadCount, timestamp: Date.now() });
...
function startMetricsPolling(serverId: string) {
  ...
  const interval = setInterval(async () => {
    ...
    if (metrics) {
      const tpsData = tpsService.collect(serverId);
...
serverProcessManager.on('server:started', ({ serverId, pid }: { serverId: string; pid: number }) => {
  webhookService.dispatch(serverId, 'server:start').catch(() => {});
  const connections = serverConnections.get(serverId);
...
serverProcessManager.on('server:ready', ({ serverId }: { serverId: string }) => {
  webhookService.dispatch(serverId, 'server:ready').catch(() => {});
  const connections = serverConnections.get(serverId);
  if (!connections) return;
  const message = JSON.stringify({
    type: 'server:ready',
...
serverProcessManager.on(
  ...
  ({ serverId, code, signal }: { serverId: string; code: number | null; signal: string | null }) => {
    ...
    webhookService
      .dispatch(serverId, isCrash ? 'server:crash' : 'server:stop', isCrash ? { error: `Exit code ${code}` } : {})
      .catch(() => {});
    if (isCrash) {
      notificationService
        .broadcast({
      ...
      try {
        ...
        if (message.type === 'console:input' && message.command) {
          ...
          if (!success) {
            ...
            commandSuggestionService.recordCommand(payload.sub, serverId, message.command).catch(() => {});
            auditService.log({
              userId: payload.sub,
      ...
      try {
        ...
        if (message.type === 'channel:subscribe' && message.serverId) {
          ...
          if (!success) {
            ...
            commandSuggestionService.recordCommand(payload.sub, message.serverId, message.command).catch(() => {});
            auditService.log({
              userId: payload.sub,
              username: payload.username,
              action: 'console_command',
