...
import { getServerMetrics, invalidateMetricsCache } from '@shulkr/backend/services/metrics_service';
...
import { broadcastNotification, notificationService } from '@shulkr/backend/services/notification_service';
import { collectTps } from '@shulkr/backend/services/tps_service';
import { recordCommand } from '@shulkr/backend/services/command_suggestion_service';
...
function addUserConnection(userId: number, socket: WebSocket) {
  if (!userConnections.has(userId)) userConnections.set(userId, new Set());
  userConnections.get(userId)!.add(socket);
}

function removeUserConnection(userId: number, socket: WebSocket) {
...
notificationService.on('notification:new', ({ userId, unreadCount }: { userId: number; unreadCount: number }) => {
  const sockets = userConnections.get(userId);
  if (!sockets) return;
  const message = JSON.stringify({ type: 'notification:new', unreadCount, timestamp: getAppDeps().clock().getTime() });
...
function startMetricsPolling(serverId: string) {
  ...
  const interval = setInterval(async () => {
    ...
    if (metrics) {
      ...
      if (tpsData.tps !== null) metrics.tps = tpsData.tps;
      if (tpsData.mspt !== null) metrics.mspt = tpsData.mspt;
    }
    ...
    if (!metrics) {
      const message = JSON.stringify({
        ...
        server_id: serverId,
        metrics: null,
        timestamp: getAppDeps().clock().getTime(),
      });
...
serverProcessManager.on('server:started', ({ serverId, pid }: { serverId: string; pid: number }) => {
  dispatchWebhooks(getAppDeps(), serverId, 'server:start').catch(() => {});
  const connections = serverConnections.get(serverId);
...
serverProcessManager.on('server:ready', ({ serverId }: { serverId: string }) => {
  dispatchWebhooks(getAppDeps(), serverId, 'server:ready').catch(() => {});
  const connections = serverConnections.get(serverId);
  if (!connections) return;
...
serverProcessManager.on(
  ...
  ({ serverId, code, signal }: { serverId: string; code: number | null; signal: string | null }) => {
    ...
    dispatchWebhooks(
      getAppDeps(),
      serverId,
      isCrash ? 'server:crash' : 'server:stop',
      isCrash ? { error: `Exit code ${code}` } : {}
    ).catch(() => {});

    if (isCrash) {
      broadcastNotification(getAppDeps(), {
        type: 'server_crash',
        title: `Server crashed`,
        message: `Server ${serverId} crashed with exit code ${code}`,
        link: `/app/servers/${serverId}`,
      }).catch(() => {});
    }

    playersService.clearPlayers(serverId);
    invalidateMetricsCache(serverId);
    const connections = serverConnections.get(serverId);
    if (!connections) return;
...
export async function registerWebSocketRoutes(fastify: FastifyInstance) {
  // WebSocket route for console streaming Connect with: ws://localhost:3001/ws/console?serverId=<id> Auth via Sec-WebSocket-Protocol: access_token, <jwt>
  fastify.get('/ws/console', { websocket: true }, async (socket: WebSocket, request) => {
    ...
    const [dbUser] = await getAppDeps()
      .db.select({ token_version: users.token_version })
      .from(users)
    ...
    if (!hasPermission(payload.permissions, 'server:console:read')) {
    ...
    socket.on('message', (rawMessage: Buffer | string) => {
      const now = getAppDeps().clock().getTime();
      ...
      try {
        ...
        if (message.type === 'console:input' && message.command) {
          ...
          if (!success) {
            ...
            recordCommand(getAppDeps(), payload.sub, serverId, message.command).catch(() => {});
          ...
          getServerMetrics(getAppDeps(), serverId).then((metrics) => {
            socket.send(
              JSON.stringify({
                type: 'metrics:update',
                server_id: serverId,
          ...
          socket.send(JSON.stringify({ type: 'pong', timestamp: getAppDeps().clock().getTime() }));
        }
      } catch {
  ...
  // Multiplexed WebSocket: single connection, multiple server channels Connect with: ws://localhost:3001/ws Auth via Sec-WebSocket-Protocol: access_token, <jwt>
  fastify.get('/ws', { websocket: true }, async (socket: WebSocket, request) => {
    ...
    socket.send(JSON.stringify({ type: 'connected', user: payload.username, timestamp: getAppDeps().clock().getTime() }));
    addUserConnection(payload.sub, socket);
    ...
    socket.on('message', (rawMessage: Buffer | string) => {
      ...
      try {
        ...
        if (message.type === 'channel:subscribe' && message.serverId) {
          ...
          getServerMetrics(getAppDeps(), serverId).then((metrics) => {
            socket.send(
              JSON.stringify({ type: 'metrics:update', server_id: serverId, metrics, timestamp: getAppDeps().clock().getTime() })
            );
          });

          socket.send(JSON.stringify({ type: 'channel:subscribed', serverId, timestamp: getAppDeps().clock().getTime() }));
        } else if (message.type === 'channel:unsubscribe' && message.serverId) {
          const serverId = message.serverId;
          subscribedServers.delete(serverId);
          removeConnection(serverId, socket);
          removeMetricsSubscription(serverId, socket);
          socket.send(JSON.stringify({ type: 'channel:unsubscribed', serverId, timestamp: getAppDeps().clock().getTime() }));
          ...
          if (!success) {
            ...
            recordCommand(getAppDeps(), payload.sub, message.serverId, message.command).catch(() => {});
            ...
            socket.send(
              JSON.stringify({ type: 'console:input:ack', command: message.command, timestamp: getAppDeps().clock().getTime() })
            );
          }
    ...
    socket.on('close', () => {
      removeUserConnection(payload.sub, socket);
