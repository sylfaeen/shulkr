...
// Inserts a user with the given permissions and signs a JWT against the Fastify instance's @fastify/jwt so the token verifies cleanly. Returns pre-built `headers` ready to drop into `app.inject({ headers })`.
