...
export interface FakeFsAdapter extends FsAdapter {
  ...
  reset(): void;
  // Override what statfs reports, until the next reset.
  setStatfs(value: FsStatfs): void;
}

const DEFAULT_STATFS: FsStatfs = { bsize: 4096, blocks: 1_000_000, bfree: 500_000, bavail: 500_000 };

export function createFakeFs(): FakeFsAdapter {
  const files = new Map<string, Buffer>();
  const dirs = new Set<string>(['/']);
  let statfsValue = DEFAULT_STATFS;
  ...
  return {
    ...
    writeAt(path, data, position) {
      const p = normalize(path);
      const existing = files.get(p);
      if (!existing) throw Object.assign(new Error(`ENOENT: no such file '${path}'`), { code: 'ENOENT' });
      const next = Buffer.alloc(Math.max(existing.length, position + data.length));
      existing.copy(next);
    ...
    statfs(_path): Promise<FsStatfs> {
      return Promise.resolve(statfsValue);
    },
