...
import { syncGlobalIpBans } from '@shulkr/backend/services/global_ip_ban_service';
...
const start = async () => {
  try {
    ...
    if (config.NODE_ENV === 'production') {
      ...
      const banSync = await syncGlobalIpBans(getAppDeps());
      if (banSync.synced > 0) {
        console.log(`Global IP bans: synced ${banSync.synced}/${banSync.total} bans to firewall`);
      }
    }
