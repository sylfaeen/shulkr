...
import { initializeDatabaseMaintenance } from '@shulkr/backend/services/database_engine_service';
import { refreshAccessActivity } from '@shulkr/backend/services/database_access_service';
import { initializePublicFileMaintenance } from '@shulkr/backend/services/public_file_service';
...
const start = async () => {
  try {
    ...
    initializeDatabaseMaintenance(getAppDeps(), refreshAccessActivity);
    initializePublicFileMaintenance(getAppDeps());
    ...
    app.setNotFoundHandler(async (request, reply) => {
      // /f/ is excluded so that any path under the public share route, whatever its shape, gets the same 404 as an invalid token.
      if (
        request.url.startsWith('/api/') ||
        request.url.startsWith('/ws/') ||
        request.url.startsWith('/docs/') ||
        request.url.startsWith('/f/')
      ) {
