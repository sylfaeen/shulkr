...
import { contract, ErrorCodes, type WebhookEvent, type WebhookLanguage } from '@shulkr/shared';
import { eq, and, desc, count } from 'drizzle-orm';
import { webhooks, webhookDeliveries } from '@shulkr/backend/db/schema';
import { sendTestWebhook } from '@shulkr/backend/services/webhook_service';
...
function formatWebhook(wh: typeof webhooks.$inferSelect, reveal = false) {
  return {
    ...
    language: wh.language as WebhookLanguage,
    events: JSON.parse(wh.events) as Array<WebhookEvent>,
    messageTemplates: parseMessageTemplates(wh.message_templates),
    enabled: wh.enabled,
...
export const webhooksRoutes = s.router(contract.webhooks, {
  get: async ({ request, params }) => {
    try {
      ...
      const [existing] = await getAppDeps()
        .db.select()
        .from(webhooks)
  ...
  list: async ({ request, params }) => {
    try {
      ...
      const result = await getAppDeps().db.select().from(webhooks).where(eq(webhooks.server_id, params.serverId));
  ...
  create: async ({ request, params, body }) => {
    try {
      ...
      const [created] = await getAppDeps()
        .db.insert(webhooks)
        .values({
          ...
          message_templates: body.messageTemplates ? JSON.stringify(body.messageTemplates) : null,
        })
        .returning();
  ...
  update: async ({ request, params, body }) => {
    try {
      ...
      const updateData: Record<string, unknown> = { updated_at: getAppDeps().clock().toISOString() };
      ...
      if (body.messageTemplates !== undefined)
        updateData.message_templates = body.messageTemplates ? JSON.stringify(body.messageTemplates) : null;
      if (body.enabled !== undefined) updateData.enabled = body.enabled;

      const [updated] = await getAppDeps().db.update(webhooks).set(updateData).where(eq(webhooks.id, webhookId)).returning();
  ...
  delete: async ({ request, params }) => {
    try {
      ...
      await getAppDeps().db.delete(webhooks).where(eq(webhooks.id, webhookId));
  ...
  templates: async ({ request, query }) => {
    try {
      await authenticate(request);

      const templates = loadWebhookTemplates(query.language as WebhookLanguage);
  ...
  test: async ({ request, params }) => {
    try {
      ...
      const result = await sendTestWebhook(getAppDeps(), webhookId);
  ...
  deliveries: async ({ request, params, query }) => {
    try {
      ...
      const [totalResult] = await getAppDeps()
        .db.select({ value: count() })
        .from(webhookDeliveries)
        .where(eq(webhookDeliveries.webhook_id, webhookId));
      const rows = await getAppDeps()
        .db.select({
  ...
  deliveryDetail: async ({ request, params }) => {
    try {
      ...
      const [delivery] = await getAppDeps()
        .db.select()
        .from(webhookDeliveries)
