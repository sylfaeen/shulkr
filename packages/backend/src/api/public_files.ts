...
async function resolveBaseUrl(deps: AppDeps, request: FastifyRequest): Promise<string | null> {
  const domain = await getPanelDomain(deps);
  if (!domain) return null;

  // ssl_enabled can lag behind nginx, for instance after a certificate installed from the command line. A request that reached the panel over HTTPS proves HTTPS works, so the link is never downgraded below what the administrator already uses: browsers hide an http link on HSTS-preloaded TLDs, but clients such as Minecraft do not follow the http to https redirect and fail. request.protocol only honours X-Forwarded-Proto from a trusted proxy (TRUST_PROXY, the local nginx by default).
  const secure = domain.ssl_enabled || request.protocol === 'https';

  return `${secure ? 'https' : 'http'}://${domain.domain}`;
}
      ...
      try {
        ...
        return {
          ...
          body: {
            ...
            baseUrl: await resolveBaseUrl(deps, request),
