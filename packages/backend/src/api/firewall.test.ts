...
// Story 59.6 ACs 8/9/11 adapted: the actual contract is a global panel firewall (/api/firewall, not per-server). Tests assert the equivalent behaviour on the real endpoint.
describe('firewall routes (story 59.6)', () => {
  ...
  it('AC8: POST /api/firewall with settings:firewall:add inserts the rule and calls the firewall script with allow', async () => {
    ...
    const res = await testApp.app.inject({
      ...
      payload: { port: 25565, protocol: 'tcp', label: 'Minecraft default' },
    });
    expect(res.statusCode).toBe(201);
    const body = res.json() as { id: number; port: number; protocol: string; label: string };
  ...
  it('AC8 bis: reserved port (22) is rejected by Zod with 400 before any shell call', async () => {
    ...
    const res = await testApp.app.inject({
      ...
      url: '/api/firewall',
      headers: admin.headers,
      payload: { port: 22, protocol: 'tcp', label: 'SSH' },
    });
  ...
  it('AC9: DELETE /api/firewall/:ruleId removes the row and calls deny', async () => {
    const admin = await seedAuthenticatedUser(testApp.app, testApp.deps, {
    ...
    const addRes = await testApp.app.inject({
      ...
      url: '/api/firewall',
      headers: admin.headers,
      payload: { port: 26000, protocol: 'tcp', label: 'Test' },
    });
  ...
  it('AC11: user without settings:firewall:add receives 403 and no shell call', async () => {
    ...
    const res = await testApp.app.inject({
      ...
      payload: { port: 26500, protocol: 'tcp', label: 'Nope' },
    });
  ...
  it('toggle: POST /api/firewall/:ruleId/toggle flips enabled and calls the matching script action', async () => {
    ...
    const addRes = await testApp.app.inject({
      ...
      url: '/api/firewall',
      headers: admin.headers,
      payload: { port: 27000, protocol: 'udp', label: 'UDP test' },
    });
  ...
  it('list: GET /api/firewall returns all rules for an authorised user', async () => {
    ...
    for (const port of [28001, 28002, 28003]) {
      await testApp.app.inject({
        ...
        url: '/api/firewall',
        headers: admin.headers,
        payload: { port, protocol: 'tcp', label: `Rule ${port}` },
      });
    }
    ...
    const body = res.json() as { rules: Array<{ port: number }> };
