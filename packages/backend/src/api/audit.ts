...
export const auditRoutes = s.router(contract.audit, {
  list: async ({ request, query }) => {
    try {
      ...
      const serverIds = [
        ...new Set(result.logs.filter((log) => log.resource_type === 'server' && log.resource_id).map((log) => log.resource_id!)),
      ];
      ...
      const entries = result.logs.map((log) => ({
        ...
        resourceName: log.resource_type === 'server' && log.resource_id ? (serverNames.get(log.resource_id) ?? null) : null,
