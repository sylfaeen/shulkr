...
import {
  ...
  getPlayerSessions,
  searchPlayers,
} from '@shulkr/backend/services/player_profile_service';
...
export const playerProfileRoutes = s.router(contract.playerProfile, {
  profile: async ({ request, params }) => {
    try {
      ...
      const profile = await getPlayerProfile(getAppDeps(), params.serverId, params.playerName);
      if (!profile) {
  ...
  sessions: async ({ request, params, query }) => {
    try {
      ...
      const result = await getPlayerSessions(
        getAppDeps(),
        params.serverId,
        params.playerName,
        query.limit ?? 50,
  ...
  moderation: async ({ request, params }) => {
    try {
      ...
      const result = await getPlayerModeration(getAppDeps(), params.serverId, params.playerName);
  ...
  search: async ({ request, params, query }) => {
    try {
      ...
      const results = await searchPlayers(getAppDeps(), params.serverId, query.q);
