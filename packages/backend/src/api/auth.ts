...
function checkLoginLockout(ip: string): void {
  ...
  if (entry && entry.count >= LOCKOUT_MAX_ATTEMPTS && Date.now() - entry.firstAt < LOCKOUT_WINDOW_MS) {
    const retryAfterSeconds = Math.ceil((LOCKOUT_WINDOW_MS - (Date.now() - entry.firstAt)) / 1000);
    console.warn(`Login lockout triggered for IP ${ip} (${entry.count} failed attempts)`);
    throw {
      status: 429 as const,
...
function recordLoginFailure(ip: string): void {
  ...
  if (!entry || Date.now() - entry.firstAt >= LOCKOUT_WINDOW_MS) {
    failedLoginAttempts.set(ip, { count: 1, firstAt: Date.now() });
  } else {
    entry.count++;
  }
}
...
export const authRoutes = s.router(contract.auth, {
  ...
  verifyPassword: async ({ request, body }) => {
    try {
      const user = await authenticate(request);
      checkRateLimit(`user:${user.sub}:verify-password`, 10, 60_000);

      const authService = new AuthService(request.server);
      const result = await authService.validateCredentials(user.username, body.password);

      if (!result.valid) {
        return { status: 403 as const, body: { code: ErrorCodes.AUTH_INVALID_PASSWORD, message: ErrorCodes.AUTH_INVALID_PASSWORD } };
      }
