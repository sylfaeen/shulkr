...
import { authenticate, checkRateLimit, isMiddlewareError } from '@shulkr/backend/api/middleware';
import {
  checkLoginLockout as dbCheckLoginLockout,
  recordLoginFailure,
  clearLoginFailures,
...
function checkLoginLockout(ip: string): void {
  dbCheckLoginLockout(`lockout:${ip}`, LOCKOUT_MAX_ATTEMPTS, LOCKOUT_WINDOW_MS);
}

function recordFailure(ip: string): void {
  recordLoginFailure(`lockout:${ip}`, LOCKOUT_WINDOW_MS);
}

function clearFailures(ip: string): void {
  clearLoginFailures(`lockout:${ip}`);
}
