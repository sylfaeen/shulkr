...
import { getAgentLive, ingestAgentPayload, queryAgentHistory } from '@shulkr/backend/services/agent_ingest_service';
...
async function serverExists(id: string): Promise<boolean> {
  const [row] = await getAppDeps().db.select({ id: servers.id }).from(servers).where(eq(servers.id, id)).limit(1);
  return !!row;
}
...
async function buildStatus(serverId: string): Promise<AgentStatus> {
  ...
  const expected = await getExpectedPluginVersion(platform ?? 'paper');
  return {
    ...
    version_mismatch: await hasVersionMismatch(row?.plugin_version ?? null, platform),
...
export const agentsRoutes = s.router(contract.agents, {
  ingestMetrics: async ({ request, params, body }) => {
    try {
      ...
      await ingestAgentPayload(getAppDeps(), params.serverId, body);
  ...
  updateAgent: async ({ request, params, body }) => {
    try {
      ...
      // Update = reinstall with fresh jar but reuse existing token (installAgent regenerates token, so for an update we skip the token rotation by copying the jar only if a token already exists).
  ...
  getAgentLive: async ({ request, params }) => {
    try {
      ...
      const live = getAgentLive(getAppDeps(), params.id);
      if (!live) {
  ...
  getAgentHistory: async ({ request, params, query }) => {
    try {
      ...
      const points = queryAgentHistory(getAppDeps(), params.id, period);
