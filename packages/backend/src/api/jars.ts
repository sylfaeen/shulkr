...
import { downloadPaperMCJar, getPaperMCBuilds, getPaperMCVersions } from '@shulkr/backend/services/papermc_service';
import { getServerById, updateServer } from '@shulkr/backend/services/server_service';
import { deleteFile, getFileInfo, listDirectory } from '@shulkr/backend/services/file_service';
...
export const jarsRoutes = s.router(contract.jars, {
  getVersions: async ({ request, query }) => {
    try {
      ...
      const versions = await getPaperMCVersions(query.project);
  ...
  getBuilds: async ({ request, query }) => {
    try {
      ...
      const builds = await getPaperMCBuilds(query.project, query.version);
  ...
  download: async ({ request, params, body }) => {
    try {
      ...
      const result = await downloadPaperMCJar(getAppDeps(), body.project, body.version, targetBuild, server.path, (progress) => {
  ...
  list: async ({ request, params }) => {
    try {
      ...
      const result = await listDirectory(getAppDeps(), server.path, '/');
  ...
  setActive: async ({ request, params, body }) => {
    try {
      ...
      const fileInfo = await getFileInfo(getAppDeps(), server.path, body.jarFile);

      if (!fileInfo.success) {
        return { status: 200 as const, body: { jarFile: '' } };
      }

      await updateServer(getAppDeps(), serverId, {
  ...
  delete: async ({ request, params }) => {
    try {
      ...
      const fileInfo = await getFileInfo(getAppDeps(), server.path, jarFile);

      if (!fileInfo.success) {
        return { status: 200 as const, body: { deleted: '' } };
      }

      const result = await deleteFile(getAppDeps(), server.path, jarFile);
