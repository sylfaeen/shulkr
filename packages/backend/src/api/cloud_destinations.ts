...
export const cloudDestinationsRoutes = s.router(contract.cloudDestinations, {
  list: async ({ request }) => {
    try {
      ...
      const rows = await getAppDeps().db.select().from(cloudDestinations);
  ...
  get: async ({ request, params }) => {
    try {
      ...
      const [row] = await getAppDeps().db.select().from(cloudDestinations).where(eq(cloudDestinations.id, params.id)).limit(1);
  ...
  create: async ({ request, body }) => {
    try {
      ...
      const [row] = await getAppDeps()
        .db.insert(cloudDestinations)
        .values({
          name: body.name,
  ...
  update: async ({ request, params, body }) => {
    try {
      ...
      const [existing] = await getAppDeps()
        ...
        .where(eq(cloudDestinations.id, params.id))
        .limit(1);
  ...
  delete: async ({ request, params }) => {
    try {
      ...
      const usedBy = await getAppDeps()
        .db.select()
        .from(backupMetadata)
        .where(eq(backupMetadata.cloud_destination_id, params.id))
        .limit(1);
      ...
      await getAppDeps().db.delete(cloudDestinations).where(eq(cloudDestinations.id, params.id));
