...
export const marketplaceRoutes = s.router(contract.marketplace, {
  search: async ({ request, query }) => {
    try {
      ...
      if (query.source === 'hangar') {
        const result = await searchHangarProjects({ query: query.q, limit: query.limit, offset: query.offset });
        return {
          status: 200 as const,
  ...
  project: async ({ request, params, query }) => {
    try {
      ...
      if (query.source === 'hangar') {
        const p = await getHangarProject(params.idOrSlug);
        return {
          status: 200 as const,
          body: {
            id: p.namespace.slug,
      ...
      const project = await getModrinthProject(params.idOrSlug);
  ...
  versions: async ({ request, params, query }) => {
    try {
      ...
      if (query.source === 'hangar') {
        const { versions: hangarVersions } = await getHangarVersions(params.idOrSlug);
      ...
      const versions = await getModrinthVersions(params.idOrSlug, {
        gameVersions: query.gameVersion ? [query.gameVersion] : undefined,
        loaders: query.loader ? [query.loader] : undefined,
      });
  ...
  updates: async ({ request, params }) => {
    try {
      ...
      const server = await getServerById(getAppDeps(), params.serverId);
      if (!server) {
      ...
      try {
        files = await getAppDeps().fs.readdir(pluginsDir);
      } catch {
      ...
      for (const metaFile of metaFiles) {
        try {
          const content = await getAppDeps().fs.readFileText(path.join(pluginsDir, metaFile), 'utf-8');
          ...
          if (metaSource === 'modrinth') {
            const versions = await getModrinthVersions(meta.projectId);
            ...
            const { versions } = await getHangarVersions(meta.projectId, { limit: 1 });
            if (versions.length === 0) continue;
            const latest = versions[0];
  ...
  install: async ({ request, params, body }) => {
    try {
      ...
      if (body.source === 'hangar') {
        result = await downloadHangarPlugin(getAppDeps(), body.projectId, body.versionId, 'PAPER', pluginsDir);
      } else {
        result = await downloadModrinthPlugin(getAppDeps(), body.fileUrl, pluginsDir, body.filename, body.fileHash);
      }

      // Write .marketplace metadata file
      const metaPath = path.join(pluginsDir, `${result.filename}.marketplace`);
      await getAppDeps().fs.writeFile(
        metaPath,
        JSON.stringify({
          source: body.source,
          projectId: body.projectId,
          versionId: body.versionId,
          installedAt: getAppDeps().clock().toISOString(),
        })
      );
