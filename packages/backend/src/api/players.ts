...
import { queryPlayerHistory } from '@shulkr/backend/services/player_history_service';
...
async function getServerPath(serverId: string): Promise<string | null> {
  const [server] = await getAppDeps().db.select({ path: servers.path }).from(servers).where(eq(servers.id, serverId)).limit(1);
...
async function readWhitelistFile(serverPath: string): Promise<Array<{ uuid: string; name: string }>> {
  ...
  if (!(await getAppDeps().fs.exists(filePath))) return [];

  try {
    const content = await getAppDeps().fs.readFileText(filePath);
...
async function readServerProperty(serverPath: string, key: string): Promise<string | null> {
  ...
  if (!(await getAppDeps().fs.exists(filePath))) return null;
...
export const playersRoutes = s.router(contract.players, {
  history: async ({ request, params, query }) => {
    try {
      ...
      const result = queryPlayerHistory(getAppDeps(), params.serverId, limit, offset);
    ...
    try {
      ...
      if (status.status === 'running') {
        ...
        if (!entries.some((e) => e.name.toLowerCase() === body.name.toLowerCase())) {
          ...
          await getAppDeps().fs.writeFile(path.join(serverPath, 'whitelist.json'), JSON.stringify(entries, null, 2));
        }
      }
  ...
  whitelistRemove: async ({ request, params }) => {
    try {
      ...
      if (status.status === 'running') {
        ...
        await getAppDeps().fs.writeFile(path.join(serverPath, 'whitelist.json'), JSON.stringify(filtered, null, 2));
      }
  ...
  pardon: async ({ request, params }) => {
    try {
      ...
      if (status.status === 'running') {
        ...
        const entries = await readBannedPlayersFile(serverPath);
        const filtered = entries.filter((e) => e.name.toLowerCase() !== playerName.toLowerCase());
        await getAppDeps().fs.writeFile(path.join(serverPath, 'banned-players.json'), JSON.stringify(filtered, null, 2));
      }
  ...
  pardonIp: async ({ request, params }) => {
    try {
      ...
      if (status.status === 'running') {
        ...
        await getAppDeps().fs.writeFile(path.join(serverPath, 'banned-ips.json'), JSON.stringify(filtered, null, 2));
      }
