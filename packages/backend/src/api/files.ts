...
import {
  ...
  renameFile,
  writeFile,
} from '@shulkr/backend/services/file_service';
...
export const filesRoutes = s.router(contract.files, {
  list: async ({ request, params, query }) => {
    try {
      ...
      assertPermissions(user, 'server:files:read:list');

      const server = await getServerOrThrow(params.serverId);
      const result = await listDirectory(getAppDeps(), server.path, query.path || '/');
  ...
  directorySizes: async ({ request, params, query }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'server:files:read:list');

      const server = await getServerOrThrow(params.serverId);
      const result = await getDirectorySizes(getAppDeps(), server.path, query.path || '/');
  ...
  read: async ({ request, params, query }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'server:files:read:content');

      const server = await getServerOrThrow(params.serverId);
      const result = await readFile(getAppDeps(), server.path, query.path);
  ...
  write: async ({ request, params, body }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'server:files:write:edit');
      checkRateLimit(`user:${user.sub}:files.write`, 30, ONE_MINUTE);

      const server = await getServerOrThrow(params.serverId);
      const result = await writeFile(getAppDeps(), server.path, body.path, body.content);
  ...
  delete: async ({ request, params, query }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'server:files:write:delete');
      checkRateLimit(`user:${user.sub}:files.delete`, 30, ONE_MINUTE);

      const server = await getServerOrThrow(params.serverId);
      const result = await deleteFile(getAppDeps(), server.path, query.path);
  ...
  mkdir: async ({ request, params, body }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'server:files:write:mkdir');
      checkRateLimit(`user:${user.sub}:files.mkdir`, 30, ONE_MINUTE);

      const server = await getServerOrThrow(params.serverId);
      const result = await createDirectory(getAppDeps(), server.path, body.path);
  ...
  rename: async ({ request, params, body }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'server:files:write:rename');
      checkRateLimit(`user:${user.sub}:files.rename`, 30, ONE_MINUTE);

      const server = await getServerOrThrow(params.serverId);
      const result = await renameFile(getAppDeps(), server.path, body.oldPath, body.newPath);
  ...
  info: async ({ request, params, query }) => {
    try {
      ...
      const server = await getServerOrThrow(params.serverId);
      const result = await getFileInfo(getAppDeps(), server.path, query.path);
