...
export const domainsRoutes = s.router(contract.domains, {
  ...
  setPanelDomain: async ({ request, body }) => {
    try {
      ...
      if (error instanceof DomainDnsMismatchError) {
        return {
          status: 400 as const,
          body: {
            code: error.code,
      ...
      if (error instanceof DomainSslError) {
        return { status: 422 as const, body: { code: error.code, message: error.message, detail: error.detail } };
      }
