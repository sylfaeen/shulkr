...
} from '@shulkr/backend/services/notification_service';
...
export const notificationsRoutes = s.router(contract.notifications, {
  list: async ({ request, query }) => {
    try {
      ...
      const result = await listNotifications(getAppDeps(), user.sub, limit, offset);
  ...
  unreadCount: async ({ request }) => {
    try {
      const user = await authenticate(request);
      const count = await getUnreadNotificationCount(getAppDeps(), user.sub);
  ...
  markRead: async ({ request, params }) => {
    try {
      const user = await authenticate(request);
      await markNotificationRead(getAppDeps(), Number(params.notificationId), user.sub);
  ...
  markAllRead: async ({ request }) => {
    try {
      const user = await authenticate(request);
      await markAllNotificationsRead(getAppDeps(), user.sub);
