...
export const alertsRoutes = s.router(contract.alerts, {
  list: async ({ request, params }) => {
    try {
      ...
      const rules = await getAppDeps().db.select().from(alertRules).where(eq(alertRules.server_id, params.serverId));
  ...
  update: async ({ request, params, body }) => {
    try {
      ...
      const [updated] = await getAppDeps().db.update(alertRules).set(updateData).where(eq(alertRules.id, alertId)).returning();
  ...
  delete: async ({ request, params }) => {
    try {
      ...
      await getAppDeps().db.delete(alertRules).where(eq(alertRules.id, alertId));
