...
export const logsRoutes = s.router(contract.logs, {
  ...
  merge: async ({ request, params, body }) => {
    try {
      ...
      // Validate all filenames: reject path traversal attempts
      for (const raw of body.filenames) {
        if (raw !== path.basename(raw)) {
          return {
            status: 400 as const,
            body: { code: 'INVALID_FILENAME', message: 'Invalid filename' },
          };
        }
      }

      const filenames = body.filenames.map((f) => path.basename(f));
      ...
      for (const filename of filenames) {
        ...
        if (!fs.existsSync(path.join(logsDir, filename))) {
          return {
            status: 404 as const,
            body: { code: 'FILE_NOT_FOUND', message: 'Log file not found' },
          };
        }
      }

      // Sort filenames chronologically (filenames are date-based: YYYY-MM-DD-N.log.gz)
      const sorted = [...filenames].sort();

      // Decompress and concatenate all files
      ...
      const mergedFilename = `merged-${Date.now()}.log.gz`;
      const compressedMerged = await gzip(Buffer.from(merged, 'utf-8'));
      fs.writeFileSync(path.join(logsDir, mergedFilename), compressedMerged);
      ...
      await auditService.log({
        ...
        resourceType: 'log',
        details: { sources: filenames, result: mergedFilename, serverId: params.serverId },
        ip: request.ip,
      });

      return {
        status: 200 as const,
        body: { filename: mergedFilename, message: 'Log archives merged successfully' },
      };
    } catch (error: unknown) {
      if (isMiddlewareError(error)) return error;
      throw error;
