...
type ValidationError =
  { status: 400; body: { code: string; message: string } } | { status: 404; body: { code: string; message: string } };

async function validateLogFilenames(
  rawFilenames: Array<string>,
  logsDir: string
): Promise<{ filenames: Array<string>; error?: never } | { filenames?: never; error: ValidationError }> {
  ...
  for (const filename of filenames) {
    ...
    if (!(await fs.exists(path.join(logsDir, filename)))) {
...
export const logsRoutes = s.router(contract.logs, {
  list: async ({ request, params }) => {
    try {
      ...
      if (await fs.exists(latestPath)) {
        ...
        files.push({ filename: 'latest.log', size: stat.size, modified: new Date(stat.mtimeMs).toISOString(), isLatest: true });
      }
      ...
      if (await fs.exists(logsDir)) {
        ...
        for (const entry of entries) {
          ...
          files.push({ filename: entry, size: stat.size, modified: new Date(stat.mtimeMs).toISOString(), isLatest: false });
        }
      }
  ...
  read: async ({ request, params, query }) => {
    try {
      ...
      if (filename.endsWith('.log.gz')) {
        ...
        content = await fs.readFileText(filePath, 'utf-8');
      }
  ...
  merge: async ({ request, params, body }) => {
    try {
      ...
      const server = await getServerOrThrow(params.serverId);
      const logsDir = path.join(server.path, 'logs');
      const validated = await validateLogFilenames(body.filenames, logsDir);
      ...
      for (const filename of sorted) {
        const compressed = await fs.readFile(path.join(logsDir, filename));
        const decompressed = await gunzip(compressed);
        const dateMatch = filename.match(/^(\d{4}-\d{2}-\d{2})/);
        const sourceDate = dateMatch ? dateMatch[1] : null;

        for (const line of decompressed.toString('utf-8').split('\n')) {
          if (!line) {
            allLines.push(line);
            continue;
          }

          if (/^\[\d{4}-\d{2}-\d{2}\] /.test(line) || !sourceDate) {
            allLines.push(line);
          } else {
            allLines.push(`[${sourceDate}] ${line}`);
          }
        }
      }

      const timestamp = getAppDeps().clock().getTime();
      ...
      if (shouldSplit) {
        ...
        for (let i = 0; i < allLines.length; i += MERGE_MAX_LINES) {
          ...
          await fs.writeFile(path.join(logsDir, partFilename), compressed);
        ...
        await fs.writeFile(path.join(logsDir, mergedFilename), compressed);
