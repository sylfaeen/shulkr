...
// Story 59.5 ACs adapted to the actual contract. The route is /api/sftp (global), not per-server. Username is passed as-is to subs_sftp.sh; the script handles the actual chroot/sshd_match plumbing, so AC2 (server-prefixed username) and AC3 (sshd_match config writes) are not observable from the backend layer and are documented as gaps. AC4 (duplicate username same server) and AC5 (reuse across servers) covered by story 59.8.
