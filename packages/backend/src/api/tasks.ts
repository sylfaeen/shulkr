...
export const tasksRoutes = s.router(contract.tasks, {
  list: async ({ request, params }) => {
    try {
      ...
      const tasks = await getAppDeps().db.select().from(scheduledTasks).where(eq(scheduledTasks.server_id, serverId));

      return { status: 200 as const, body: { tasks: tasks.map(formatTask), serverTime: getAppDeps().clock().toISOString() } };
    } catch (error: unknown) {
  ...
  create: async ({ request, params, body }) => {
    try {
      ...
      const [newTask] = await getAppDeps()
  ...
  update: async ({ request, params, body }) => {
    try {
      ...
      const [existingTask] = await getAppDeps()
        .db.select()
        .from(scheduledTasks)
      ...
      const updateData: Record<string, unknown> = {
        updated_at: getAppDeps().clock().toISOString(),
      };
      ...
      if (isChainUpdate) {
        ...
        const hasWarnChanges = body.warnPlayers !== undefined || body.warnMessage !== undefined || body.warnSeconds !== undefined;
      ...
      const [updatedTask] = await getAppDeps()
  ...
  delete: async ({ request, params }) => {
    try {
      ...
      await taskScheduler.unscheduleTask(taskId);
      await getAppDeps().db.delete(scheduledTasks).where(eq(scheduledTasks.id, taskId));
  ...
  run: async ({ request, params }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'server:tasks:run');
      checkRateLimit(`user:${user.sub}:tasks.run`, 10, ONE_MINUTE);
      ...
      await enqueueJob(getAppDeps(), existingTask);
      ...
      return { status: 200 as const, body: { message: 'Task enqueued' } };
    } catch (error: unknown) {
      if (isMiddlewareError(error)) return error;
      throw error;
    }
  ...
  history: async ({ request, params, query }) => {
    try {
      ...
      const executions = await getAppDeps()
        .db.select()
        .from(taskExecutions)
        .where(eq(taskExecutions.task_id, taskId))
