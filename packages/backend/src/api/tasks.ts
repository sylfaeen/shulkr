        ...
        updateData.config =
          newType === 'restart'
            ...
            : newType === 'backup'
              ? { backup_paths: body.command.replace('backup ', '').split(' ').filter(Boolean) }
              : { command: body.command };
      } else if (body.warnPlayers !== undefined || body.warnMessage !== undefined || body.warnSeconds !== undefined) {
        const existingConfig = (existingTask.config || {}) as TaskConfig;
        updateData.config = {
          ...existingConfig,
          ...(body.warnPlayers !== undefined && { warn_players: body.warnPlayers }),
          ...(body.warnMessage !== undefined && { warn_message: body.warnMessage }),
          ...(body.warnSeconds !== undefined && { warn_seconds: body.warnSeconds }),
        };
      }
