...
async function readModrinthMeta(pluginsDir: string, filename: string): Promise<ModrinthMeta | null> {
  try {
    const metaPath = path.join(pluginsDir, `${filename}.modrinth`);
    const content = await fs.readFile(metaPath, 'utf-8');
    return JSON.parse(content) as ModrinthMeta;
  } catch {
    return null;
  }
}
    ...
    try {
      ...
      const plugins = await Promise.all(
        jarFiles.map(async (file) => {
          ...
          const modrinth = await readModrinthMeta(pluginsDir, file.name);
