...
import {
  ...
  readTableSchema,
  searchAcrossTables,
} from '@shulkr/backend/services/database_query_service';
...
function mapScriptError(error: unknown) {
  ...
  if (error.message.startsWith(ErrorCodes.DATABASE_QUERY_FAILED)) {
    return {
      status: 400 as const,
      body: {
        code: ErrorCodes.DATABASE_QUERY_FAILED,
        message: error.message.replace(`${ErrorCodes.DATABASE_QUERY_FAILED}: `, ''),
      },
    };
  }

  if (error.message === ErrorCodes.DATABASE_ACCESS_NO_CERTIFICATE) {
    return {
      status: 400 as const,
      body: { code: error.message, message: 'This access does not require a client certificate' },
    };
  }
...
export const databasesRoutes = s.router(contract.databases, {
  ...
  tables: async ({ request, params }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'server:databases:browse');

      const deps = getAppDeps();
      const row = await getServerDatabase(deps, params.id);
      if (!row) return notFound;

      return { status: 200 as const, body: { tables: await listDatabaseTables(deps, row) } };
    } catch (error: unknown) {
      if (isMiddlewareError(error)) return error;
  ...
  tableSchema: async ({ request, params }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'server:databases:browse');
      ...
      return { status: 200 as const, body: await readTableSchema(deps, row, params.table) };
    } catch (error: unknown) {
      if (isMiddlewareError(error)) return error;
  ...
  tableData: async ({ request, params, body }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'server:databases:browse');
      ...
      return { status: 200 as const, body: await readTableData(deps, row, { table: params.table, ...body }) };
    } catch (error: unknown) {
      if (isMiddlewareError(error)) return error;
  ...
  query: async ({ request, params, body }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'server:databases:browse');
      ...
      return { status: 200 as const, body: await execReadOnlyQuery(deps, row, body.sql) };
    } catch (error: unknown) {
      if (isMiddlewareError(error)) return error;
  ...
  search: async ({ request, params, body }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'server:databases:browse');
      ...
      return { status: 200 as const, body: { matches: await searchAcrossTables(deps, row, body.term, body.tables) } };
    } catch (error: unknown) {
      if (isMiddlewareError(error)) return error;
  ...
  reissueAccessCertificate: async ({ request, params }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'server:databases:access');
      ...
      const certificate = await reissueAccessCertificate(deps, access);
      ...
      return { status: 200 as const, body: certificate };
    } catch (error: unknown) {
      if (isMiddlewareError(error)) return error;
