import os from 'node:os';
import path from 'node:path';
import { initServer } from '@ts-rest/fastify';
import { contract } from '@shulkr/shared';
import { DATABASE_PATH } from '@shulkr/backend/db';
import { getAppDeps } from '@shulkr/backend/deps';
import { authenticate, assertPermissions, isMiddlewareError } from '@shulkr/backend/api/middleware';
import { getAllServers } from '@shulkr/backend/services/server_service';
import { calculateDirectorySize } from '@shulkr/backend/services/file_service';

const s = initServer();

const GITHUB_REPO = 'sylfaeen/shulkr';
const VERSION_CACHE_TTL = 60 * 60 * 1000;

let cachedLatestVersion: string | null = null;
let lastVersionCheck = 0;

async function getCurrentVersion(): Promise<string> {
  try {
    const pkgPath = path.resolve(process.cwd(), '..', '..', 'package.json');
    const pkg = JSON.parse(await getAppDeps().fs.readFileText(pkgPath, 'utf-8')) as { version?: string };
    return pkg.version ?? '0.0.0';
  } catch {
    return '0.0.0';
  }
}

async function fetchLatestVersion(): Promise<string | null> {
  const now = getAppDeps().clock().getTime();
  if (cachedLatestVersion && now - lastVersionCheck < VERSION_CACHE_TTL) {
    return cachedLatestVersion;
  }
  try {
    const response = await fetch(`https://api.github.com/repos/${GITHUB_REPO}/releases/latest`, {
      headers: { Accept: 'application/vnd.github.v3+json' },
    });
    if (!response.ok) return cachedLatestVersion;
    const data = (await response.json()) as { tag_name: string };
    cachedLatestVersion = data.tag_name.replace(/^v/, '');
    lastVersionCheck = now;
    return cachedLatestVersion;
  } catch {
    return cachedLatestVersion;
  }
}

function getIpAddress(): string | null {
  const interfaces = os.networkInterfaces();
  for (const entries of Object.values(interfaces)) {
    if (!entries) continue;
    for (const entry of entries) {
      if (!entry.internal && entry.family === 'IPv4') {
        return entry.address;
      }
    }
  }
  return null;
}

export const settingsRoutes = s.router(contract.settings, {
  getVersionInfo: async ({ request }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'settings:general:read');

      const currentVersion = await getCurrentVersion();
      const latestVersion = await fetchLatestVersion();
      const ipAddress = getIpAddress();

      return { status: 200 as const, body: { currentVersion, latestVersion, ipAddress } };
    } catch (error: unknown) {
      if (isMiddlewareError(error)) return error;
      throw error;
    }
  },
  getSystemdUnit: async ({ request }) => {
    try {
      const authUser = await authenticate(request);
      assertPermissions(authUser, 'settings:general:read');

      const nodePath = process.execPath;
      const workingDirectory = process.cwd();
      const systemUser = process.env.USER ?? os.userInfo().username;

      return {
        status: 200 as const,
        body: {
          content: [
            '[Unit]',
            'Description=Shulkr - Minecraft Server Management Panel',
            'After=network.target',
            '',
            '[Service]',
            'Type=simple',
            `User=${systemUser}`,
            `WorkingDirectory=${workingDirectory}`,
            `ExecStart=${nodePath} dist/index.js`,
            'Environment=NODE_ENV=production',
            `EnvironmentFile=${workingDirectory}/../../.env`,
            'Restart=always',
            'RestartSec=10',
            'StandardOutput=journal',
            'StandardError=journal',
            'SyslogIdentifier=shulkr',
            '',
            '# Security hardening',
            'NoNewPrivileges=true',
            'PrivateTmp=true',
            'ProtectSystem=strict',
            'ProtectHome=true',
            `ReadWritePaths=${workingDirectory}/../..`,
            '',
            '[Install]',
            'WantedBy=multi-user.target',
          ].join('\n'),
        },
      };
    } catch (error: unknown) {
      if (isMiddlewareError(error)) return error;
      throw error;
    }
  },
  getDiskUsage: async ({ request }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'settings:general:read');

      const shulkrRoot = path.resolve(process.cwd(), '..', '..');
      const diskStats = await getAppDeps().fs.statfs(shulkrRoot);
      const blockSize = diskStats.bsize;
      const diskTotal = blockSize * diskStats.blocks;
      const diskAvailable = blockSize * diskStats.bavail;
      const diskUsed = diskTotal - blockSize * diskStats.bfree;

      return {
        status: 200 as const,
        body: {
          disk: { total: diskTotal, used: diskUsed, available: diskAvailable },
          shulkr: { total: 0, servers: [] },
        },
      };
    } catch (error: unknown) {
      if (isMiddlewareError(error)) return error;
      throw error;
    }
  },
  getServerSizes: async ({ request }) => {
    try {
      const user = await authenticate(request);
      assertPermissions(user, 'settings:general:read');

      const allServers = await getAllServers(getAppDeps());
      const serverSizes = await Promise.all(
        allServers.map(async (s) => ({
          id: s.id,
          name: s.name,
          size: await calculateDirectorySize(getAppDeps(), s.path),
        }))
      );

      return { status: 200 as const, body: serverSizes };
    } catch (error: unknown) {
      if (isMiddlewareError(error)) return error;
      throw error;
    }
  },
  getDatabaseInfo: async ({ request }) => {
    try {
      const authUser = await authenticate(request);
      assertPermissions(authUser, 'settings:general:read');

      const stat = await getAppDeps().fs.stat(DATABASE_PATH);

      return {
        status: 200 as const,
        body: {
          name: path.basename(DATABASE_PATH),
          size: stat.size,
          modified: new Date(stat.mtimeMs).toISOString(),
        },
      };
    } catch (error: unknown) {
      if (isMiddlewareError(error)) return error;
      throw error;
    }
  },
});
