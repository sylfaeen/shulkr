...
import { serverService as diskServerService } from '@shulkr/backend/services/server_service';
...
export const settingsRoutes = s.router(contract.settings, {
  ...
  getDiskUsage: async ({ request }) => {
    try {
      ...
      const diskStats = await fsPromises.statfs(shulkrRoot);
  ...
  getServerSizes: async ({ request }) => {
    try {
      ...
      const allServers = await diskServerService.getAllServers();
      const serverSizes = await Promise.all(
        allServers.map(async (s) => ({
          id: s.id,
          name: s.name,
          size: await fileService.calculateDirectorySize(s.path),
        }))
      );
  ...
  getDatabaseInfo: async ({ request }) => {
    try {
      ...
      const stat = await fsPromises.stat(DATABASE_PATH);
