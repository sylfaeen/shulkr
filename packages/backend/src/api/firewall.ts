...
export const firewallRoutes = s.router(contract.firewall, {
  ...
  add: async ({ request, body }) => {
    try {
      ...
      const rule = await addFirewallRule(getAppDeps(), {
        action: body.action,
        port: body.port ?? null,
        protocol: body.protocol,
        from_ip: body.from_ip ?? null,
        label: body.label,
      });
      await logAuditAction(getAppDeps(), {
        ...
        details: { action: body.action, port: body.port, protocol: body.protocol, from_ip: body.from_ip, label: body.label },
        ip: request.ip,
      });
      ...
      if (error instanceof Error) {
        if (error.message === ErrorCodes.FIREWALL_RULE_INCOMPLETE) {
          return { status: 200 as const, body: { message: 'Either port or from_ip must be set' } };
        }
        if (error.message === ErrorCodes.FIREWALL_SCRIPT_FAILED) {
          return { status: 200 as const, body: { message: 'Firewall script failed to apply rule' } };
        }
