  ...
  uploadBackupToCloud: async ({ request, params, body }) => {
    try {
      ...
      if (!result.success) {
        ...
        if (result.error === 'destination_disabled' || result.error === 'already_in_cloud') {
          return { status: 400 as const, body: { code: result.error, message: result.error } };
        }
        return { status: 422 as const, body: { code: result.error, message: result.message ?? result.error } };
      }
      ...
      return { status: 200 as const, body: { message: 'Backup uploaded to cloud' } };
    } catch (error: unknown) {
      if (isMiddlewareError(error)) return error;
      throw error;
    }
