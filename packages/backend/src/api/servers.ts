    ...
    try {
      ...
      try {
        ...
        if (error instanceof Error && error.message === ErrorCodes.BACKUP_SHARE_NOT_LOCAL) {
          return {
            status: 404 as const,
            body: { code: ErrorCodes.BACKUP_SHARE_NOT_LOCAL, message: ErrorCodes.BACKUP_SHARE_NOT_LOCAL },
          };
        }
  ...
  revokeBackupShareLink: async ({ request, params }) => {
    try {
      ...
      if (!revoked) {
        return {
          status: 404 as const,
          body: { code: ErrorCodes.BACKUP_SHARE_NOT_FOUND, message: ErrorCodes.BACKUP_SHARE_NOT_FOUND },
        };
      }
