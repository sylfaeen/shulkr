...
export const serversRoutes = s.router(contract.servers, {
  ...
  listBackups: async ({ request, params }) => {
    try {
      ...
      // Prepend pending (creating) backups that aren't already in the ready list
      ...
      if (server) {
        const slug = server.name
          ...
          .replace(/[^a-z0-9]+/g, '-')
          .replace(/^-|-$/g, '');
        const readyFilenames = new Set(backups.map((b) => b.filename));
        const pending = backupService.getPendingBackups(slug);
        for (const p of pending) {
          if (!readyFilenames.has(p.filename)) {
            backups.unshift({
              filename: p.filename,
              size: 0,
              created: p.startedAt,
