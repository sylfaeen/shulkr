...
import { getServerStrategy, setServerStrategy } from '@shulkr/backend/services/cloud_backup_strategy';
  ...
  updateBackupStrategy: async ({ request, params, body }) => {
    try {
      ...
      if ((body.mode === 'cloud-only' || body.mode === 'hybrid') && !body.cloudDestinationId) {
        return { status: 400 as const, body: { code: 'BAD_REQUEST', message: 'cloudDestinationId is required for cloud-only or hybrid mode' } };
      }
