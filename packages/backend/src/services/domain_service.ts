...
// Thrown by setPanelDomain when no certificate could be installed. By then the domain is rolled back and the panel answers on its IP address again; detail carries the certbot output so the administrator can see the actual cause.
export class DomainSslError extends Error {
  readonly code = ErrorCodes.DOMAIN_SSL_FAILED;
  readonly detail: string;

  constructor(domain: string, detail: string) {
    super(
      `No SSL certificate could be installed for ${domain}. The panel domain was not configured and the panel stays reachable on its IP address.`
    );

    this.name = 'DomainSslError';
    this.detail = detail;
  }
}
...
export async function setPanelDomain(deps: Deps, domain: string, port: number) {
  ...
  // Nothing restarts the service before the certificate step: a restart kills every child of the unit, certbot included, and can leave nginx half-configured.
  const existing = await getPanelDomain(deps);
  if (existing) await detachPanelDomain(deps, existing.id);

  const updated = await runDomainScript(deps, ['update-panel', domain]);

  if (!updated.success) {
    if (existing) await restoreIpAccess(deps);
    throw new Error(updated.error || 'Failed to update panel domain');
  }

  // The certificate is part of setting the domain, never a later step. Browsers force HTTPS on HSTS-preloaded TLDs (.dev, .app, ...), so a panel domain without a certificate is unreachable there, and on any other TLD it would carry passwords and tokens in clear.
  const ssl = await runDomainScript(deps, ['enable-ssl', domain]);

  if (!ssl.success) {
    await runDomainScript(deps, ['reset-panel']);
    if (existing) await restoreIpAccess(deps);
    throw new DomainSslError(domain, ssl.error || 'Unknown certbot error');
  }

  const [created] = await deps.db
    .insert(customDomains)
    .values({ server_id: null, domain, port, type: 'panel', ssl_enabled: true, ssl_expires_at: ssl.ssl_expires_at || null })
    .returning();

  await envUpdate(deps, 'CORS_ORIGIN', `https://${domain}`);
  await envUpdate(deps, 'SECURE_COOKIES', 'true');
  restartService(deps);
...
// Puts nginx back to plain IP access and forgets the domain, without touching the environment or restarting.
async function detachPanelDomain(deps: Deps, id: number): Promise<void> {
...
async function restoreIpAccess(deps: Deps): Promise<void> {
  await envUpdate(deps, 'CORS_ORIGIN', `http://${getServerIp()}`);
...
export async function removePanelDomain(deps: Deps) {
  ...
  await detachPanelDomain(deps, panelDomain.id);
  await restoreIpAccess(deps);
