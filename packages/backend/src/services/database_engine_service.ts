...
export async function readCaCertificate(deps: Deps): Promise<string | null> {
  try {
    ...
    // A missing file and an unreadable one both end up here, and they call for opposite fixes: generate the certificate, or open the directory the panel has to traverse.
    deps.logger.warn(
      { path: CA_CERTIFICATE_PATH, error: error instanceof Error ? error.message : String(error) },
      'CA certificate unreadable'
    );
