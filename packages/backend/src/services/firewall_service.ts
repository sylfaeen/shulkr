...
// Idempotent allow rule for a single port + protocol with no from-IP. Used by server creation to make sure the Minecraft port is open without failing on rerun.
...
// Removes the allow rule (if any) for a single port + protocol with no from-IP. Used when a server is deleted.
