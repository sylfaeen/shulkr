...
export class FirewallService {
  ...
  async addRule(port: number, protocol: FirewallProtocol, label: string, options: { reuseExisting?: boolean } = {}) {
    ...
    if (existing) {
      if (options.reuseExisting) {
        ...
        if (!existing.enabled) {
          ...
          await db.update(firewallRules).set({ enabled: true }).where(eq(firewallRules.id, existing.id));
        }
        return updated;
      }
  ...
  async removeRuleByPort(port: number, protocol: FirewallProtocol) {
    const [rule] = await db
      .select()
      .from(firewallRules)
      .where(and(eq(firewallRules.port, port), eq(firewallRules.protocol, protocol)))
    ...
    return this.removeRule(rule.id);
  }
