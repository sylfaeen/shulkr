...
describe('createServerDatabase', () => {
  ...
  // No count cap: it protects nothing (a single database can fill the disk) and only blocks the administrator on their own machine.
  it('puts no cap on the number of databases per server', async () => {
    const server = seedServer(deps);

    for (const slug of ['one', 'two', 'three', 'four', 'five', 'six']) {
      await createServerDatabase(deps, { serverId: server.id, slug });
    }

    expect(await listServerDatabases(deps, server.id)).toHaveLength(6);
  });
