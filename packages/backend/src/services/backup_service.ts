...
import { clearDatabaseDumps, writeDatabaseDumps } from '@shulkr/backend/services/database_backup_service';
...
export async function createFullBackup(
  ...
  try {
    ...
    // Dumped into the server directory just before archiving, removed right after, so the archive holds the files and the data at the same point in time.
    if (serverId) await writeDatabaseDumps(getAppDeps(), serverId, serverPath);
    ...
    try {
      ...
      if (serverId) await clearDatabaseDumps(getAppDeps(), serverPath);
    }
...
export async function createIncrementalBackup(
  ...
  // Dumped before the snapshot so the database lands in the archive alongside the files it belongs to, whichever of the two modes runs below.
  await writeDatabaseDumps(getAppDeps(), serverId, serverPath);

  try {
    return await runIncrementalBackup(deps, serverPath, serverName, serverId, source, onProgress, overrideFilename);
  } finally {
    await clearDatabaseDumps(getAppDeps(), serverPath);
  }
}
...
export class BackupService {
  ...
  async createFullBackup(
    ...
    return createFullBackup(getAppDeps(), serverPath, serverName, source, onProgress, overrideFilename, serverId);
  }
