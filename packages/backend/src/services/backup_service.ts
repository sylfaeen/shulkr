...
export class BackupService {
  ...
  async createIncrementalBackup(
    ...
    if (!previousSnapshot) {
      ...
      const result = await this.createFullBackup(serverPath, serverName, source, onProgress);
      if (result.success) {
    ...
    try {
      ...
      const filename = buildBackupFilename(serverName, source, 'incremental');
      const backupPath = path.join(BACKUPS_BASE_PATH, filename);

      await this.createSelectiveZipArchive(serverPath, backupPath, changedFiles, 0, onProgress);
