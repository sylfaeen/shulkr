...
export class FileService {
  ...
  async getDirectorySizes(basePath: string, relativePath: string = '/'): Promise<FileServiceResponse<Record<string, number>>> {
    const safePath = this.validatePath(basePath, relativePath);
    ...
    try {
      ...
      await Promise.all(
        entries
          ...
          .map(async (entry) => {
            const entryPath = path.join(safePath, entry.name);
            sizes[entry.name] = await this.calculateDirectorySize(entryPath);
          })
      );
