...
function buildDiscordPayload(event: WebhookEvent, _serverId: string, serverName: string, data: WebhookEventData): object {
  ...
  const parts = [`${emoji} **${label}** — ${serverName}`];

  if (data.playerName) parts.push(`Player: **${data.playerName}**`);
  if (data.backupFilename) parts.push(`Backup: \`${data.backupFilename}\``);
  if (data.taskName) parts.push(`Task: **${data.taskName}**`);
