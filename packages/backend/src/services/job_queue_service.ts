...
class JobQueueService {
  ...
  private async executeTask(task: ScheduledTask) {
    ...
    switch (task.type) {
      ...
      case 'command':
        ...
        break;
      case 'chain':
        await this.executeChainTask(task.server_id, config);
        break;
    }
  }
  ...
  private async executeChainTask(serverId: string, config: TaskConfig) {
    const steps = config.steps;
    if (!steps || steps.length === 0) throw new Error('No steps configured');

    const stepResults: Array<{ step: number; type: string; status: string; durationMs: number; error?: string }> = [];
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      try {
        ...
        if (step.onError === 'stop') {
          // Attach step results to the error for the caller to persist
          const chainError = new Error(`Step ${i + 1} (${step.type}) failed: ${errorMessage}`);
          (chainError as Error & { stepResults: typeof stepResults }).stepResults = stepResults;
          throw chainError;
        }
      }
    }

    // Store step results — caller reads this from a custom property
    const result = new Error('__chain_success__');
    (result as Error & { stepResults: typeof stepResults }).stepResults = stepResults;
    // We don't throw on success — store results via output field
    // The caller in processNextJob will need to persist this
  }
