...
import { backupServer, getServerById, restartServer } from '@shulkr/backend/services/server_service';
import { serverProcessManager } from '@shulkr/backend/services/server_process_manager';
import { broadcastNotification } from '@shulkr/backend/services/notification_service';
...
export async function enqueueJob(deps: Deps, task: ScheduledTask): Promise<void> {
  const maxRetries = MAX_RETRIES_BY_TYPE[task.type] ?? 0;
  await deps.db.insert(taskExecutions).values({
    task_id: task.id,
    status: 'pending',
    duration_ms: 0,
...
class JobQueueService {
  ...
  async enqueue(task: ScheduledTask): Promise<void> {
    return enqueueJob(getAppDeps(), task);
  }
  ...
  private async cleanupOldExecutions() {
    ...
    const result = await deps.db.delete(taskExecutions).where(lt(taskExecutions.created_at, cutoff));
  ...
  private async processNextJob() {
    ...
    try {
      ...
      try {
        ...
        const durationMs = clock().getTime() - startTime;
        ...
        await Promise.all([
          ...
          db
            ...
            .set({
              last_run: clock().toISOString(),
              updated_at: clock().toISOString(),
            })
            .where(eq(scheduledTasks.id, task.id)),
        ]);

        dispatchWebhooks(getAppDeps(), task.server_id, task.type === 'backup' ? 'backup:success' : 'task:success', {
          taskName: task.name,
          backupFilename: taskResult.backupFilename,
        }).catch(() => {});
      } catch (error: unknown) {
        const durationMs = clock().getTime() - startTime;
        ...
        dispatchWebhooks(getAppDeps(), task.server_id, task.type === 'backup' ? 'backup:failure' : 'task:failure', {
          taskName: task.name,
          error: errorMessage,
        }).catch(() => {});
  ...
  private async executeTask(task: ScheduledTask): Promise<{ stepResults?: Array<StepResult>; backupFilename?: string }> {
    const config = (task.config || {}) as TaskConfig;
    switch (task.type) {
      case 'restart':
        ...
        return {};
      case 'backup': {
        const backupFilename = await this.executeBackupTask(task.server_id, config);
  ...
  private async executeRestartTask(serverId: string, config: TaskConfig) {
    ...
    const result = await restartServer(getAppDeps(), serverId);
    if (!result.success) throw new Error(result.error ?? 'Restart failed');
  }

  private async executeBackupTask(serverId: string, config: TaskConfig): Promise<string | undefined> {
    const result = await backupServer(getAppDeps(), serverId, config.backup_paths, 'auto');
  ...
  private async executeChainTask(serverId: string, config: TaskConfig): Promise<Array<StepResult>> {
    ...
    for (let i = 0; i < steps.length; i++) {
      const step = steps[i];
      const stepStart = clock().getTime();
      try {
        await this.executeChainStep(serverId, step);
        stepResults.push({ step: i, type: step.type, status: 'success', durationMs: clock().getTime() - stepStart });
      } catch (error: unknown) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
        stepResults.push({
          step: i,
          type: step.type,
          status: 'error',
          durationMs: clock().getTime() - stepStart,
          error: errorMessage,
        });
  ...
  private evaluateRule(serverId: string, rule: ConditionRule): boolean {
    ...
    switch (rule.type) {
      ...
      case 'time_range': {
        const from = rule.config.from as string;
        const to = rule.config.to as string;
        const now = clock();
