...
class GcService {
  ...
  /**
   * Collect GC stats via Spark if available. Returns parsed metrics.
   */
  async collectViaSpark(serverId: string): Promise<void> {
    ...
    // Spark gc command returns GC stats — we just trigger it so the console output gets parsed
    serverProcessManager.sendCommand(serverId, 'spark gc');
  }
