...
export type MetricsPeriod = '1h' | '6h' | '24h' | '7d' | '30d' | '60d' | '90d' | '120d';
...
function periodToGroupBy(period: MetricsPeriod): string {
  switch (period) {
    ...
    case '90d':
    case '120d':
      return "strftime('%Y-%m-%d ', created_at) || printf('%02d', (CAST(strftime('%H', created_at) AS INTEGER) / 6) * 6) || ':00'";
  }
}
...
export function initializeMetricsHistory(deps: Deps): void {
  ...
  console.log(`Metrics history service initialized (60s sampling, ${DEFAULT_RETENTION_DAYS}d retention)`);
}
