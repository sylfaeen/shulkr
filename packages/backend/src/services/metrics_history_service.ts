...
import { alertService } from '@shulkr/backend/services/alert_service';
  ...
  private async sampleAllServers() {
    ...
    for (const serverId of runningServers) {
      try {
        ...
        alertService
          .evaluate(serverId, { cpu: metrics.cpu, memoryPercent: metrics.memory_percent, tps: tpsData.tps ?? undefined })
