...
class MetricsHistoryService {
  ...
  private async cleanup() {
    const cutoff = new Date(Date.now() - DEFAULT_RETENTION_DAYS * 24 * 3600 * 1000).toISOString();
  ...
  async queryHistory(serverId: string, period: MetricsPeriod): Promise<Array<AggregatedMetric>> {
    const now = Date.now();
    ...
    const rows = await db.all<{ bucket: string; avg_cpu: number; avg_mem: number; max_players: number }>(
      sql`SELECT ${sql.raw(groupExpr)} as bucket,
            AVG(cpu) as avg_cpu,
            AVG(memory_percent) as avg_mem,
            MAX(player_count) as max_players
          FROM metrics_history
          WHERE server_id = ${serverId} AND created_at >= ${since}
          GROUP BY bucket
          ORDER BY bucket ASC`
    );
