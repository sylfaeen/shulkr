  ...
  private async checkAndEnqueueTasks() {
    ...
    for (const task of tasks) {
      if (this.shouldRunNow(task.cron_expression, now)) {
        // Dedup key granularity must match the cron's precision: second-level for
        // 6-field crons, minute-level for 5-field ones. Without this, a 5-field cron
        // like "0 14 * * *" re-enqueues on every seconds-precision tick from 14:00:00
        // through 14:00:59 — flooding the queue with 60 executions.
