...
import { enqueueJob } from '@shulkr/backend/services/job_queue_service';
import { getAppDeps } from '@shulkr/backend/deps';

// Simple cron parser and scheduler Supports 5-field (minute hour day month weekday) and 6-field (second minute hour day month weekday) Enqueues jobs into the job queue instead of executing them directly.
...
class TaskScheduler {
  ...
  async initialize() {
    ...
    const tasks = await getAppDeps().db.select().from(scheduledTasks).where(eq(scheduledTasks.enabled, true));
  ...
  private async checkAndEnqueueTasks() {
    const now = getAppDeps().clock();
    const tasks = await getAppDeps().db.select().from(scheduledTasks).where(eq(scheduledTasks.enabled, true));

    for (const task of tasks) {
      if (this.shouldRunNow(task.cron_expression, now)) {
        // Dedup key granularity must match the cron's precision: second-level for 6-field crons, minute-level for 5-field ones. Without this, a 5-field cron like "0 14 * * *" re-enqueues on every seconds-precision tick from 14:00:00 through 14:00:59, flooding the queue with 60 executions.
        const hasSeconds = task.cron_expression.trim().split(/\s+/).length === 6;
        const granularityMs = hasSeconds ? 1000 : 60_000;
        const dedupKey = Math.floor(now.getTime() / granularityMs);
        const lastRun = this.lastRunTimestamps.get(task.id);
        if (lastRun === dedupKey) continue;
        this.lastRunTimestamps.set(task.id, dedupKey);
        console.log(`Enqueuing scheduled task: ${task.name} (${task.type})`);
        await enqueueJob(getAppDeps(), task);
      }
    }
  }
  ...
  private shouldRunNow(cronExpression: string, now: Date): boolean {
    ...
    if (parts.length === 6) {
      const [secondStr, ...rest] = parts;
      [minuteStr, hourStr, dayStr, monthStr, weekdayStr] = rest;
      if (!this.matchCronPart(secondStr, now.getSeconds())) return false;
