...
describe('backup_share_service', () => {
  ...
  it('never stores the token in clear, only its hash and a short preview', async () => {
    const { token, preview } = await createShareLink(deps, { filename, expiresInHours: 24, createdBy: 1 });

    const [row] = await deps.db.select().from(backupShareLinks).where(eq(backupShareLinks.filename, filename));

    expect(row.token_hash).not.toBe(token);
    expect(row.token_hash).toHaveLength(64);
    expect(row.token_preview).toBe(preview);
  });
  ...
  it('recordDownload increments the counter and stores the IP', async () => {
    ...
    await recordDownload(deps, row.id, '203.0.113.7');

    const [updated] = await listShareLinks(deps, filename);
    expect(updated.download_count).toBe(1);
    expect(updated.last_downloaded_ip).toBe('203.0.113.7');
  });
