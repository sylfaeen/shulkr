...
// A reissue hands out a new certificate, it does not invalidate the previous one: REQUIRE X509 accepts any certificate signed by our authority, whatever its subject. To actually cut off a certificate that leaked, revoke the access and create a new one.
export async function reissueAccessCertificate(
  deps: Deps,
  row: DatabaseAccessRow
): Promise<{ clientCertificate: string; clientKey: string }> {
  await assertEngineAvailable(deps);

  if (!row.require_certificate) throw new Error(ErrorCodes.DATABASE_ACCESS_NO_CERTIFICATE);

  const outcome = await runDatabaseScript(deps, ['reissue-certificate', row.username, row.allowed_ip]);
  if (!outcome.success) throw new Error(ErrorCodes.DATABASE_SCRIPT_FAILED);
