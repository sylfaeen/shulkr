...
type Deps = Pick<AppDeps, 'db' | 'shell' | 'fs' | 'clock' | 'encryption' | 'config' | 'logger'>;
...
function buildCredentials(row: DatabaseAccessRow, dbName: string, password: string): DatabaseAccessCredentials {
  ...
  return {
    ...
    username: row.username,
    password,
    connectionUrl: buildDirectConnectionUrl({ host, dbUser: row.username, dbName, label: row.label, password }),
    scope: row.scope,
    allowedIp: row.allowed_ip,
...
// The password is rotated along with the certificate, which is what makes the previous pair unusable: REQUIRE X509 accepts any certificate signed by our authority, so a new certificate alone would leave the old one working.
export async function reissueAccessCertificate(
  deps: Deps,
  row: DatabaseAccessRow
): Promise<{ clientCertificate: string; clientKey: string; password: string }> {
  await assertEngineAvailable(deps);

  if (!row.require_certificate) throw new Error(ErrorCodes.DATABASE_ACCESS_NO_CERTIFICATE);

  const password = generateDatabasePassword();
  const outcome = await runDatabaseScript(deps, ['reissue-certificate', row.username, row.allowed_ip], password);
  if (!outcome.success) throw new Error(ErrorCodes.DATABASE_SCRIPT_FAILED);

  const clientCertificate = typeof outcome.payload?.clientCert === 'string' ? outcome.payload.clientCert : '';
  const clientKey = typeof outcome.payload?.clientKey === 'string' ? outcome.payload.clientKey : '';

  if (!clientCertificate || !clientKey) throw new Error(ErrorCodes.DATABASE_SCRIPT_FAILED);
  ...
  return { clientCertificate, clientKey, password };
}
