import { EventEmitter } from 'events';
import path from 'path';
import { ErrorCodes, computeAikarFlags, parseRamToMb } from '@shulkr/shared';
import { parseConsoleLine } from '@shulkr/backend/lib/log_parser';
import { parseGcLine } from '@shulkr/backend/services/gc_service';
import { type AppDeps, getAppDeps } from '@shulkr/backend/deps';
import type { SpawnHandle } from '@shulkr/backend/deps/shell_runner';

export type ServerStatus = 'stopped' | 'starting' | 'running' | 'stopping';

export interface ServerConfig {
  id: string;
  name: string;
  path: string;
  jar_file: string | null;
  min_ram: string;
  max_ram: string;
  jvm_flags: string;
  java_port: number;
  java_path: string | null;
  auto_restart_on_crash?: boolean;
}

interface ManagedServer {
  config: ServerConfig;
  process: SpawnHandle | null;
  status: ServerStatus;
  pid: number | null;
  startedAt: Date | null;
  readyEmitted: boolean;
}

const STOP_SIGTERM_DELAY_MS = 30_000;
const STOP_SIGKILL_DELAY_MS = 60_000;

interface ConsoleLogEntry {
  type: 'stdout' | 'stderr';
  data: string;
  timestamp: number;
  level?: string;
}

const CONSOLE_HISTORY_LIMIT = 2000;

const CRASH_RESTART_DELAY_MS = 10_000;
const CRASH_MAX_RESTARTS = 3;
const CRASH_WINDOW_MS = 5 * 60_000;

type Deps = Pick<AppDeps, 'shell' | 'fs' | 'clock'>;

const JAVA_CANDIDATE_PATHS = [
  '/usr/lib/jvm/temurin-25-jre/bin/java',
  '/usr/lib/jvm/temurin-25-jdk/bin/java',
  '/usr/lib/jvm/temurin-21-jre/bin/java',
  '/usr/lib/jvm/temurin-21-jdk/bin/java',
  '/usr/lib/jvm/temurin-17-jre/bin/java',
  '/usr/lib/jvm/temurin-17-jdk/bin/java',
  '/usr/lib/jvm/java-25-openjdk-amd64/bin/java',
  '/usr/lib/jvm/java-25-openjdk-arm64/bin/java',
  '/usr/lib/jvm/java-21-openjdk-amd64/bin/java',
  '/usr/lib/jvm/java-21-openjdk-arm64/bin/java',
  '/usr/lib/jvm/java-17-openjdk-amd64/bin/java',
  '/usr/lib/jvm/java-17-openjdk-arm64/bin/java',
  '/usr/bin/java',
];

async function removeSessionLocks(deps: Deps, serverDir: string): Promise<void> {
  try {
    const entries = await deps.fs.readdir(serverDir);

    for (const entryName of entries) {
      const entryPath = path.join(serverDir, entryName);

      try {
        const entryStat = await deps.fs.stat(entryPath);
        if (!entryStat.isDirectory()) continue;
      } catch {
        continue;
      }

      const lockPath = path.join(serverDir, entryName, 'session.lock');

      if (await deps.fs.exists(lockPath)) {
        await deps.fs.unlink(lockPath).catch(() => {});
      }
    }
  } catch {}
}

async function findJava(deps: Deps): Promise<string | null> {
  for (const p of JAVA_CANDIDATE_PATHS) {
    if (await deps.fs.exists(p)) return p;
  }

  const which = await deps.shell.run('which', ['java']);

  if (which.success) {
    const result = which.stdout.trim();
    if (result && (await deps.fs.exists(result))) return result;
  }

  return null;
}

async function isJavaBinary(deps: Deps, binaryPath: string): Promise<boolean> {
  const result = await deps.shell.run(binaryPath, ['-version'], { timeoutMs: 5000 });
  const combined = result.stdout + result.stderr;

  return /java|openjdk|jdk|jre/i.test(combined);
}

export class ServerProcessManager extends EventEmitter {
  private servers: Map<string, ManagedServer> = new Map();
  private lineBuffers: Map<string, { stdout: string; stderr: string }> = new Map();
  private consoleHistory: Map<string, Array<ConsoleLogEntry>> = new Map();
  private crashCounts: Map<string, { count: number; firstAt: number }> = new Map();

  private getLineBuffer(serverId: string) {
    if (!this.lineBuffers.has(serverId)) {
      this.lineBuffers.set(serverId, { stdout: '', stderr: '' });
    }

    return this.lineBuffers.get(serverId)!;
  }

  private addToHistory(serverId: string, entry: ConsoleLogEntry) {
    if (!this.consoleHistory.has(serverId)) {
      this.consoleHistory.set(serverId, []);
    }

    const history = this.consoleHistory.get(serverId)!;
    history.push(entry);

    if (history.length > CONSOLE_HISTORY_LIMIT) {
      history.splice(0, history.length - CONSOLE_HISTORY_LIMIT);
    }
  }

  getConsoleHistory(serverId: string): Array<ConsoleLogEntry> {
    return this.consoleHistory.get(serverId) ?? [];
  }

  getConsoleHistoryPage(serverId: string, offset: number, limit: number): { lines: Array<ConsoleLogEntry>; total: number } {
    const history = this.consoleHistory.get(serverId) ?? [];
    const total = history.length;
    const end = total - offset;
    const start = Math.max(0, end - limit);

    return { lines: history.slice(start, Math.max(0, end)), total };
  }

  getStatus(serverId: string): { status: ServerStatus; pid: number | null; uptime: number | null } {
    const server = this.servers.get(serverId);

    if (!server) {
      return { status: 'stopped', pid: null, uptime: null };
    }

    const uptime = server.startedAt ? Math.floor((getAppDeps().clock().getTime() - server.startedAt.getTime()) / 1000) : null;

    return {
      status: server.status,
      pid: server.pid,
      uptime,
    };
  }

  async start(config: ServerConfig): Promise<{ success: boolean; error?: string }> {
    const deps = getAppDeps();
    const existing = this.servers.get(config.id);

    if (existing && existing.status !== 'stopped') {
      return { success: false, error: ErrorCodes.SERVER_ALREADY_RUNNING };
    }

    const javaPath = config.java_path && (await deps.fs.exists(config.java_path)) ? config.java_path : await findJava(deps);

    if (!javaPath) {
      return { success: false, error: ErrorCodes.SERVER_JAVA_NOT_FOUND };
    }

    if (!(await isJavaBinary(deps, javaPath))) {
      return { success: false, error: ErrorCodes.SERVER_JAVA_NOT_FOUND };
    }

    if (!config.jar_file) {
      return { success: false, error: ErrorCodes.SERVER_JAR_NOT_FOUND };
    }

    const serverDir = config.path;
    const jarPath = path.join(serverDir, config.jar_file);

    if (!(await deps.fs.exists(serverDir))) {
      return { success: false, error: ErrorCodes.SERVER_DIR_NOT_FOUND };
    }

    if (!(await deps.fs.exists(jarPath))) {
      return { success: false, error: ErrorCodes.SERVER_JAR_NOT_FOUND };
    }

    const ramMb = parseRamToMb(config.max_ram) || 2048;
    const defaultAikar = computeAikarFlags({ ramMb }).flags.join(' ');
    const flagsSource = config.jvm_flags || defaultAikar;
    const jvmFlags = flagsSource.split(' ').filter((f) => f.trim());

    const jvmArgs: Array<string> = [
      `-Xms${config.min_ram}`,
      `-Xmx${config.max_ram}`,
      ...jvmFlags,
      '-Djline.terminal=dumb',
      '-Dterminal.jline=false',
      '-Dterminal.ansi=true',
    ];

    jvmArgs.push('-jar', jarPath, 'nogui');
    await removeSessionLocks(deps, serverDir);

    const managedServer: ManagedServer = {
      config,
      process: null,
      status: 'starting',
      pid: null,
      startedAt: null,
      readyEmitted: false,
    };

    this.servers.set(config.id, managedServer);

    let proc: SpawnHandle;

    try {
      proc = deps.shell.spawn(javaPath, jvmArgs, { cwd: serverDir });
    } catch {
      managedServer.status = 'stopped';

      return { success: false, error: ErrorCodes.SERVER_START_FAILED };
    }

    const spawnResult = await new Promise<{ success: boolean; error?: string }>((resolve) => {
      const timeout = setTimeout(() => {
        if (proc.pid && proc.pid > 0) {
          resolve({ success: true });
        } else {
          resolve({ success: false, error: ErrorCodes.SERVER_START_FAILED });
        }
      }, 500);

      proc.on('error', (error) => {
        clearTimeout(timeout);
        this.lineBuffers.delete(config.id);
        managedServer.status = 'stopped';
        managedServer.process = null;
        managedServer.pid = null;
        this.emit('server:error', { serverId: config.id, error: error.message });
        resolve({ success: false, error: error.message });
      });

      proc.on('spawn', () => {
        clearTimeout(timeout);
        resolve({ success: true });
      });
    });

    if (!spawnResult.success) {
      managedServer.status = 'stopped';

      return spawnResult;
    }

    managedServer.process = proc;
    managedServer.pid = proc.pid > 0 ? proc.pid : null;
    managedServer.status = 'running';
    managedServer.startedAt = deps.clock();

    proc.stdout?.on('data', (data: Buffer | string) => {
      const buffer = this.getLineBuffer(config.id);
      buffer.stdout += typeof data === 'string' ? data : data.toString();
      const lines = buffer.stdout.split(/\r?\n/);
      buffer.stdout = lines.pop() || '';

      lines.forEach((line) => {
        if (line.trim()) {
          parseGcLine(getAppDeps(), config.id, line);
          const parsed = parseConsoleLine(line);

          this.addToHistory(config.id, {
            type: 'stdout',
            data: parsed.content,
            timestamp: deps.clock().getTime(),
            level: parsed.level,
          });

          this.emit('console:output', {
            serverId: config.id,
            type: 'stdout',
            data: parsed.content,
            level: parsed.level,
          });

          if (/Done \([\d.]+s\)!/i.test(parsed.content) && !managedServer.readyEmitted) {
            managedServer.readyEmitted = true;
            this.emit('server:ready', { serverId: config.id });
          }
        }
      });
    });

    proc.stderr?.on('data', (data: Buffer | string) => {
      const buffer = this.getLineBuffer(config.id);
      buffer.stderr += typeof data === 'string' ? data : data.toString();
      const lines = buffer.stderr.split(/\r?\n/);
      buffer.stderr = lines.pop() || '';

      lines.forEach((line) => {
        if (line.trim()) {
          const parsed = parseConsoleLine(line);

          this.addToHistory(config.id, {
            type: 'stderr',
            data: parsed.content,
            timestamp: deps.clock().getTime(),
            level: parsed.level,
          });

          this.emit('console:output', {
            serverId: config.id,
            type: 'stderr',
            data: parsed.content,
            level: parsed.level,
          });
        }
      });
    });

    proc.on('exit', (code, signal) => {
      const wasStopping = managedServer.status === 'stopping';
      this.lineBuffers.delete(config.id);
      managedServer.status = 'stopped';
      managedServer.process = null;
      managedServer.pid = null;
      managedServer.startedAt = null;
      managedServer.readyEmitted = false;
      this.emit('server:stopped', { serverId: config.id, code, signal });

      if (config.auto_restart_on_crash && code !== 0 && !wasStopping) {
        const now = deps.clock().getTime();
        const crashes = this.crashCounts.get(config.id);

        if (crashes && crashes.count >= CRASH_MAX_RESTARTS && now - crashes.firstAt < CRASH_WINDOW_MS) {
          this.emit('console:output', {
            serverId: config.id,
            type: 'stderr',
            data: `[Shulkr] Too many crashes (${crashes.count} in 5 min). Auto-restart disabled.`,
            level: 'ERROR',
          });

          return;
        }

        if (!crashes || now - crashes.firstAt >= CRASH_WINDOW_MS) {
          this.crashCounts.set(config.id, { count: 1, firstAt: now });
        } else {
          crashes.count++;
        }

        const attempt = this.crashCounts.get(config.id)?.count ?? 1;

        this.emit('console:output', {
          serverId: config.id,
          type: 'stderr',
          data: `[Shulkr] Server crashed (exit code ${code}). Auto-restarting in 10s... (attempt ${attempt}/${CRASH_MAX_RESTARTS})`,
          level: 'WARN',
        });

        setTimeout(() => {
          this.start(config).catch((err) => {
            console.error(`Crash recovery failed for ${config.name}:`, err);
          });
        }, CRASH_RESTART_DELAY_MS);
      }
    });

    proc.on('error', (error) => {
      this.lineBuffers.delete(config.id);
      managedServer.status = 'stopped';
      managedServer.process = null;
      managedServer.pid = null;
      this.emit('server:error', { serverId: config.id, error: error.message });
    });

    this.emit('server:started', { serverId: config.id, pid: proc.pid });

    return { success: true };
  }

  async stop(serverId: string): Promise<{ success: boolean; error?: string }> {
    const server = this.servers.get(serverId);

    if (!server || server.status === 'stopped' || !server.process) {
      return { success: false, error: ErrorCodes.SERVER_NOT_RUNNING };
    }

    if (server.status === 'stopping') {
      return { success: false, error: ErrorCodes.SERVER_ALREADY_STOPPING };
    }

    server.status = 'stopping';

    return new Promise((resolve) => {
      const proc = server.process!;

      const sigkillTimeout = setTimeout(() => {
        if (server.status === 'stopping') {
          proc.kill('SIGKILL');
        }
      }, STOP_SIGKILL_DELAY_MS);

      const sigtermTimeout = setTimeout(() => {
        if (server.status === 'stopping' && proc.pid > 0) {
          proc.kill('SIGTERM');
        }
      }, STOP_SIGTERM_DELAY_MS);

      proc.once('exit', () => {
        clearTimeout(sigtermTimeout);
        clearTimeout(sigkillTimeout);
        resolve({ success: true });
      });

      if (proc.stdin && (proc.stdin as NodeJS.WritableStream & { writable?: boolean }).writable !== false) {
        proc.stdin.write('stop\n');
      }
    });
  }

  async restart(config: ServerConfig): Promise<{ success: boolean; error?: string }> {
    const server = this.servers.get(config.id);

    if (server && server.status !== 'stopped') {
      const stopResult = await this.stop(config.id);

      if (!stopResult.success) {
        return stopResult;
      }

      await new Promise((resolve) => setTimeout(resolve, 2000));
    }

    return this.start(config);
  }

  sendCommand(serverId: string, command: string): boolean {
    const server = this.servers.get(serverId);

    if (!server || server.status !== 'running' || !server.process?.stdin) {
      return false;
    }

    const stdin = server.process.stdin as NodeJS.WritableStream & { writable?: boolean };
    if (stdin.writable === false) return false;
    stdin.write(command + '\n');

    return true;
  }

  getRunningServers(): Array<string> {
    const running: Array<string> = [];

    this.servers.forEach((server, id) => {
      if (server.status === 'running') {
        running.push(id);
      }
    });

    return running;
  }

  async shutdownAll(): Promise<void> {
    const stopPromises: Array<Promise<unknown>> = [];

    this.servers.forEach((server, id) => {
      if (server.status !== 'stopped') {
        stopPromises.push(this.stop(id));
      }
    });

    await Promise.all(stopPromises);
  }
}

// Standalone exports for callers that prefer function-injection over the class facade. Internally delegate to the singleton (which holds the EventEmitter state).

export async function startServerProcess(_deps: Deps, config: ServerConfig) {
  return serverProcessManager.start(config);
}

export async function stopServerProcess(_deps: Deps, serverId: string) {
  return serverProcessManager.stop(serverId);
}

export function sendServerCommand(_deps: Deps, serverId: string, command: string): boolean {
  return serverProcessManager.sendCommand(serverId, command);
}

export function getServerProcessStatus(_deps: Deps, serverId: string) {
  return serverProcessManager.getStatus(serverId);
}

export const serverProcessManager = new ServerProcessManager();
