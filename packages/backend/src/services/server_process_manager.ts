...
class ServerProcessManager extends EventEmitter {
  ...
  private removeSessionLocks(serverDir: string): void {
    try {
      const entries = fs.readdirSync(serverDir, { withFileTypes: true });
      for (const entry of entries) {
        if (!entry.isDirectory()) continue;
        const lockPath = path.join(serverDir, entry.name, 'session.lock');
  ...
  async start(config: ServerConfig): Promise<{ success: boolean; error?: string }> {
    ...
    // Remove stale session.lock files left by a previous crash
    this.removeSessionLocks(serverDir);
    ...
    try {
      ...
      proc.stdout?.on('data', (data: Buffer) => {
        ...
        lines.forEach((line) => {
          if (line.trim()) {
            gcService.parseGcLine(config.id, line);
  ...
  async stop(serverId: string): Promise<{ success: boolean; error?: string }> {
    ...
    return new Promise((resolve) => {
      ...
      // Shutdown escalation: 'stop' command → SIGTERM after 30s → SIGKILL after 60s.
      // Paper + many plugins routinely takes 20-45s to save the world, so escalating
      // too early (previously 5s) kills the process mid-save with exit 143 and corrupts
      // chunks.
