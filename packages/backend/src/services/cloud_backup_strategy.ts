...
export type UploadExistingBackupResult =
  ...
  | {
      success: false;
      error: 'backup_not_found' | 'already_in_cloud' | 'upload_in_progress' | 'destination_not_found' | 'destination_disabled';
      message?: string;
    };
...
export async function uploadExistingBackupWithDeps(
  ...
  if (existing && existing.cloud_upload_status === 'uploading') {
    return { success: false, error: 'upload_in_progress' };
  }
  ...
  void runUpload(deps, { serverId, filename, localPath, destRow, key }).catch((error: unknown) => {
    console.error('[cloud_backup] unexpected upload error', error);
  });
...
async function runUpload(
  deps: Deps,
  args: { serverId: string; filename: string; localPath: string; destRow: CloudDestinationRow; key: string }
): Promise<void> {
  const { serverId, filename, localPath, destRow, key } = args;
  const creds = rowToCredentials(destRow);

  try {
    const uploaded = await deps.s3.uploadFile(creds, localPath, key);
    ...
    await deps.db
      .update(backupMetadata)
      .set({ cloud_upload_status: 'failed', cloud_upload_error: message })
      .where(and(eq(backupMetadata.server_id, serverId), eq(backupMetadata.filename, filename)));
  }
}
...
// Downloads a cloud-only backup into a staging dir next to the local backups so it can be streamed, then removed by the caller. Never under os.tmpdir(): the systemd unit runs with PrivateTmp and Debian 13 / Ubuntu 24.10+ mount /tmp as a RAM-backed tmpfs capped at half the RAM, which a multi-GB archive would exhaust. A failed download leaves no partial file behind.
export async function stageCloudBackupWithDeps(deps: Deps, metadata: BackupMetadataRow): Promise<string> {
  const stagingDir = join(BACKUPS_BASE_PATH, '.cloud-restore');
  await deps.fs.mkdir(stagingDir, { recursive: true });
  const localPath = join(stagingDir, metadata.filename);

  try {
    await downloadFromCloudWithDeps(deps, metadata, localPath);
  } catch (err) {
    await deps.fs.unlink(localPath).catch(() => {});
    throw err;
  }
