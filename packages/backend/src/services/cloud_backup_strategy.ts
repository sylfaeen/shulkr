...
export async function applyPostBackup(
  ...
  const [serverRow] = await db
    ...
    .limit(1);
  await enforceRotation(serverId, destRow, strategy, serverRow?.max_backups ?? 0);
...
export async function uploadExistingBackup(
  serverId: string,
  filename: string,
  ...
  const [destRow] = await db.select().from(cloudDestinations).where(eq(cloudDestinations.id, cloudDestinationId)).limit(1);
  ...
  try {
    uploaded = await uploadFile(creds, localPath, key);
  } catch (error: unknown) {
...
export async function setServerStrategy(serverId: string, strategy: BackupStrategy): Promise<void> {
  await db
    .update(servers)
    .set({ backup_strategy: strategy, updated_at: new Date().toISOString() })
    .where(eq(servers.id, serverId));
}
...
async function enforceRotation(
  ...
  if (strategy.mode === 'hybrid' && maxLocalBackups > 0) {
    ...
    for (const old of excess) {
      ...
      await db.update(backupMetadata).set({ location: 'cloud', local_path: null }).where(eq(backupMetadata.id, old.id));
    }
  }
