...
function candidateDirs(): Array<string> {
  return [
    ...
    // Dev via tsx: source lives at packages/backend/src/services/, so jump up twice
    // to reach the backend package root, then into dist/assets/plugins/.
    path.resolve(__dirname, '..', '..', 'dist', 'assets', 'plugins'),
    // Dev when cwd = backend package (turbo/pnpm launch scripts from the package dir).
    path.resolve(process.cwd(), 'dist', 'assets', 'plugins'),
    // Dev when cwd = repo root.
