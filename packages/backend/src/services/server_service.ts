...
export class ServerService {
  ...
  async createServer(data: CreateServerRequest) {
    ...
    // Automatically open the server port on the firewall (reuse existing rule if present)
    await firewallService.addRule(javaPort, 'tcp', `${data.name} (Minecraft)`, { reuseExisting: true });
  ...
  async deleteServer(
    ...
    // Remove the firewall rule opened at server creation (best-effort)
    try {
      await firewallService.removeRuleByPort(existing.java_port, 'tcp');
    } catch (error) {
  ...
  async renameBackup(filename: string, newFilename: string) {
    return backupService.renameBackup(filename, newFilename);
  }
