  ...
  async backupServerAsync(
    ...
    const doBackup = async () => {
      try {
        const result =
          paths && paths.length > 0
            ? await backupService.createSelectiveBackup(existing.path, existing.name, paths, source, undefined, filename)
            : await backupService.createIncrementalBackup(existing.path, existing.name, id, source, undefined, filename);
