...
import { addFirewallRule, removeFirewallRuleByPort } from '@shulkr/backend/services/firewall_service';
...
export async function createServer(deps: Deps, data: CreateServerRequest) {
  ...
  await addFirewallRule(getAppDeps(), javaPort, 'tcp', `${data.name} (Minecraft)`, { reuseExisting: true });
...
export async function deleteServer(
  ...
  try {
    await removeFirewallRuleByPort(getAppDeps(), existing.java_port, 'tcp');
  } catch (error) {
