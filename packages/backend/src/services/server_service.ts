...
import { deleteDatabasesForServer } from '@shulkr/backend/services/database_service';
...
export async function deleteServer(
  ...
  if (options?.createBackup) {
    backupResult = await createFullBackup(getAppDeps(), existing.path, existing.name, 'manual', options.onBackupProgress, undefined, id);
  ...
  // The SQLite cascade only removes rows. Without this call the MariaDB databases would survive with nothing pointing at them, invisible in the panel and removable only over SSH.
  try {
    await deleteDatabasesForServer(getAppDeps(), id);
  } catch (error) {
    console.warn(`Failed to drop MariaDB databases for server ${id}:`, error);
  }
...
async function runCloudPostBackup(id: string, filename: string): Promise<void> {
  try {
...
export async function backupServer(
  ...
  if (result.filename) {
    await runCloudPostBackup(id, result.filename);
  }
...
export async function backupServerAsync(
  ...
  const doBackup = async () => {
    try {
      ...
      await runCloudPostBackup(id, filename);
    } catch (error: unknown) {
