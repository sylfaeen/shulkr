...
import { webhooks } from '@shulkr/backend/db/schema';
import { PLUGINS_BY_TYPE, MC_SETTINGS_BY_TYPE, clampRam, getSizePresets } from '@shulkr/backend/services/wizard_presets';
import { computeAikarFlags, type CreateFirstServerInput } from '@shulkr/shared';

export type ProvisionResult = {
  serverId: string;
  name: string;
  pendingPluginInstalls: Array<string>;
};
...
export async function provisionFirstServer(input: CreateFirstServerInput): Promise<ProvisionResult> {
  ...
  if (input.backup.frequency !== 'off') {
    if (input.backup.destination === 'cloud' && input.backup.cloudDestinationId) {
      await setServerStrategy(server.id, {
        ...
        cloudRetentionDays: input.backup.retentionDays,
      });
    } else {
  ...
  const allowedPluginIds = new Set(PLUGINS_BY_TYPE[input.type].map((p) => p.id));
  const pendingPluginInstalls = input.selectedPluginIds.filter((id) => allowedPluginIds.has(id));

  return { serverId: server.id, name: server.name, pendingPluginInstalls };
}
