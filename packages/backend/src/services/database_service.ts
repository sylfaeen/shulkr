...
function buildCredentials(
  deps: Deps,
  row: { db_name: string; db_user: string; slug: string },
  password: string
): DatabaseCredentials {
  return {
    ...
    password,
    connectionUrl: buildTunnelConnectionUrl({
      sshUser: deps.config.SSH_TUNNEL_USER,
      sshHost: getServerPublicIp(),
      dbUser: row.db_user,
      dbName: row.db_name,
      label: row.slug,
      password,
...
export async function createServerDatabase(
  ...
  try {
    ...
    return {
      ...
      credentials: buildCredentials(deps, row, password),
    };
  } catch (error) {
...
export function readDatabaseCredentials(deps: Deps, row: ServerDatabaseRow): DatabaseCredentials {
  return buildCredentials(deps, row, cipherDecrypt(deps, row.password_encrypted));
}
...
export async function rotateDatabasePassword(deps: Deps, row: ServerDatabaseRow): Promise<DatabaseCredentials> {
  ...
  return buildCredentials(deps, row, password);
}
