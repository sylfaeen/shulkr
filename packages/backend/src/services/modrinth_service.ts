...
export async function searchModrinthProjects(options: SearchOptions): Promise<ModrinthSearchResult> {
...
export async function getModrinthVersions(projectId: string, options?: VersionFilterOptions): Promise<Array<ModrinthVersion>> {
  const params = new URLSearchParams();
...
export async function getModrinthCategories(): Promise<Array<ModrinthCategory>> {
...
export async function downloadModrinthPlugin(
  ...
  const fileStream = deps.fs.createWriteStream(tmpPath);
  ...
  try {
    ...
    await deps.fs.unlink(tmpPath).catch(() => {});
    throw error;
  }
}
