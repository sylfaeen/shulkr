...
export async function createShareLink(
  ...
  const expiresAt = input.expiresInHours ? new Date(now.getTime() + input.expiresInHours * 3_600_000).toISOString() : null;
...
export async function revokeShareLink(deps: Deps, id: number): Promise<boolean> {
  ...
  await deps.db.update(backupShareLinks).set({ revoked_at: deps.clock().toISOString() }).where(eq(backupShareLinks.id, id));
