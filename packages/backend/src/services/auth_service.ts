...
export class AuthService {
  ...
  validateCredentials = (username: string, password: string) => validateCredentials(getAppDeps(), username, password);
  generateAccessToken = (user: typeof users.$inferSelect) => generateAccessToken(this.fastify.jwt, user);
  generateTotpToken = (userId: number) => generateTotpToken(this.fastify.jwt, userId);
  verifyTotpToken = (token: string) => verifyTotpToken(this.fastify.jwt, token);
  generateRefreshToken = () => generateRefreshToken();
  createSession = (userId: number, refreshToken: string) => createSession(getAppDeps(), userId, refreshToken);
  validateRefreshToken = (refreshToken: string) => validateRefreshToken(getAppDeps(), refreshToken);
  invalidateSession = (refreshToken: string) => invalidateSession(getAppDeps(), refreshToken);
  invalidateAllUserSessions = (userId: number) => invalidateAllUserSessions(getAppDeps(), userId);
  login = (username: string, password: string) => login(getAppDeps(), this.fastify.jwt, username, password);
  refresh = (refreshToken: string) => refresh(getAppDeps(), this.fastify.jwt, refreshToken);
}
