...
// Per-server metrics pushed by the shulkr-core plugin. IMPORTANT: this table is kept SEPARATE from `metrics_history` on purpose. The panel must behave identically with or without the plugin installed, so the existing metrics pipeline (metrics_service + tps_service parsing) is never mixed with agent data. This table only feeds the Analytics page.
