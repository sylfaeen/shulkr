...
import { scheduledTasks } from '@shulkr/backend/db/schema/tasks';

export type TaskExecutionStatus = 'pending' | 'running' | 'success' | 'error' | 'skipped';
...
export const taskExecutions = sqliteTable('task_executions', {
  ...
  error: text('error'),
  output: text('output'), // JSON: { logs: string[], stepResults: Array<{ step, status, duration, error? }> }
