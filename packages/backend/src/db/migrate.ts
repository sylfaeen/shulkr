...
function createFoundationTables(sqlite: SqliteHandle): void {
  sqlite.exec(`
    ...
    CREATE TABLE IF NOT EXISTS global_ip_bans (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
...
function applyDataMigrations(sqlite: SqliteHandle): void {
  ...
  // Story 59.7: rewrite IPv4-mapped IPv6 bans (`::ffff:1.2.3.4`) to their canonical IPv4 form so isGloballyBanned() lookups work after normalizeIp() learned to decode them. Idempotent: no-op once all rows are normalized.
  normalizeLegacyIpv4MappedBans(sqlite);
}

function normalizeLegacyIpv4MappedBans(sqlite: SqliteHandle): void {
  const legacyRows = sqlite.prepare(`SELECT id, ip FROM global_ip_bans WHERE ip LIKE '::ffff:%'`).all() as Array<{
    id: number;
    ip: string;
  }>;
  if (legacyRows.length === 0) return;

  const findByIp = sqlite.prepare(`SELECT id FROM global_ip_bans WHERE ip = ? LIMIT 1`);
  const updateIp = sqlite.prepare(`UPDATE global_ip_bans SET ip = ? WHERE id = ?`);
  const deleteRow = sqlite.prepare(`DELETE FROM global_ip_bans WHERE id = ?`);

  for (const row of legacyRows) {
    const suffix = row.ip.slice(7);
    if (isIP(suffix) !== 4) continue;
    const existing = findByIp.get(suffix) as { id: number } | undefined;
    if (existing && existing.id !== row.id) {
      deleteRow.run(row.id);
    } else {
      updateIp.run(suffix, row.id);
    }
  }
}
