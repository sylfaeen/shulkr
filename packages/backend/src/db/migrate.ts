...
export async function initializeDatabase(sqlite: SqliteHandle): Promise<void> {
  ...
  createDatabaseManagerTables(sqlite);
  createPublicFileTables(sqlite);
...
function createDatabaseManagerTables(sqlite: SqliteHandle): void {
  // servers predates the database manager, so the prefix column is added in place on existing installations.
  const serverColumns = sqlite.prepare(`PRAGMA table_info(servers)`).all() as Array<{ name: string }>;

  if (serverColumns.length > 0 && !serverColumns.some((c) => c.name === 'db_prefix')) {
    sqlite.exec(`ALTER TABLE servers ADD COLUMN db_prefix TEXT;`);
  }
  ...
  sqlite.exec(`
    ...
    CREATE UNIQUE INDEX IF NOT EXISTS idx_server_databases_server_slug ON server_databases(server_id, slug);
    ...
    CREATE TABLE IF NOT EXISTS database_accesses (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      database_id INTEGER NOT NULL REFERENCES server_databases(id) ON DELETE CASCADE,
      label TEXT NOT NULL,
      username TEXT NOT NULL UNIQUE,
      password_encrypted TEXT NOT NULL,
      scope TEXT NOT NULL,
      allowed_ip TEXT NOT NULL,
      require_certificate INTEGER NOT NULL DEFAULT 0,
      last_used_at TEXT,
      created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
      updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
    );

    CREATE UNIQUE INDEX IF NOT EXISTS idx_database_accesses_db_ip_scope ON database_accesses(database_id, allowed_ip, scope);
    CREATE INDEX IF NOT EXISTS idx_database_accesses_database ON database_accesses(database_id);
  `);
}

function createPublicFileTables(sqlite: SqliteHandle): void {
  sqlite.exec(`
    ...
    CREATE TABLE IF NOT EXISTS public_file_uploads (
      id TEXT PRIMARY KEY,
      user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      original_name TEXT NOT NULL,
      size_bytes INTEGER NOT NULL,
      ...
      max_downloads INTEGER,
      replaces_file_id INTEGER REFERENCES public_files(id) ON DELETE CASCADE,
      created_at TEXT NOT NULL,
      updated_at TEXT NOT NULL
    );

    CREATE INDEX IF NOT EXISTS idx_public_files_expires ON public_files(expires_at);
    CREATE INDEX IF NOT EXISTS idx_public_file_uploads_user ON public_file_uploads(user_id);
  `);
}
