...
export async function initializeDatabase(): Promise<void> {
  ...
  if (!serverColumnNames.has('auto_restart_on_crash')) {
    sqlite.exec(`ALTER TABLE servers ADD COLUMN auto_restart_on_crash INTEGER NOT NULL DEFAULT 1`);
  }
  sqlite.exec(`
  ...
  // Epic 57: Shulkr-Core Plugin — server_agents + agent_metrics_history
  sqlite.exec(`
  ...
  `);
  // Schema migration: add platform columns if table existed before
  const agentColumns = sqlite.prepare("SELECT name FROM pragma_table_info('server_agents')").all() as Array<{
    name: string;
  }>;
  const agentColumnNames = new Set(agentColumns.map((c) => c.name));
  if (!agentColumnNames.has('platform')) {
    sqlite.exec(`ALTER TABLE server_agents ADD COLUMN platform TEXT`);
  }
  if (!agentColumnNames.has('platform_version')) {
    sqlite.exec(`ALTER TABLE server_agents ADD COLUMN platform_version TEXT`);
  }
  sqlite.exec(`
