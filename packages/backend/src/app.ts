...
// Builds a fully-wired Fastify instance from an AppDeps container. Used both by the production entry point (index.ts) and by createTestApp() so that tests exercise the same stack as production. Out of scope (handled by the caller in index.ts): service initializations (taskScheduler, jobQueue, etc.), auto-starting servers, firewall sync, static frontend serving + SPA fallback, listening on a port.
