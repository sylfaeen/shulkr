...
validate_action() {
  if ! [[ "$1" =~ ^(install|create-db|drop-db|stream-dump|rotate-password|grant-access|revoke-access|set-bind|usage|prune-dumps)$ ]]; then
    json_error "Invalid action: must be install, create-db, drop-db, stream-dump, rotate-password, grant-access, revoke-access, set-bind, usage or prune-dumps"
  fi
}
...
ACTION="${1:-}"
[ -n "$ACTION" ] || json_error "Usage: $0 <install|create-db|drop-db|stream-dump|rotate-password|grant-access|revoke-access|set-bind|usage|prune-dumps> [args...]"
