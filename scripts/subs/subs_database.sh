...
validate_action() {
  if ! [[ "$1" =~ ^(install|renew-certificates|create-db|drop-db|stream-dump|rotate-password|grant-access|reissue-certificate|revoke-access|set-bind|usage|prune-dumps)$ ]]; then
    json_error "Invalid action: must be install, renew-certificates, create-db, drop-db, stream-dump, rotate-password, grant-access, reissue-certificate, revoke-access, set-bind, usage or prune-dumps"
  fi
}
...
# skip-name-resolve is what makes the per-IP grants work. Without it MariaDB reverse-resolves every client and compares the resulting hostname to the grant, so an access pinned to an address is refused as soon as that address has a PTR record. It also removes a DNS dependency from the connection path.
write_config() {
  local conf_dir
  conf_dir="$(dirname "$CONF_FILE")"
  mkdir -p "$conf_dir"

  # Reapplying the configuration must not silently close a remote access: whatever bind-address is in place wins over the local default.
  local bind_address="127.0.0.1"
  if [ -f "$CONF_FILE" ]; then
    local existing
    existing=$(grep -E '^bind-address' "$CONF_FILE" | awk -F'= *' '{print $2}' | tr -d '[:space:]')
    [ -n "$existing" ] && bind_address="$existing"
  fi
...
# Traversable so the panel can serve the CA, which is public by nature. The private keys inside stay 600 and unreadable.
apply_ssl_permissions() {
  chown -R mysql:mysql "$SSL_DIR" 2>/dev/null || true
  chmod 755 "$SSL_DIR"
  chmod 600 "${SSL_DIR}"/*.pem 2>/dev/null || true
  chmod 644 "${SSL_DIR}/ca.pem" 2>/dev/null || true
}

# A client that verifies the CA also verifies the name it dialled. Without these alternative names, a connection to the server's IP is refused with "Cannot connect to MySQL using SSL", whatever the CA says.
server_certificate_config() {
  local conf="$1"
  local ip hostname_fqdn
...

  {
    ...
    echo "DNS.2 = ${hostname_fqdn}"
    echo "IP.1 = 127.0.0.1"
    [ -n "$ip" ] && echo "IP.2 = ${ip}"
    local extra index=3
    for extra in ${SHULKR_DB_EXTRA_SAN:-}; do
      if [[ "$extra" =~ ^[0-9.]+$ ]]; then
        echo "IP.${index} = ${extra}"
      else
        echo "DNS.${index} = ${extra}"
      fi
      index=$((index + 1))
    done
  } > "$conf"
...
generate_certificates() {
  ...
  # Permissions are reapplied on every run: an early return would leave a machine installed with older, wrong modes stuck with them.
  ...
  local conf="${SSL_DIR}/server-ext.cnf"
  server_certificate_config "$conf"
...
# Forces a new authority and a new server certificate, which is what an instance installed before the alternative names existed needs, or one whose address changed. Every client must then take the new CA.
action_renew_certificates() {
  rm -f "${SSL_DIR}"/ca.pem "${SSL_DIR}"/ca-key.pem "${SSL_DIR}"/server-cert.pem "${SSL_DIR}"/server-key.pem "${SSL_DIR}"/ca.srl
  generate_certificates

  local svc
  svc="$(service_name)"
  systemctl restart "$svc" >/dev/null 2>&1 || json_error "MariaDB failed to restart with the new certificates"

  json_success "renew-certificates"
}
...
# Writes the dump to stdout so the panel can stream it straight to the browser. Nothing lands on disk: a dump file owned by the shulkr user would be readable by every Minecraft server on this machine for as long as it exists.
action_stream_dump() {
  local dbname="$1"
  ...
  local exists
  exists=$(echo "SELECT COUNT(*) FROM information_schema.schemata WHERE schema_name='${dbname}';" | run_sql)
  if [ "$exists" = "0" ]; then json_error "Database '${dbname}' does not exist"; fi

  local bin
  bin="$(dump_bin)"
  "$bin" --protocol=socket -u root --single-transaction --routines "$dbname" 2>/dev/null | gzip
}
...
# Issues a fresh client certificate for an existing access. Nothing is stored: the panel shows the certificate and its key once, and the private key never exists on the server outside this call.
action_reissue_certificate() {
  ...
  local ssl_type
  ssl_type=$(echo "SELECT ssl_type FROM mysql.user WHERE User='${username}' AND Host='${ip}';" | run_sql)

  if [ -z "$ssl_type" ]; then json_error "Access '${username}'@'${ip}' does not exist"; fi
  if [ "$ssl_type" != "X509" ]; then json_error "Access '${username}'@'${ip}' does not require a client certificate"; fi

  CLIENT_CERT_JSON=""
  CLIENT_KEY_JSON=""
  issue_client_certificate "$username"

  # The password changes with the certificate. REQUIRE X509 accepts any certificate signed by our authority, so a reissue alone would leave the previous one working; rotating the password makes the old pair useless.
  run_sql << SQL || json_error "Failed to rotate the password while reissuing the certificate"
ALTER USER '${username}'@'${ip}' IDENTIFIED BY '${password}';
FLUSH PRIVILEGES;
SQL

  echo "{\"success\":true,\"action\":\"reissue-certificate\",\"clientCert\":\"${CLIENT_CERT_JSON}\",\"clientKey\":\"${CLIENT_KEY_JSON}\"}"
}
...
ACTION="${1:-}"
[ -n "$ACTION" ] || json_error "Usage: $0 <install|renew-certificates|create-db|drop-db|stream-dump|rotate-password|grant-access|reissue-certificate|revoke-access|set-bind|usage|prune-dumps> [args...]"

validate_action "$ACTION"

case "$ACTION" in
  install)         action_install ;;
  renew-certificates) action_renew_certificates ;;
  create-db)       action_create_db "${2:-}" "${3:-}" ;;
  drop-db)         action_drop_db "${2:-}" ;;
  stream-dump)     action_stream_dump "${2:-}" ;;
  rotate-password) action_rotate_password "${2:-}" "${3:-}" ;;
  grant-access)    action_grant_access "${2:-}" "${3:-}" "${4:-}" "${5:-}" "${6:-ssl}" ;;
  reissue-certificate) action_reissue_certificate "${2:-}" "${3:-}" ;;
