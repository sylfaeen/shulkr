...
# Story 59.9: defense-in-depth against chpasswd newline injection. A control character (notably \n, \r, \0) in $password would let `echo "$user:$password" | chpasswd` rewrite the password of an arbitrary OS account.
validate_password() {
  local password="$1"
  if [ -z "$password" ]; then
    json_error "Password cannot be empty"
  fi
  if [[ "$password" =~ [[:cntrl:]] ]]; then
    json_error "Invalid password: control characters not allowed"
  fi
}
