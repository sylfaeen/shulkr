...
action_update_panel() {
  ...
  # Catch-all vhost for IP access. It keeps serving the panel instead of redirecting to the domain: a redirect to http:// on an HSTS-preloaded TLD (.dev, .app, ...) is upgraded to https:// by the browser, and until a certificate exists nothing answers there, which locked the administrator out of both the domain and the IP. enable-ssl turns it into an https redirect once the certificate is installed.
  local port
  port=$(grep -oP 'proxy_pass http://127\.0\.0\.1:\K[0-9]+' "$panel_vhost" | head -1 || true)
  port="${port:-3001}"
  local fallback_vhost="${SITES_AVAILABLE}/shulkr-fallback"
  cat > "$fallback_vhost" << FALLBACK
server {
