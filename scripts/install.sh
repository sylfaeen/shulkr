...
install_dependencies() {
    ...
    # Story 61.5: pnpm is no longer required at install time.
    # The shulkr release tarball is self-contained (story 61.2): node_modules is
    # bundled with prebuilt natives, so no package manager is needed on the host.
...
download_shulkr() {
    ...
    # Story 61.5: dependencies are bundled in the tarball (story 61.2).
    # The backend boots directly from packages/backend/dist/index.js using the
    # pre-populated packages/backend/node_modules. No install step required.
...
# An admin with no key on file loses every way back in the moment password
# authentication goes off, and only the provider's rescue console gets them
# back. Look for a usable key before touching the global setting.
