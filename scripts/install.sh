    ...
    case $OS in
        ubuntu|debian|raspbian)
            ...
            if [[ -z "${OS_CODENAME:-}" ]]; then
                fail "Could not detect the distribution codename required by the Adoptium repository. Install Java manually: https://adoptium.net/installation/"
            fi
            ...
            if ! apt_update >/dev/null 2>&1; then
                fail "Failed to refresh package lists after adding the Adoptium repository for '${OS_CODENAME}'. Install Java manually: https://adoptium.net/installation/"
            fi
...
detect_system() {
    if [[ -f /etc/os-release ]]; then
        ...
        # Codename for third-party apt repositories (Adoptium). Minimal images ship without lsb-release, so os-release is the primary source.
...
set_automated_defaults() {
    ...
    PUBLIC_FILES_DIR="${SHULKR_HOME}/public-files"
...
install_dependencies() {
    ...
    # Install ufw on Debian-family (host firewall used by subs_firewall.sh).
    # On RHEL-family, firewalld/iptables is used instead (see configure_firewall).
    case $OS in
        ubuntu|debian|raspbian)
    ...
    # pnpm is no longer required at install time.
    # The shulkr release tarball is self-contained (story 61.2): node_modules is bundled with prebuilt natives, so no package manager is needed on the host.
    ...
    # MariaDB, backing engine for the per-server database manager.
    # Hardening (local bind, TLS, secure defaults) runs in download_shulkr once subs_database.sh is on disk. The package install is best-effort: a failure here leaves the panel working with the database feature unavailable.
    if command_exists mariadbd || command_exists mysqld; then
        print_ok "MariaDB"
    else
        apt_install mariadb-server >/dev/null 2>&1 &
        spinner $! "Installing MariaDB..."
        if command_exists mariadbd || command_exists mysqld; then
            print_ok "MariaDB installed"
        else
            print_warn "MariaDB could not be installed, the database manager will be unavailable"
        fi
    fi
}
...
download_shulkr() {
    ...
    # dependencies are bundled in the tarball (story 61.2).
    # The backend boots directly from packages/backend/dist/index.js using the pre-populated packages/backend/node_modules. No install step required.
    ...
    chmod +x "$APP_DIR/scripts/subs/subs_database.sh"
...
${SERVICE_USER} ALL=(root) NOPASSWD: ${APP_DIR}/scripts/subs/subs_database.sh
...
SUDOERS_EOF
    ...
    print_ok "Sudoers configured (firewall + domains + sftp + database + service)"
...
# The service user must never be able to modify what it is allowed to run as root.
# sudo checks the path of a script, not who owns it, so a shulkr-owned copy of any subs_*.sh is a direct path to root for anything running under that account: the backend, every Minecraft plugin, and anyone holding the SSH password.
# Owning a parent directory is enough to swap a root-owned file, so the whole application tree stays root-owned and only the two paths the backend genuinely writes to are handed over.
apply_ownership() {
    chown -R root:root "$SHULKR_HOME"
    chmod 755 "$SHULKR_HOME" "$APP_DIR"

    # Minecraft servers and their backups are the service user's working area
    mkdir -p "$SERVERS_DIR" "$BACKUPS_DIR"
    chown -R "$SERVICE_USER:$SERVICE_USER" "$SERVERS_DIR" "$BACKUPS_DIR"
    chmod 755 "$SERVERS_DIR" "$BACKUPS_DIR"

    # Files published through public links. 0700: nothing but the backend reads them, and the parent is root-owned so the backend cannot create this directory itself.
    mkdir -p "$PUBLIC_FILES_DIR"
    chown -R "$SERVICE_USER:$SERVICE_USER" "$PUBLIC_FILES_DIR"
    chmod 700 "$PUBLIC_FILES_DIR"

    # The panel database, the only file the backend writes inside app/. Derived from DATABASE_FILE rather than DATABASE_DIR, which the automated path never sets.
    local database_dir
    database_dir="$(dirname "$DATABASE_FILE")"
    mkdir -p "$database_dir"
    chown -R "$SERVICE_USER:$SERVICE_USER" "$database_dir"
    chmod 750 "$database_dir"

    # The .env is rewritten from the panel (settings:environment:write) and read at boot
...
# Hardens the MariaDB instance through the privileged script, which is the single implementation shared by install.sh and do_update.sh. Idempotent: it reapplies the configuration without touching existing data. A failure is reported and leaves the panel usable with the database manager unavailable.
provision_database_engine() {
    if [[ ! -x "$APP_DIR/scripts/subs/subs_database.sh" ]]; then
        return
    fi

    if ! command_exists mariadbd && ! command_exists mysqld; then
        print_warn "MariaDB not installed, skipping database engine setup"
        return
    fi

    "$APP_DIR/scripts/subs/subs_database.sh" install >/dev/null 2>&1 &
    spinner $! "Hardening MariaDB..."

    if "$APP_DIR/scripts/subs/subs_database.sh" usage >/dev/null 2>&1; then
        print_ok "MariaDB hardened (local bind, TLS, secure defaults)"
    else
        print_warn "MariaDB hardening failed, the database manager will be unavailable"
    fi
}
...
PUBLIC_FILES_PATH=${PUBLIC_FILES_DIR}
...
configure_firewall() {
    ...
    case $OS in
        ubuntu|debian|raspbian)
            ...
            print_ok "ufw rules applied (SSH, HTTP, HTTPS), enabled and persisted"
            ;;
        centos|rhel|rocky|almalinux|fedora)
            if ! command_exists firewall-cmd; then
                apt_install firewalld >/dev/null 2>&1 &
                spinner $! "Installing firewalld..."
            fi
            ...
            print_ok "firewalld rules applied (SSH, HTTP, HTTPS), enabled and persisted"
            ;;
    esac
}
...
# An admin with no key on file loses every way back in the moment password authentication goes off, and only the provider's rescue console gets them back. Look for a usable key before touching the global setting.
admin_ssh_key_present() {
    local users="root" group members user home keyfile

    for group in sudo wheel admin; do
        members=$(getent group "$group" 2>/dev/null | cut -d: -f4 | tr ',' ' ') || true
        if [[ -n "$members" ]]; then
            users="${users} ${members}"
        fi
    done
    ...
    for user in $users; do
        ...
        for keyfile in "${home}/.ssh/authorized_keys" "${home}/.ssh/authorized_keys2"; do
            [[ -f "$keyfile" ]] || continue
            # Drop comments and blank lines first, then look for a key type
            # anywhere on what is left: real entries may carry options in front.
            if grep -vE '^[[:space:]]*(#|$)' "$keyfile" 2>/dev/null \
                | grep -qE '(ssh-(rsa|dss|ed25519)|ecdsa-sha2)'; then
                return 0
            fi
        done
    done
...
EOF
        ...
        print_warn "No SSH key found for root or any sudo account"
        print_warn "Password authentication left enabled: turning it off would lock you out"
        print_warn "Add your key to /root/.ssh/authorized_keys, then run:"
        print_warn "  echo 'PasswordAuthentication no' >> /etc/ssh/sshd_config.d/49-shulkr.conf"
        print_warn "  systemctl reload ssh"
    fi

    # Allow password auth for the shulkr system user (panel SFTP access)
    cat >> /etc/ssh/sshd_config.d/49-shulkr.conf << EOF

# Allow password authentication for the shulkr system user
Match User ${SERVICE_USER}
    PasswordAuthentication yes
EOF
    print_ok "SFTP password authentication enabled for user '${SERVICE_USER}'"

    # Allow password auth for SFTP group (per-server SFTP accounts)
...
finalize() {
    ...
    if [[ "${SSH_HARDENING_SKIPPED:-false}" == true ]]; then
        echo -e "  ${YELLOW}⚠${NC}  ${WHITE}SSH:${NC} password authentication is still enabled"
        echo -e "  ${GRAY}No admin SSH key was found, so hardening was skipped to avoid locking you out.${NC}"
        echo ""
    fi
