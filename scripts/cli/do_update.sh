...
preflight() {
    ...
    # Node.js runtime check. better-sqlite3 v11 uses node-gyp, so its native
    # binding is ABI-locked to the Node major it was built for. CI ships Node 22
    # binaries, so user must run Node 22+. Fail fast with a clear remediation
    # rather than touching the system, services are still running at this point.
...
download_and_replace() {
    ...
    # Dependencies are bundled in the tarball: the backend boots directly from
    # packages/backend/dist/index.js using packages/backend/node_modules.
