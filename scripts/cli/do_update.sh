...
preflight() {
    ...
    # Node.js runtime check. Story 61.x bundles node_modules into the tarball,
    # which removes the pnpm dependency user-side BUT keeps a hard Node major
    # coupling: better-sqlite3 v11 uses node-gyp (not NAPI), so its compiled
    # binary is ABI-locked to the Node version it was built for. CI builds on
    # Node 22 (current LTS until oct. 2027, also required by pnpm 11), therefore
    # users must run Node 22+ to load the binding. Node 20 is EOL april 2026.
    # Older Node versions are auto-upgraded.
    if command_exists node; then
        CURRENT_NODE_MAJOR=$(node -v | cut -d'v' -f2 | cut -d'.' -f1)
    fi
    if (( CURRENT_NODE_MAJOR < REQUIRED_NODE_MAJOR )); then
        NEEDS_NODE_UPGRADE=1
        print_warn "Node.js v${CURRENT_NODE_MAJOR:-none} found, this update needs v${REQUIRED_NODE_MAJOR}+ (native binding ABI)"
    else
        print_ok "Node.js v$(node -v | cut -d'v' -f2)"
    fi

    # Already up to date?
    if [[ "$CURRENT_VERSION" == "$SHULKR_VERSION" ]] && (( ! NEEDS_NODE_UPGRADE )); then
        echo ""
    ...
    if (( NEEDS_NODE_UPGRADE )); then
        echo -e "  ${WHITE}Runtime:${NC} Node.js v${CURRENT_NODE_MAJOR:-none} → v${REQUIRED_NODE_MAJOR} (LTS, will upgrade via NodeSource)"
    fi
...
upgrade_runtime() {
    (( NEEDS_NODE_UPGRADE )) || return 0

    echo ""
    echo -e "${WHITE}Upgrading Node.js runtime${NC}"
    ...
    local setup_url
    case "$os_id" in
        ubuntu|debian|raspbian) setup_url="https://deb.nodesource.com/setup_${REQUIRED_NODE_MAJOR}.x" ;;
        centos|rhel|rocky|almalinux|fedora) setup_url="https://rpm.nodesource.com/setup_${REQUIRED_NODE_MAJOR}.x" ;;
        *) fail "Unsupported OS '${os_id}' for automatic Node.js upgrade. Install Node.js v${REQUIRED_NODE_MAJOR}+ manually, then re-run shulkr update." ;;
    esac

    (curl -fsSL "$setup_url" | bash -) >/dev/null 2>&1 &
    spinner $! "Configuring NodeSource repository..."
    print_ok "NodeSource repository configured"
    ...
    spinner $! "Installing Node.js v${REQUIRED_NODE_MAJOR}..."

    # Verify the new version actually took (services are still running, safe to abort)
    local new_major
    new_major=$(node -v 2>/dev/null | cut -d'v' -f2 | cut -d'.' -f1 || echo 0)
    if (( new_major < REQUIRED_NODE_MAJOR )); then
        fail "Node.js upgrade failed (still v${new_major}). System unchanged, services still running. Update aborted."
    fi
...
download_and_replace() {
    ...
    # Story 61.3: dependencies are bundled in the tarball (story 61.2). The backend
    # boots directly from packages/backend/dist/index.js using packages/backend/node_modules.
