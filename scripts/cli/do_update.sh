...
preflight() {
    ...
    # Node.js runtime check (this version requires pnpm 11, which needs Node 22+)
...
download_and_replace() {
    ...
    # Ensure pnpm matches the packageManager pin in the freshly extracted package.json.
    # Corepack reads packageManager and fetches the right pnpm version on first invocation.
    # Drop any npm-installed pnpm first so corepack's shim wins on PATH.
