...
preflight() {
    ...
    # Node.js runtime check. Since story 61.x (self-contained tarball with bundled
    # node_modules), shulkr no longer requires pnpm or a specific pnpm-compatible
    # Node version. The minimum is dictated by the backend code itself (Node 20,
    # tsup target node20, audit 61.1). Older Node versions are auto-upgraded as a
    # convenience for legacy installs.
    if command_exists node; then
        CURRENT_NODE_MAJOR=$(node -v | cut -d'v' -f2 | cut -d'.' -f1)
    fi
    if (( CURRENT_NODE_MAJOR < REQUIRED_NODE_MAJOR )); then
        NEEDS_NODE_UPGRADE=1
        print_warn "Node.js v${CURRENT_NODE_MAJOR:-none} found, this update needs v${REQUIRED_NODE_MAJOR}+"
    else
