...
if [[ $EUID -ne 0 ]]; then
    fail "This script must be run as root. Run: sudo shulkr uninstall"
fi
...
echo ""
echo -e "  ${GREEN}Packages will NOT be removed:${NC} Node.js, pnpm, Java, Nginx, Certbot, MariaDB"
echo ""
read -p "   Type 'DELETE' to confirm uninstall: " CONFIRM < /dev/tty
[[ "$CONFIRM" == "DELETE" ]] || { echo -e "\n  ${WHITE}Cancelled.${NC}\n"; exit 0; }

# Plugin data lives in MariaDB, outside SHULKR_HOME, so it survives the uninstall unless it is explicitly asked for. Keeping it is the default: dropping a database is not something a panel uninstall should do silently.
DROP_DATABASES="no"
if command -v mariadb &>/dev/null || command -v mysql &>/dev/null; then
    echo ""
    echo -e "  ${WHITE}Shulkr created MariaDB databases for your plugins.${NC}"
    echo -e "  ${GRAY}They are not removed by default. Their data (teams, economy, logs, ...) would be lost.${NC}"
    read -p "   Also delete Shulkr databases and their dumps? [y/N]: " DROP_CONFIRM < /dev/tty
    [[ "$DROP_CONFIRM" =~ ^[yY]$ ]] && DROP_DATABASES="yes"
fi
...
if [[ "$DROP_DATABASES" == "yes" ]]; then
    MYSQL_BIN=$(command -v mariadb || command -v mysql)
    SHULKR_DBS=$(echo "SELECT schema_name FROM information_schema.schemata WHERE schema_name LIKE 's\\_%';" | sudo "$MYSQL_BIN" --protocol=socket -u root --batch --skip-column-names 2>/dev/null || true)
    for DB in $SHULKR_DBS; do
        echo "DROP DATABASE IF EXISTS \`${DB}\`;" | sudo "$MYSQL_BIN" --protocol=socket -u root 2>/dev/null || true
    done
    SHULKR_USERS=$(echo "SELECT CONCAT(QUOTE(User), '@', QUOTE(Host)) FROM mysql.user WHERE User LIKE 'u\\_%' OR User LIKE 'a\\_%';" | sudo "$MYSQL_BIN" --protocol=socket -u root --batch --skip-column-names 2>/dev/null || true)
    while IFS= read -r MYSQL_USER; do
        [[ -n "$MYSQL_USER" ]] || continue
        echo "DROP USER IF EXISTS ${MYSQL_USER};" | sudo "$MYSQL_BIN" --protocol=socket -u root 2>/dev/null || true
    done <<< "$SHULKR_USERS"
    sudo rm -rf /var/lib/shulkr
    echo -e "  ${GREEN}✓${NC} Shulkr databases and dumps removed"
else
    echo -e "  ${GREEN}✓${NC} MariaDB databases kept (dumps in /var/lib/shulkr/db-dumps)"
fi
