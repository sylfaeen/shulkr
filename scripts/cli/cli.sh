...
do_debug() {
    ...
    # Sub-commands that only read the DB can run without sudo; bundle mode
    # enforces root inside do_debug.sh itself.
    if [[ "${1:-}" == "" || "${1:-}" == "bundle" ]]; then
        sudo SHULKR_HOME="$SHULKR_HOME" bash "$debug_script" "$@"
    else
