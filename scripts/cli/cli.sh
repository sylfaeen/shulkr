...
require_root() {
    [[ $EUID -eq 0 ]] && return 0
    echo -e "  ${RED}✗${NC} '${1}' requires root"
    echo -e "  ${GRAY}Run:${NC} ${CYAN}sudo shulkr ${1}${NC} ${GRAY}or log in as root${NC}"
    exit 1
}
...
do_update() {
    ...
    SHULKR_HOME="$SHULKR_HOME" bash "$update_script"
}
...
show_domains() {
    ...
    SHULKR_HOME="$SHULKR_HOME" bash "$domains_script"
}
...
do_uninstall() {
    ...
    SHULKR_HOME="$SHULKR_HOME" bash "$uninstall_script"
}

do_debug() {
    local debug_script="${SHULKR_HOME}/app/scripts/cli/do_debug.sh"
    if [[ ! -f "$debug_script" ]]; then
        echo -e "  ${RED}✗${NC} Debug script not found at ${debug_script}"
        exit 1
    fi
    # Sub-commands that only read the DB run unprivileged; bundle mode needs
    # root, and do_debug.sh enforces that on its own side too.
    if [[ "${1:-}" == "" || "${1:-}" == "bundle" ]]; then
        require_root "debug${1:+ $1}"
        SHULKR_HOME="$SHULKR_HOME" bash "$debug_script" "$@"
    else
        SHULKR_HOME="$SHULKR_HOME" bash "$debug_script" "$@"
    fi
}
...
show_help() {
    ...
    echo -e "  ${CYAN}debug${NC}       Collect diagnostic bundle or inspect live (see: shulkr debug help)"
    echo -e "  ${CYAN}version${NC}     Show current version"
    echo -e "  ${CYAN}uninstall${NC}   Remove Shulkr (keeps packages)"
    echo ""
    echo -e "  ${GRAY}start, stop, restart, update, domains, uninstall and debug${NC}"
    echo -e "  ${GRAY}require root: run them with sudo, or log in as root.${NC}"
    echo ""
}
