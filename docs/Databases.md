...
If you lose them, the access entry has a reissue button that signs a fresh pair. Note what that does and does not do: it hands you a new certificate, it does not invalidate the old one. `REQUIRE X509` accepts any certificate signed by our authority, whatever its subject. If a client certificate actually leaked, revoke the access and create a new one, which changes the MariaDB user and the password with it.
