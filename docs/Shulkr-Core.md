...
| --------- | --------- | --------------- | ---------------------------------------------- |
| Paper     | Supported | 1.19 to 26.2    | Recommended runtime                            |
| Folia     | Supported | 1.20.x to 26.2  | Same JAR as Paper, regional schedulers handled |
| Velocity  | Supported | 3.2 to 3.3.x    | Proxy mode, reports backends instead of worlds |
| Waterfall | Supported | 1.19 to 1.21.x  | Proxy mode, reports backends instead of worlds |

The Paper and Folia JAR is built against the Paper 26.2 API and compiled as Java 17 bytecode, so the same file loads on Java 17, 21 and 25 servers. Paper 26.x itself requires Java 25 on the server, the plugin adds no requirement of its own.
