...
_Configure_ sets up the domain and its SSL certificate as a single step:

1. It resolves the domain and refuses it unless it points to this server. The panel tells you which address the domain resolves to and which one this server answers on.
2. It points the Nginx vhost at the domain. Access through the IP address keeps serving the panel at this stage.
3. It issues the Let's Encrypt certificate. Only once the certificate is installed does the IP address start redirecting to `https://` on the domain.

If the certificate cannot be issued, the domain is rolled back, the panel stays reachable on its IP address, and the settings page shows what certbot reported. The panel is never left on a domain without a certificate.

Older versions redirected the IP address to the domain as soon as it was set, before any certificate existed. When the domain did not answer, both doors were closed at once. Installations configured that way are not changed by an update, see [Recovering access from the server](#recovering-access-from-the-server) if you are locked out.
...
Whole TLDs are HSTS-preloaded in every browser, among them `.dev`, `.app`, `.page` and other Google-operated ones. Browsers refuse plain HTTP on them, with no click-through, so a panel domain without a certificate is unreachable there. This is why _Configure_ issues the certificate itself and refuses to keep a domain it could not secure. A domain configured by an older version without SSL is still in that state: click **Enable SSL**, or use the commands below if you cannot reach the panel.
