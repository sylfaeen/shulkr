...
sudo shulkr start       # Start the service
sudo shulkr stop        # Stop the service
sudo shulkr restart     # Restart the service
shulkr logs             # Follow live logs (journalctl)
sudo shulkr update      # Update Shulkr to the latest version
sudo shulkr domains     # Show Nginx domains and test connectivity
sudo shulkr debug       # Collect a diagnostic bundle or inspect live (see: Troubleshooting)
shulkr version          # Show current version
sudo shulkr uninstall   # Remove Shulkr and all its files (keeps packages)
```

Commands that change the service or read system files need root. Run them from
an account with sudo, or as root. The `shulkr` system account that owns the
panel and serves SFTP is not an administrator and cannot run them.

For generating a diagnostic snapshot to share with the maintainer, or inspecting the service live, see the [Troubleshooting](Troubleshooting) page.
...
[Scheduled Tasks](Tasks) - Automate restarts and backups
